...
} from '@/data/spanish-5/palacio-final-test';
...
const LESSON_ID = 'es-palacio-final-test';
...
// 👑 AnimatedCrown Component - تاج ملكي فخم متوهج
function AnimatedCrown({ size = 200 }: { size?: number }) {
  return (
    <motion.div
    ...
    >
      {/* هالة ذهبية فخمة */}
      <motion.div
        className="absolute top-[5%] left-1/2 -translate-x-1/2 rounded-full blur-3xl"
        style={{
          width: size * 1.8,
          height: size * 1.4,
          background: 'radial-gradient(circle, rgba(252,211,77,1), rgba(234,179,8,0.6), transparent 70%)',
        }}
        animate={{ 
          opacity: [0.6, 1, 0.6],
          scale: [1, 1.2, 1],
      ...
      {/* شعاع ذهبي فخم من الأعلى */}
      <motion.div
        className="absolute top-0 left-1/2 -translate-x-1/2"
        style={{
          width: '6px',
          height: size * 0.9,
          background: 'linear-gradient(to bottom, rgba(252,211,77,1), rgba(234,179,8,0.6), transparent)',
          filter: 'blur(3px)',
        }}
        animate={{
          opacity: [0.7, 1, 0.7],
      ...
      {/* جواهر ملكية متلألئة */}
      {[
        { top: '10%', left: '10%', delay: 0, emoji: '💎', size: '20px' },
        { top: '15%', right: '10%', delay: 0.4, emoji: '💠', size: '18px' },
        { top: '35%', left: '5%', delay: 0.8, emoji: '⭐', size: '22px' },
        { top: '30%', right: '5%', delay: 1.2, emoji: '✨', size: '20px' },
        { top: '55%', left: '12%', delay: 0.2, emoji: '💎', size: '18px' },
        { top: '60%', right: '12%', delay: 0.6, emoji: '⭐', size: '20px' },
      ].map((jewel, i) => (
        <motion.div
          key={i}
          className="absolute"
          style={{ ...jewel, fontSize: jewel.size }}
          animate={{ 
            opacity: [0.4, 1, 0.4],
            scale: [0.8, 1.4, 0.8],
            rotate: [0, 360],
      ...
      <motion.div
        ...
        style={{ 
          fontSize: size * 1,
          filter: 'drop-shadow(0 20px 50px rgba(252,211,77,0.8))',
          lineHeight: 1,
        }}
      >
        👑
      ...
      <motion.div
        ...
        className="absolute bottom-2 left-1/2 -translate-x-1/2 flex gap-2 text-3xl opacity-90"
      >
        <span>🦁</span>
        <span>🏰</span>
        <span>🦁</span>
    ...
    <div className="w-full max-w-md mx-auto">
      ...
      <div className="w-full h-2.5 md:h-3 bg-black/40 rounded-full overflow-hidden border border-white/20">
        <motion.div 
          ...
          style={{ background: 'linear-gradient(to right, #FDE047, #EAB308, #FCD34D)' }}
          initial={{ width: 0 }}
...
function SpanishPalacioFinalTestInner() {
    ...
    <div className="relative min-h-screen overflow-hidden" 
      ...
      <div className="fixed inset-0 pointer-events-none" style={{ zIndex: 0 }}>
        ...
        <div className="absolute inset-0" style={{
          background: isMobile 
            ? 'linear-gradient(180deg, rgba(60,45,15,0.9) 0%, rgba(40,30,10,0.96) 100%)'
            : 'radial-gradient(ellipse at 20% 20%, rgba(100,70,20,0.92) 0%, rgba(70,50,15,0.95) 50%, rgba(40,30,10,0.98) 100%)',
        }} />
        {/* هالة ذهبية ملكية */}
        <motion.div
          className="absolute inset-0 opacity-30"
          style={{ background: `radial-gradient(ellipse 100% 60% at 50% 30%, rgba(252,211,77,0.6), transparent 70%)` }}
        ...
        <AnimatePresence mode="wait">
          ...
          {phase === 'intro' && (
            <motion.div key="intro"
              ...
              <AnimatedCrown size={isMobile ? 170 : 220} />
              ...
              <div className="space-y-2">
                ...
                <p className="text-xl md:text-2xl font-bold text-yellow-300">
                  👑 الاختبار الملكي النهائي 👑
                </p>
                <p className="text-sm md:text-base text-yellow-200/80 font-bold">
                  🏆 آخر اختبار في المنهج الأسباني كله! 🏆
                </p>
              </div>

              <div className="bg-black/60 backdrop-blur-md rounded-2xl p-4 md:p-6 border-2 border-yellow-400/40 space-y-3 text-right">
                ...
                <div className="grid grid-cols-2 gap-3 text-sm md:text-base">
                  ...
                  <div className="bg-white/5 rounded-xl p-2 text-center">
                    ...
                    <div className="text-[10px] md:text-xs text-white/60">تاج ملكي</div>
                  </div>
                </div>
              </div>

              <div className="bg-gradient-to-r from-yellow-500/20 to-orange-500/20 backdrop-blur-md rounded-2xl p-4 border-2 border-yellow-400/40">
                <p className="text-sm md:text-base text-yellow-100 font-bold">
                  👑 هذا الاختبار سيحدد إذا كنت جاهزاً لشهادة <span className="text-yellow-300 font-black">A2</span> الرسمية في اللغة الأسبانية!
                </p>
              </div>
              ...
              <motion.button
                ...
                whileTap={{ scale: 0.95 }}
                onClick={startTest}
                className="px-10 md:px-14 py-4 md:py-5 rounded-2xl font-black text-xl md:text-2xl text-white relative overflow-hidden"
                style={{ 
                  background: 'linear-gradient(135deg, #FCD34D, #EAB308, #A16207)', 
                  boxShadow: '0 20px 50px rgba(252,211,77,0.6)',
                  borderBottom: '6px solid #713F12'
                }}
              >
                <motion.div
                ...
                />
                <span className="relative">ابدأ التحدي الملكي 👑</span>
              </motion.button>
            </motion.div>
          )}
                ...
                return (
                  <>
                    <motion.div
                      ...
                      transition={{ type: 'spring', duration: 1 }}
                    >
                      <AnimatedCrown size={isMobile ? 140 : 200} />
                    </motion.div>
                    ...
                    <StarsDisplay stars={finalStars} size={isMobile ? 55 : 80} />
                    ...
                    <div className="bg-black/60 backdrop-blur-md rounded-2xl p-4 md:p-6 border-2 border-yellow-400/40 space-y-3">
                      ...
                      <p className="text-sm md:text-base text-white/90 pt-2 border-t border-white/10 font-bold">
                        {resultMsg.messageAr}
                      </p>
                    </div>
                    ...
                    {isExcellent && (
                      <motion.div
                        ...
                        transition={{ delay: 1 }}
                        className="bg-gradient-to-r from-yellow-500/40 to-orange-500/40 backdrop-blur-md rounded-2xl p-5 border-2 border-yellow-400/70"
                      >
                        <div className="flex items-center justify-center gap-2 mb-2">
                          <Crown size={32} className="text-yellow-300" />
                          <p className="text-lg md:text-2xl font-black text-yellow-200">
                            إنجاز أسطوري!
                          </p>
                          <Crown size={32} className="text-yellow-300" />
                        </div>
                        <p className="text-sm md:text-base text-white font-bold mb-2">
                          🎉 مبروك! أنت أكملت المنهج الأسباني بالكامل! 🎉
                        </p>
                        <p className="text-xs md:text-sm text-yellow-100 font-bold">
                          تعلمت <span className="text-yellow-300 font-black">5 خرائط</span> و<span className="text-yellow-300 font-black">40 معلم</span> و<span className="text-yellow-300 font-black">600 كلمة إسبانية</span>!
                        </p>
                        <p className="text-xs md:text-sm text-yellow-200 font-bold mt-2">
                          دلوقتي أنت جاهز لشهادة <span className="text-yellow-300 font-black">A2</span> الرسمية! 👑🇪🇸
                        </p>
                      </motion.div>
                    )}
                    ...
                    <div className="flex flex-col md:flex-row gap-3 justify-center pt-2">
                      ...
                      <motion.button
                        whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}
                        onClick={saveAndExit}
                        className="px-8 py-3 rounded-2xl font-black text-base text-white flex items-center justify-center gap-2 relative overflow-hidden"
                        style={{ 
                          background: 'linear-gradient(135deg, #FCD34D, #EAB308, #A16207)', 
                          boxShadow: '0 15px 40px rgba(252,211,77,0.6)',
                          borderBottom: '5px solid #713F12'
                        }}
                      >
                        <motion.div
                          className="absolute inset-0 bg-white/20"
                        ...
                        <span className="relative">العودة للخريطة 🗺️</span>
                      </motion.button>
                    </div>
                  </>
                );
      ...
      <ConfettiBurst trigger={confettiTrigger} x={confettiPos.x} y={confettiPos.y} 
        colors={['#FCD34D', '#EAB308', '#FDE047', '#FFD700', '#FFFFFF']} />
    </div>
  );
}

export default function SpanishPalacioFinalTestPage() {
  return (
    <Suspense fallback={<div className="min-h-screen bg-[#07090D] flex items-center justify-center text-white">Loading...</div>}>
      <SpanishPalacioFinalTestInner />
    </Suspense>
  );
}