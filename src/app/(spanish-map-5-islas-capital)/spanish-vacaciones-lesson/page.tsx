...
import { 
  SPANISH_VACACIONES_ITEMS, 
  SPANISH_VACACIONES_GROUPS, 
  generateSpanishVacacionesChoices,
  generateVacacionesSentenceWordPool,
  checkVacacionesSentenceOrder,
  type SpanishVacacionesItem
} from '@/data/spanish-5/vacaciones';
...
const LESSON_ID = 'es-palma-vacaciones';
...
function getCategoryStyle(category: 'Playa' | 'Viajar' | 'Diversion') {
  switch (category) {
    case 'Playa':
      return { bg: 'rgba(14,165,233,0.7)', border: '#0EA5E9', icon: '🏖️', label: 'شاطئ' };
    case 'Viajar':
      return { bg: 'rgba(139,92,246,0.7)', border: '#8B5CF6', icon: '✈️', label: 'سفر' };
    case 'Diversion':
      return { bg: 'rgba(236,72,153,0.7)', border: '#EC4899', icon: '🌴', label: 'متعة' };
  }
}

// ═══════════════════════════════════════
// 🌴 Swaying Palm Tree - نخلة متأرجحة
// ═══════════════════════════════════════
function SwayingPalm({ size = 32 }: { size?: number }) {
  return (
    <motion.div
      animate={{ 
        rotate: [-8, 8, -8],
...
// ═══════════════════════════════════════
// 🌊 Ocean Waves - موجات البحر
// ═══════════════════════════════════════
function OceanWaves({ size = 28 }: { size?: number }) {
  const waves = ['🌊', '💧', '🌊'];
  ...
  return (
    ...
    >
      ...
      <div 
        ...
        style={{
          background: `linear-gradient(180deg, ${item.gradient[0]}44, ${item.gradient[1]}77, #05192e)`,
      ...
      >
          ...
          style={{
            backgroundImage: `
              repeating-linear-gradient(90deg, 
                transparent 0px, 
                transparent 25px, 
                rgba(14,165,233,0.4) 25px, 
                rgba(14,165,233,0.4) 30px, 
                transparent 30px, 
                transparent 55px, 
                rgba(56,189,248,0.4) 55px, 
                rgba(56,189,248,0.4) 60px
              )
            `,
          }}
        />
        ...
        <div className="absolute top-3 left-1/2 -translate-x-1/2 z-20 flex flex-col items-center gap-1">
          ...
          >
            <Palmtree size={12} className="text-yellow-300" /> الإجازات
          </div>
...
function SpanishVacacionesLessonInner() {
  ...
  const [itemChoices, setItemChoices] = useState<SpanishVacacionesItem[]>([]);
  ...
  const group = SPANISH_VACACIONES_GROUPS[groupIdx];
  ...
  useEffect(() => {
    if (item && phase === 'listen') {
      setItemChoices(generateSpanishVacacionesChoices(item.word, 3));
  ...
  useEffect(() => {
    if (item && phase === 'build') {
      setAvailableWords(generateVacacionesSentenceWordPool(item));
      setSelectedWords([]);
      setStatus('idle');
  ...
  const handleItemChoice = (choice: SpanishVacacionesItem, e: React.MouseEvent) => {
  ...
  const handleSelectWord = (word: string, idx: number) => {
    ...
    if (newSelected.length === item.sentenceWords.length) {
      const isCorrect = checkVacacionesSentenceOrder(newSelected, item.sentenceWords);
      if (isCorrect) {
        setStatus('correct');
        setTimeout(() => {
    ...
    <div className="relative min-h-screen overflow-hidden" 
      ...
      <div className="fixed inset-0 pointer-events-none" style={{ zIndex: 0 }}>
        ...
        <div className="absolute inset-0" style={{
          background: isMobile 
            ? 'linear-gradient(180deg, rgba(5,25,50,0.8) 0%, rgba(5,15,40,0.92) 100%)'
            : 'radial-gradient(ellipse at 20% 20%, rgba(10,50,90,0.9) 0%, rgba(5,35,70,0.94) 50%, rgba(5,15,40,0.98) 100%)',
        }} />
        <motion.div
                ...
                <div className="flex items-center gap-2">
                  <div className="w-20 h-2 bg-white/15 rounded-full overflow-hidden border border-white/20">
                    <motion.div className="h-full rounded-full" 
                      style={{ background: 'linear-gradient(to right, #0EA5E9, #FCD34D)' }}
        ...
        <AnimatePresence mode="wait">
          ...
          {phase === 'listen' && item && (
            <motion.div key={`listen-${groupIdx}-${itemIdx}`}
              ...
              <div className={`flex ${isMobile ? 'flex-col' : 'flex-row'} items-center justify-center gap-6 md:gap-10`}>
                ...
                <VacacionesCard item={item} size={isMobile ? 200 : 280} mode="word" />
          ...
          {phase === 'build' && item && (
            <motion.div key={`build-${groupIdx}-${itemIdx}`}
              ...
              <div className={`flex ${isMobile ? 'flex-col' : 'flex-row'} items-center justify-center gap-6 md:gap-10`}>
                ...
                <VacacionesCard item={item} size={isMobile ? 220 : 280} mode="sentence" />
                ...
                <div className={`flex flex-col items-center gap-3 ${isMobile ? 'w-full max-w-sm' : 'flex-1 max-w-md'}`}>
                  ...
                  {status === 'correct' && (
                    <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }}
                      ...
                      <span className="font-black text-white text-sm">¡Vamos a la playa!</span>
                    </motion.div>
                  )}
                </div>
              </div>
          ...
          {phase === 'group-success' && (
            <motion.div key="success" initial={{ scale: 0.5, opacity: 0 }} animate={{ scale: 1, opacity: 1 }}
              ...
              <h2 className="text-3xl md:text-5xl font-black text-white">¡Qué vacaciones!</h2>
              ...
              <button 
                onClick={() => {
                  if (groupIdx < SPANISH_VACACIONES_GROUPS.length - 1) {
                    setGroupIdx(g => g + 1); 
                    setItemIdx(0); 
                    setPhase('listen');
                    setCorrectInGroup(0);
                ...
                style={{ background: 'linear-gradient(135deg, #0EA5E9, #FCD34D)', boxShadow: '0 10px 30px rgba(14,165,233,0.4)' }}
              >
                المجموعة التالية 🚀
              </button>
            </motion.div>
          ...
          {phase === 'all-done' && (
            <motion.div key="done" initial={{ opacity: 0 }} animate={{ opacity: 1 }}
              ...
              <div className="flex flex-wrap justify-center gap-2 max-w-2xl mx-auto mb-4">
                {SPANISH_VACACIONES_ITEMS.map((s, i) => (
              ...
              <h1 className="text-3xl md:text-5xl font-black text-white">¡Playa de Palma completa!</h1>
              <p className="text-lg md:text-xl text-white/60 max-w-md mx-auto">تعلمت 15 كلمة عن الإجازات والسفر + 15 جملة كاملة!</p>
              <p className="text-sm text-sky-300/80 max-w-md mx-auto">
                دلوقتي تقدر تسافر وتستمتع بالإجازة بالأسباني! 🏖️✈️
              </p>
              ...
              <button 
                ...
                style={{ 
                  background: 'linear-gradient(135deg, #0EA5E9, #FCD34D)', 
      ...
      <div className="fixed bottom-0 left-0 right-0 z-30 px-2 md:px-4 pb-1.5 pointer-events-none"
        ...
        <div className={`mx-auto pointer-events-auto ${isMobile ? 'max-w-md' : 'max-w-[1500px]'}`}>
          <div className="relative rounded-xl px-4 md:px-6 py-1.5 md:py-2"
            style={{
              background: 'linear-gradient(135deg, rgba(10,50,90,0.85), rgba(5,35,70,0.9))',
      ...
      <ConfettiBurst trigger={confettiTrigger} x={confettiPos.x} y={confettiPos.y} 
        colors={[item?.color || '#0EA5E9', '#FCD34D', '#ffffff']} />
    </div>
  );
}

export default function SpanishVacacionesLessonPage() {
  return (
    <Suspense fallback={<div className="min-h-screen bg-[#07090D] flex items-center justify-center text-white">Loading...</div>}>
      <SpanishVacacionesLessonInner />
    </Suspense>
  );
}