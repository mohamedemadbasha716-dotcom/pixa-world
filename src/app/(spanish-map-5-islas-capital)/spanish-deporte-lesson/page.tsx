...
import { 
  SPANISH_DEPORTE_ITEMS, 
  SPANISH_DEPORTE_GROUPS, 
  generateSpanishDeporteChoices,
  generateDeporteSentenceWordPool,
  checkDeporteSentenceOrder,
  type SpanishDeporteItem
} from '@/data/spanish-5/deporte';
...
const LESSON_ID = 'es-bernabeu-deporte';
...
function getCategoryStyle(category: 'Partido' | 'Equipo' | 'Aficionados') {
  switch (category) {
    case 'Partido':
      return { bg: 'rgba(234,179,8,0.7)', border: '#EAB308', icon: '🏟️', label: 'مباراة' };
    case 'Equipo':
      return { bg: 'rgba(124,58,237,0.7)', border: '#7C3AED', icon: '👥', label: 'فريق' };
    case 'Aficionados':
      return { bg: 'rgba(220,38,38,0.7)', border: '#DC2626', icon: '📣', label: 'مشجعين' };
  }
}

// ═══════════════════════════════════════
// ⚽ Spinning Ball - كرة قدم دوّارة
// ═══════════════════════════════════════
function SpinningBall({ size = 32 }: { size?: number }) {
  return (
    <motion.div
      animate={{ 
        rotate: 360,
...
// ═══════════════════════════════════════
// 🏆 Sparkling Trophy - كأس ذهبي متلألئ
// ═══════════════════════════════════════
function SparklingTrophy({ size = 28 }: { size?: number }) {
  return (
    <motion.div
      animate={{ 
        scale: [1, 1.15, 1],
  ...
  return (
    ...
    >
      ...
      <div 
        ...
        style={{
          background: `linear-gradient(180deg, ${item.gradient[0]}44, ${item.gradient[1]}77, #1a1005)`,
          borderColor: item.color,
          boxShadow: `0 20px 60px ${item.color}66, inset 0 2px 0 rgba(255,255,255,0.3)`,
        }}
      >
        <div className="absolute top-3 right-3 opacity-50">
          <SpinningBall size={28} />
        </div>
        <div className="absolute bottom-16 left-3 opacity-60">
          <SparklingTrophy size={24} />
        </div>
        
        {/* خطوط الملعب في الخلفية */}
        <div 
        ...
        <div className="absolute top-3 left-1/2 -translate-x-1/2 z-20 flex flex-col items-center gap-1">
          ...
          >
            <Award size={12} className="text-white" /> الرياضة
          </div>
...
function SpanishDeporteLessonInner() {
  ...
  const [itemChoices, setItemChoices] = useState<SpanishDeporteItem[]>([]);
  ...
  const group = SPANISH_DEPORTE_GROUPS[groupIdx];
  ...
  useEffect(() => {
    if (item && phase === 'listen') {
      setItemChoices(generateSpanishDeporteChoices(item.word, 3));
  ...
  useEffect(() => {
    if (item && phase === 'build') {
      setAvailableWords(generateDeporteSentenceWordPool(item));
      setSelectedWords([]);
      setStatus('idle');
  ...
  const handleItemChoice = (choice: SpanishDeporteItem, e: React.MouseEvent) => {
  ...
  const handleSelectWord = (word: string, idx: number) => {
    ...
    if (newSelected.length === item.sentenceWords.length) {
      const isCorrect = checkDeporteSentenceOrder(newSelected, item.sentenceWords);
      if (isCorrect) {
        setStatus('correct');
        setTimeout(() => {
    ...
    <div className="relative min-h-screen overflow-hidden" 
      ...
      <div className="fixed inset-0 pointer-events-none" style={{ zIndex: 0 }}>
        ...
        <div className="absolute inset-0" style={{
          background: isMobile 
            ? 'linear-gradient(180deg, rgba(50,35,10,0.8) 0%, rgba(35,25,5,0.92) 100%)'
            : 'radial-gradient(ellipse at 20% 20%, rgba(90,60,15,0.9) 0%, rgba(60,40,10,0.94) 50%, rgba(35,25,5,0.98) 100%)',
        }} />
        <motion.div
            ...
            {phase !== 'group-success' && phase !== 'all-done' && (
              <div className="flex justify-center mt-1.5">
                <div className="flex items-center gap-1 px-2 py-1 rounded-lg bg-black/50 backdrop-blur-md border border-white/15">
                  <Stepper currentStep={itemIdx} totalSteps={group?.items.length || 5} isMobile color={item?.color || '#EAB308'} />
                </div>
              </div>
            )}
          </>
          ...
          <div className="max-w-[1600px] mx-auto flex items-center justify-between gap-4">
            ...
            {phase !== 'group-success' && phase !== 'all-done' && (
              <div className="flex items-center gap-1 px-4 py-2 rounded-2xl bg-black/50 backdrop-blur-xl border-2 border-white/15">
                <Stepper currentStep={itemIdx} totalSteps={group?.items.length || 5} isMobile={false} color={item?.color || '#EAB308'} />
              </div>
            )}
                ...
                <div className="flex items-center gap-2">
                  <div className="w-20 h-2 bg-white/15 rounded-full overflow-hidden border border-white/20">
                    <motion.div className="h-full rounded-full" 
                      style={{ background: 'linear-gradient(to right, #EAB308, #FCD34D)' }}
        ...
        <AnimatePresence mode="wait">
          ...
          {phase === 'listen' && item && (
            <motion.div key={`listen-${groupIdx}-${itemIdx}`}
              ...
              <div className={`flex ${isMobile ? 'flex-col' : 'flex-row'} items-center justify-center gap-6 md:gap-10`}>
                ...
                <DeporteCard item={item} size={isMobile ? 200 : 280} mode="word" />
          ...
          {phase === 'build' && item && (
            <motion.div key={`build-${groupIdx}-${itemIdx}`}
              ...
              <div className={`flex ${isMobile ? 'flex-col' : 'flex-row'} items-center justify-center gap-6 md:gap-10`}>
                ...
                <DeporteCard item={item} size={isMobile ? 220 : 280} mode="sentence" />
                ...
                <div className={`flex flex-col items-center gap-3 ${isMobile ? 'w-full max-w-sm' : 'flex-1 max-w-md'}`}>
                  ...
                  {status === 'correct' && (
                    <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }}
                      ...
                      <span className="font-black text-white text-sm">¡Gooool!</span>
                    </motion.div>
                  )}
                </div>
              </div>
          ...
          {phase === 'group-success' && (
            <motion.div key="success" initial={{ scale: 0.5, opacity: 0 }} animate={{ scale: 1, opacity: 1 }}
              ...
              <h2 className="text-3xl md:text-5xl font-black text-white">¡Eres campeón!</h2>
              ...
              <button 
                onClick={() => {
                  if (groupIdx < SPANISH_DEPORTE_GROUPS.length - 1) {
                    setGroupIdx(g => g + 1); 
                    setItemIdx(0); 
                    setPhase('listen');
                    setCorrectInGroup(0);
                ...
                style={{ background: 'linear-gradient(135deg, #EAB308, #FCD34D)', boxShadow: '0 10px 30px rgba(234,179,8,0.4)' }}
              >
                المجموعة التالية 🚀
              </button>
            </motion.div>
          ...
          {phase === 'all-done' && (
            <motion.div key="done" initial={{ opacity: 0 }} animate={{ opacity: 1 }}
              ...
              <h1 className="text-3xl md:text-5xl font-black text-white">¡Estadio Bernabéu completo!</h1>
              <p className="text-lg md:text-xl text-white/60 max-w-md mx-auto">تعلمت 15 كلمة عن الرياضة المحترفة + 15 جملة كاملة!</p>
              <p className="text-sm text-yellow-300/80 max-w-md mx-auto">
                دلوقتي تقدر تشجع فريقك بالأسباني! ¡Vamos, campeón! ⚽🏆
              </p>
      ...
      <div className="fixed bottom-0 left-0 right-0 z-30 px-2 md:px-4 pb-1.5 pointer-events-none"
        ...
        <div className={`mx-auto pointer-events-auto ${isMobile ? 'max-w-md' : 'max-w-[1500px]'}`}>
          <div className="relative rounded-xl px-4 md:px-6 py-1.5 md:py-2"
            style={{
              background: 'linear-gradient(135deg, rgba(90,60,15,0.85), rgba(60,40,10,0.9))',
              backdropFilter: 'blur(30px)',
              border: '1.5px solid rgba(255,255,255,0.2)',
              boxShadow: `0 10px 30px rgba(0,0,0,0.5), 0 0 25px rgba(234,179,8,0.2)`,
            }}>
      ...
      <ToroBull mood={toroMood} message={toroMessage} idleGlowColor={item?.color || '#EAB308'} />
      <ConfettiBurst trigger={confettiTrigger} x={confettiPos.x} y={confettiPos.y} 
        colors={[item?.color || '#EAB308', '#FCD34D', '#ffffff']} />
    </div>
  );
}

export default function SpanishDeporteLessonPage() {
  return (
    <Suspense fallback={<div className="min-h-screen bg-[#07090D] flex items-center justify-center text-white">Loading...</div>}>
      <SpanishDeporteLessonInner />
    </Suspense>
  );
}