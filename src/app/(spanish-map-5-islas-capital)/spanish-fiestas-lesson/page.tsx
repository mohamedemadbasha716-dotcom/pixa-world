...
import { 
  ...
  PartyPopper, X, BookOpen
} from 'lucide-react';

import { 
  SPANISH_FIESTAS_ITEMS, 
  SPANISH_FIESTAS_GROUPS, 
  generateSpanishFiestasChoices,
  generateFiestasSentenceWordPool,
  checkFiestasSentenceOrder,
  type SpanishFiestasItem
} from '@/data/spanish-5/fiestas';
...
const LESSON_ID = 'es-puertasol-fiestas';
...
function getCategoryStyle(category: 'Fiesta' | 'Celebracion' | 'Ambiente') {
  switch (category) {
    case 'Fiesta':
      return { bg: 'rgba(220,38,38,0.7)', border: '#DC2626', icon: '🎉', label: 'أعياد' };
    case 'Celebracion':
      return { bg: 'rgba(234,179,8,0.7)', border: '#EAB308', icon: '🥳', label: 'احتفال' };
    case 'Ambiente':
      return { bg: 'rgba(139,92,246,0.7)', border: '#8B5CF6', icon: '🎊', label: 'جو' };
  }
}

// ═══════════════════════════════════════
// 🕛 Countdown Clock - ساعة تنازلية
// ═══════════════════════════════════════
function CountdownClock({ size = 32 }: { size?: number }) {
  const [count, setCount] = useState(12);

  useEffect(() => {
    const interval = setInterval(() => {
      setCount(prev => (prev === 1 ? 12 : prev - 1));
    }, 1000);
  ...
  return (
    ...
    >
      <div className="relative">
        <div style={{ fontSize: size * 0.7 }}>🕛</div>
        <div 
          className="absolute inset-0 flex items-center justify-center font-black text-white text-xs"
...
// ═══════════════════════════════════════
// 🎆 Fireworks - ألعاب نارية متلألئة
// ═══════════════════════════════════════
function Fireworks({ size = 26 }: { size?: number }) {
  const fireworks = ['🎆', '🎇', '✨', '🌟'];
  ...
  return (
    ...
    >
      ...
      <div 
        ...
        style={{
          background: `linear-gradient(180deg, ${item.gradient[0]}44, ${item.gradient[1]}77, #1a0511)`,
      ...
      >
        ...
        <div 
          ...
          style={{
            backgroundImage: `
              radial-gradient(circle at 15% 25%, rgba(252,211,77,0.9) 1px, transparent 3px),
              radial-gradient(circle at 85% 30%, rgba(252,211,77,0.8) 1px, transparent 3px),
              radial-gradient(circle at 25% 75%, rgba(220,38,38,0.8) 1px, transparent 3px),
              radial-gradient(circle at 75% 80%, rgba(252,211,77,0.9) 1px, transparent 3px),
              radial-gradient(circle at 50% 40%, rgba(255,255,255,0.9) 1px, transparent 3px)
            `,
          }}
        />
        ...
        <div className="absolute top-3 left-1/2 -translate-x-1/2 z-20 flex flex-col items-center gap-1">
          ...
          >
            <PartyPopper size={12} className="text-yellow-300" /> الأعياد
          </div>
...
function SpanishFiestasLessonInner() {
  ...
  const [itemChoices, setItemChoices] = useState<SpanishFiestasItem[]>([]);
  ...
  const group = SPANISH_FIESTAS_GROUPS[groupIdx];
  ...
  useEffect(() => {
    if (item && phase === 'listen') {
      setItemChoices(generateSpanishFiestasChoices(item.word, 3));
  ...
  useEffect(() => {
    if (item && phase === 'build') {
      setAvailableWords(generateFiestasSentenceWordPool(item));
      setSelectedWords([]);
      setStatus('idle');
  ...
  const handleItemChoice = (choice: SpanishFiestasItem, e: React.MouseEvent) => {
  ...
  const handleSelectWord = (word: string, idx: number) => {
    ...
    if (newSelected.length === item.sentenceWords.length) {
      const isCorrect = checkFiestasSentenceOrder(newSelected, item.sentenceWords);
      if (isCorrect) {
        setStatus('correct');
        setTimeout(() => {
    ...
    <div className="relative min-h-screen overflow-hidden" 
      ...
      <div className="fixed inset-0 pointer-events-none" style={{ zIndex: 0 }}>
        ...
        <div className="absolute inset-0" style={{
          background: isMobile 
            ? 'linear-gradient(180deg, rgba(50,10,15,0.8) 0%, rgba(35,5,10,0.92) 100%)'
            : 'radial-gradient(ellipse at 20% 20%, rgba(90,15,25,0.9) 0%, rgba(60,10,20,0.94) 50%, rgba(35,5,10,0.98) 100%)',
        }} />
        <motion.div
                ...
                <div className="flex items-center gap-2">
                  <div className="w-20 h-2 bg-white/15 rounded-full overflow-hidden border border-white/20">
                    <motion.div className="h-full rounded-full" 
                      style={{ background: 'linear-gradient(to right, #DC2626, #FCD34D)' }}
        ...
        <AnimatePresence mode="wait">
          ...
          {phase === 'listen' && item && (
            <motion.div key={`listen-${groupIdx}-${itemIdx}`}
              ...
              <div className={`flex ${isMobile ? 'flex-col' : 'flex-row'} items-center justify-center gap-6 md:gap-10`}>
                ...
                <FiestasCard item={item} size={isMobile ? 200 : 280} mode="word" />
          ...
          {phase === 'build' && item && (
            <motion.div key={`build-${groupIdx}-${itemIdx}`}
              ...
              <div className={`flex ${isMobile ? 'flex-col' : 'flex-row'} items-center justify-center gap-6 md:gap-10`}>
                ...
                <FiestasCard item={item} size={isMobile ? 220 : 280} mode="sentence" />
                ...
                <div className={`flex flex-col items-center gap-3 ${isMobile ? 'w-full max-w-sm' : 'flex-1 max-w-md'}`}>
                  ...
                  {status === 'correct' && (
                    <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }}
                      ...
                      <span className="font-black text-white text-sm">¡Fiesta!</span>
                    </motion.div>
                  )}
                </div>
              </div>
          ...
          {phase === 'group-success' && (
            <motion.div key="success" initial={{ scale: 0.5, opacity: 0 }} animate={{ scale: 1, opacity: 1 }}
              ...
              <h2 className="text-3xl md:text-5xl font-black text-white">¡Qué fiesta!</h2>
              ...
              <button 
                onClick={() => {
                  if (groupIdx < SPANISH_FIESTAS_GROUPS.length - 1) {
                    setGroupIdx(g => g + 1); 
                    setItemIdx(0); 
                    setPhase('listen');
                    setCorrectInGroup(0);
                ...
                style={{ background: 'linear-gradient(135deg, #DC2626, #FCD34D)', boxShadow: '0 10px 30px rgba(220,38,38,0.4)' }}
              >
                المجموعة التالية 🚀
              </button>
            </motion.div>
          ...
          {phase === 'all-done' && (
            <motion.div key="done" initial={{ opacity: 0 }} animate={{ opacity: 1 }}
              ...
              <h1 className="text-3xl md:text-5xl font-black text-white">¡Puerta del Sol completa!</h1>
              <p className="text-lg md:text-xl text-white/60 max-w-md mx-auto">تعلمت 15 كلمة عن الأعياد والاحتفالات + 15 جملة كاملة!</p>
              <p className="text-sm text-red-300/80 max-w-md mx-auto">
                دلوقتي تقدر تحتفل بالأسباني! ¡Feliz Año Nuevo! 🎊🎉
              </p>
      ...
      <ConfettiBurst trigger={confettiTrigger} x={confettiPos.x} y={confettiPos.y} 
        colors={[item?.color || '#DC2626', '#FCD34D', '#ffffff']} />
    </div>
  );
}

export default function SpanishFiestasLessonPage() {
  return (
    <Suspense fallback={<div className="min-h-screen bg-[#07090D] flex items-center justify-center text-white">Loading...</div>}>
      <SpanishFiestasLessonInner />
    </Suspense>
  );
}