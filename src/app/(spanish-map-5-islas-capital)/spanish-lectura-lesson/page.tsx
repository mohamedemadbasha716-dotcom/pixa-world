...
import { 
  ...
  BookMarked, X, BookOpen
} from 'lucide-react';

import { 
  SPANISH_LECTURA_ITEMS, 
  SPANISH_LECTURA_GROUPS, 
  generateSpanishLecturaChoices,
  generateLecturaSentenceWordPool,
  checkLecturaSentenceOrder,
  type SpanishLecturaItem
} from '@/data/spanish-5/lectura';
...
const LESSON_ID = 'es-biblioteca-lectura';
...
function getCategoryStyle(category: 'Biblioteca' | 'Escribir' | 'Aprender') {
  switch (category) {
    case 'Biblioteca':
      return { bg: 'rgba(161,98,7,0.7)', border: '#A16207', icon: '📖', label: 'مكتبة' };
    case 'Escribir':
      return { bg: 'rgba(234,179,8,0.7)', border: '#EAB308', icon: '✍️', label: 'كتابة' };
    case 'Aprender':
      return { bg: 'rgba(124,58,237,0.7)', border: '#7C3AED', icon: '🎓', label: 'تعلم' };
  }
}

// ═══════════════════════════════════════
// 📖 Open Book - كتاب مفتوح متحرك
// ═══════════════════════════════════════
function OpenBook({ size = 32 }: { size?: number }) {
  const books = ['📖', '📕', '📗', '📘'];
  ...
  return (
    <motion.div
      ...
      style={{ 
        ...
        fontSize: size,
        lineHeight: 1,
        filter: 'drop-shadow(0 4px 12px rgba(161,98,7,0.7))',
      }}
    >
      {books[idx]}
    </motion.div>
...
// ═══════════════════════════════════════
// 🖋️ Writing Pen - قلم يكتب
// ═══════════════════════════════════════
function WritingPen({ size = 26 }: { size?: number }) {
  return (
    <motion.div
      animate={{ 
        rotate: [-20, 20, -20],
      ...
      >
          ...
          style={{
            backgroundImage: `
              repeating-linear-gradient(0deg, 
                transparent 0px, 
                transparent 15px, 
                rgba(161,98,7,0.4) 15px, 
                rgba(161,98,7,0.4) 17px
              )
            `,
          }}
        />
        ...
        <div className="absolute top-3 left-1/2 -translate-x-1/2 z-20 flex flex-col items-center gap-1">
          <div 
            ...
            style={{ 
              ...
              borderColor: '#FCD34D',
              boxShadow: '0 4px 12px rgba(161,98,7,0.5)',
              fontFamily: "'Tajawal', sans-serif",
            }}
          >
            <BookMarked size={12} className="text-yellow-300" /> القراءة
          </div>
        ...
        {mode === 'word' ? (
          ...
          <div className="px-3 pb-3 flex flex-col items-center gap-2 z-10 w-full">
            ...
            >
              <BookOpen size={10} className="text-amber-300" />
              <span className="text-[10px] font-black text-amber-200" dir="ltr">
...
function SpanishLecturaLessonInner() {
  ...
  const [itemChoices, setItemChoices] = useState<SpanishLecturaItem[]>([]);
  ...
  const group = SPANISH_LECTURA_GROUPS[groupIdx];
  ...
  useEffect(() => {
    if (item && phase === 'listen') {
      setItemChoices(generateSpanishLecturaChoices(item.word, 3));
  ...
  useEffect(() => {
    if (item && phase === 'build') {
      setAvailableWords(generateLecturaSentenceWordPool(item));
      setSelectedWords([]);
      setStatus('idle');
  ...
  const handleItemChoice = (choice: SpanishLecturaItem, e: React.MouseEvent) => {
  ...
  const handleSelectWord = (word: string, idx: number) => {
    ...
    if (newSelected.length === item.sentenceWords.length) {
      const isCorrect = checkLecturaSentenceOrder(newSelected, item.sentenceWords);
      if (isCorrect) {
        setStatus('correct');
        setTimeout(() => {
    ...
    <div className="relative min-h-screen overflow-hidden" 
      ...
      <div className="fixed inset-0 pointer-events-none" style={{ zIndex: 0 }}>
        ...
        <div className="absolute inset-0" style={{
          background: isMobile 
            ? 'linear-gradient(180deg, rgba(50,30,10,0.8) 0%, rgba(35,20,5,0.92) 100%)'
            : 'radial-gradient(ellipse at 20% 20%, rgba(90,55,15,0.9) 0%, rgba(60,35,10,0.94) 50%, rgba(35,20,5,0.98) 100%)',
        }} />
        <motion.div
            ...
            {phase !== 'group-success' && phase !== 'all-done' && (
              <div className="flex justify-center mt-1.5">
                <div className="flex items-center gap-1 px-2 py-1 rounded-lg bg-black/50 backdrop-blur-md border border-white/15">
                  <Stepper currentStep={itemIdx} totalSteps={group?.items.length || 5} isMobile color={item?.color || '#A16207'} />
                </div>
              </div>
            )}
          </>
          ...
          <div className="max-w-[1600px] mx-auto flex items-center justify-between gap-4">
            ...
            {phase !== 'group-success' && phase !== 'all-done' && (
              <div className="flex items-center gap-1 px-4 py-2 rounded-2xl bg-black/50 backdrop-blur-xl border-2 border-white/15">
                <Stepper currentStep={itemIdx} totalSteps={group?.items.length || 5} isMobile={false} color={item?.color || '#A16207'} />
              </div>
            )}
                ...
                <div className="flex items-center gap-2">
                  <div className="w-20 h-2 bg-white/15 rounded-full overflow-hidden border border-white/20">
                    <motion.div className="h-full rounded-full" 
                      style={{ background: 'linear-gradient(to right, #A16207, #FCD34D)' }}
        ...
        <AnimatePresence mode="wait">
          ...
          {phase === 'listen' && item && (
            <motion.div key={`listen-${groupIdx}-${itemIdx}`}
              ...
              <div className={`flex ${isMobile ? 'flex-col' : 'flex-row'} items-center justify-center gap-6 md:gap-10`}>
                ...
                <LecturaCard item={item} size={isMobile ? 200 : 280} mode="word" />
          ...
          {phase === 'build' && item && (
            <motion.div key={`build-${groupIdx}-${itemIdx}`}
              ...
              <div className={`flex ${isMobile ? 'flex-col' : 'flex-row'} items-center justify-center gap-6 md:gap-10`}>
                ...
                <LecturaCard item={item} size={isMobile ? 220 : 280} mode="sentence" />
                ...
                <div className={`flex flex-col items-center gap-3 ${isMobile ? 'w-full max-w-sm' : 'flex-1 max-w-md'}`}>
                  ...
                  <div className="px-3 py-1.5 rounded-xl border flex items-center gap-2 text-center"
                    ...
                    <BookOpen size={12} className="text-amber-300 flex-shrink-0" />
                    <span className="text-[10px] font-bold text-amber-100">{item.grammarHint.rule}</span>
                  </div>
                  ...
                  {status === 'correct' && (
                    <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }}
                      ...
                      <span className="font-black text-white text-sm">¡Bien escrito!</span>
                    </motion.div>
                  )}
                </div>
              </div>
              ...
              <div className={`flex ${isMobile ? 'flex-col' : 'flex-row'} items-center justify-center gap-6 md:gap-10`}>
                ...
                <div className={`flex flex-col items-center gap-4 ${isMobile ? 'w-full max-w-sm' : 'flex-1 max-w-md'}`}>
                  ...
                  <div className="w-full p-3 rounded-2xl border-2"
                    ...
                    <div className="text-[10px] text-amber-300 font-black mb-2 text-center">
          ...
          {phase === 'group-success' && (
            <motion.div key="success" initial={{ scale: 0.5, opacity: 0 }} animate={{ scale: 1, opacity: 1 }}
              ...
              <h2 className="text-3xl md:text-5xl font-black text-white">¡Eres un lector!</h2>
              <p className="text-lg md:text-xl text-amber-400 font-bold">أنهيت {group.titleEs}</p>
              ...
              <button 
                onClick={() => {
                  if (groupIdx < SPANISH_LECTURA_GROUPS.length - 1) {
                    setGroupIdx(g => g + 1); 
                    setItemIdx(0); 
                    setPhase('listen');
                    setCorrectInGroup(0);
                ...
                style={{ background: 'linear-gradient(135deg, #A16207, #FCD34D)', boxShadow: '0 10px 30px rgba(161,98,7,0.4)' }}
              >
                المجموعة التالية 🚀
              </button>
            </motion.div>
          ...
          {phase === 'all-done' && (
            <motion.div key="done" initial={{ opacity: 0 }} animate={{ opacity: 1 }}
              ...
              <h1 className="text-3xl md:text-5xl font-black text-white">¡Biblioteca completa!</h1>
              <p className="text-lg md:text-xl text-white/60 max-w-md mx-auto">تعلمت 15 كلمة عن القراءة والكتابة + 15 جملة كاملة!</p>
              <p className="text-sm text-amber-300/80 max-w-md mx-auto">
                دلوقتي تقدر تقرا وتكتب بالأسباني! 📚✍️
              </p>
              ...
              <button 
                ...
                style={{ 
                  ...
                  boxShadow: '0 15px 40px rgba(161,98,7,0.5)',
      ...
      <div className="fixed bottom-0 left-0 right-0 z-30 px-2 md:px-4 pb-1.5 pointer-events-none"
        ...
        <div className={`mx-auto pointer-events-auto ${isMobile ? 'max-w-md' : 'max-w-[1500px]'}`}>
          <div className="relative rounded-xl px-4 md:px-6 py-1.5 md:py-2"
            style={{
              background: 'linear-gradient(135deg, rgba(90,55,15,0.85), rgba(60,35,10,0.9))',
              backdropFilter: 'blur(30px)',
              border: '1.5px solid rgba(255,255,255,0.2)',
              boxShadow: `0 10px 30px rgba(0,0,0,0.5), 0 0 25px rgba(161,98,7,0.2)`,
            }}>
      ...
      <ToroBull mood={toroMood} message={toroMessage} idleGlowColor={item?.color || '#A16207'} />
      <ConfettiBurst trigger={confettiTrigger} x={confettiPos.x} y={confettiPos.y} 
        colors={[item?.color || '#A16207', '#FCD34D', '#ffffff']} />
    </div>
  );
}

export default function SpanishLecturaLessonPage() {
  return (
    <Suspense fallback={<div className="min-h-screen bg-[#07090D] flex items-center justify-center text-white">Loading...</div>}>
      <SpanishLecturaLessonInner />
    </Suspense>
  );
}