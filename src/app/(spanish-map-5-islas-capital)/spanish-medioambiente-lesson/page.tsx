...
import { 
  SPANISH_MEDIOAMBIENTE_ITEMS, 
  SPANISH_MEDIOAMBIENTE_GROUPS, 
  generateSpanishMedioAmbienteChoices,
  generateMedioAmbienteSentenceWordPool,
  checkMedioAmbienteSentenceOrder,
  type SpanishMedioAmbienteItem
} from '@/data/spanish-5/medioambiente';
...
const LESSON_ID = 'es-teide-medioambiente';
...
function getCategoryStyle(category: 'Naturaleza' | 'Proteger' | 'Ecologia') {
  switch (category) {
    case 'Naturaleza':
      return { bg: 'rgba(249,115,22,0.7)', border: '#F97316', icon: '🌋', label: 'طبيعة' };
    case 'Proteger':
      return { bg: 'rgba(34,197,94,0.7)', border: '#22C55E', icon: '♻️', label: 'حماية' };
    case 'Ecologia':
      return { bg: 'rgba(22,163,74,0.7)', border: '#16A34A', icon: '🌱', label: 'بيئة' };
  }
}

// ═══════════════════════════════════════
// 🌋 Erupting Volcano - بركان بدخان
// ═══════════════════════════════════════
function EruptingVolcano({ size = 32 }: { size?: number }) {
  return (
    <motion.div
      animate={{ 
        scale: [1, 1.1, 1],
      ...
      style={{ 
        filter: 'drop-shadow(0 4px 12px rgba(249,115,22,0.7))',
      }}
    >
      <motion.div
        animate={{ 
...
// ═══════════════════════════════════════
// 🌿 Swaying Leaf - ورقة شجر متأرجحة
// ═══════════════════════════════════════
function SwayingLeaf({ size = 24 }: { size?: number }) {
  const leaves = ['🌿', '🍃', '🌱'];
  ...
  return (
    ...
    >
      ...
      <div 
        ...
        style={{
          background: `linear-gradient(180deg, ${item.gradient[0]}44, ${item.gradient[1]}77, #1a0f05)`,
          borderColor: item.color,
          boxShadow: `0 20px 60px ${item.color}66, inset 0 2px 0 rgba(255,255,255,0.3)`,
        }}
      >
        <div className="absolute top-3 right-3 opacity-50">
          <EruptingVolcano size={28} />
        </div>
        <div className="absolute bottom-16 left-3 opacity-60">
          <SwayingLeaf size={24} />
        </div>
        
        {/* بقع طبيعية في الخلفية */}
        <div 
          className="absolute inset-0 opacity-15 pointer-events-none"
          style={{
            backgroundImage: `
              radial-gradient(circle at 20% 30%, rgba(34,197,94,0.7) 20px, transparent 25px),
              radial-gradient(circle at 80% 40%, rgba(249,115,22,0.7) 15px, transparent 20px),
              radial-gradient(circle at 40% 80%, rgba(14,165,233,0.7) 18px, transparent 22px),
              radial-gradient(circle at 70% 70%, rgba(22,163,74,0.6) 12px, transparent 15px)
            `,
            filter: 'blur(12px)',
          }}
        />
        ...
        <div className="absolute top-3 left-1/2 -translate-x-1/2 z-20 flex flex-col items-center gap-1">
          ...
          >
            <Leaf size={12} className="text-green-300" /> البيئة
          </div>
        ...
        {mode === 'word' ? (
          ...
          <div className="px-3 pb-3 flex flex-col items-center gap-2 z-10 w-full">
            ...
            >
              ...
              <span className="text-[10px] font-black text-orange-200" dir="ltr">
...
function SpanishMedioAmbienteLessonInner() {
  ...
  const [itemChoices, setItemChoices] = useState<SpanishMedioAmbienteItem[]>([]);
  ...
  const group = SPANISH_MEDIOAMBIENTE_GROUPS[groupIdx];
  ...
  useEffect(() => {
    if (item && phase === 'listen') {
      setItemChoices(generateSpanishMedioAmbienteChoices(item.word, 3));
  ...
  useEffect(() => {
    if (item && phase === 'build') {
      setAvailableWords(generateMedioAmbienteSentenceWordPool(item));
      setSelectedWords([]);
      setStatus('idle');
  ...
  const handleItemChoice = (choice: SpanishMedioAmbienteItem, e: React.MouseEvent) => {
  ...
  const handleSelectWord = (word: string, idx: number) => {
    ...
    if (newSelected.length === item.sentenceWords.length) {
      const isCorrect = checkMedioAmbienteSentenceOrder(newSelected, item.sentenceWords);
      if (isCorrect) {
        setStatus('correct');
        setTimeout(() => {
  ...
  return (
    <div className="relative min-h-screen overflow-hidden" 
      ...
      <div className="fixed inset-0 pointer-events-none" style={{ zIndex: 0 }}>
        <img 
          src={isMobile ? "/maps/spanish-map-5-mob.webp" : "/maps/spanish-map-5-pc.webp"} 
          alt="bg" className="w-full h-full object-cover" 
        />
        <div className="absolute inset-0" style={{
          background: isMobile 
            ? 'linear-gradient(180deg, rgba(50,25,10,0.8) 0%, rgba(35,15,5,0.92) 100%)'
            : 'radial-gradient(ellipse at 20% 20%, rgba(90,45,15,0.9) 0%, rgba(60,30,10,0.94) 50%, rgba(35,15,5,0.98) 100%)',
        }} />
        <motion.div
            ...
            {phase !== 'group-success' && phase !== 'all-done' && (
              <div className="flex justify-center mt-1.5">
                <div className="flex items-center gap-1 px-2 py-1 rounded-lg bg-black/50 backdrop-blur-md border border-white/15">
                  <Stepper currentStep={itemIdx} totalSteps={group?.items.length || 5} isMobile color={item?.color || '#F97316'} />
                </div>
              </div>
            )}
          </>
          ...
          <div className="max-w-[1600px] mx-auto flex items-center justify-between gap-4">
            ...
            {phase !== 'group-success' && phase !== 'all-done' && (
              <div className="flex items-center gap-1 px-4 py-2 rounded-2xl bg-black/50 backdrop-blur-xl border-2 border-white/15">
                <Stepper currentStep={itemIdx} totalSteps={group?.items.length || 5} isMobile={false} color={item?.color || '#F97316'} />
              </div>
            )}

            <div className="flex items-center gap-3">
              <div className="flex flex-col items-end">
                <span className="text-[10px] font-bold text-white/80">المستوى A2.2</span>
                <div className="flex items-center gap-2">
                  <div className="w-20 h-2 bg-white/15 rounded-full overflow-hidden border border-white/20">
                    <motion.div className="h-full rounded-full" 
                      style={{ background: 'linear-gradient(to right, #F97316, #22C55E)' }}
        ...
        <AnimatePresence mode="wait">
          ...
          {phase === 'listen' && item && (
            <motion.div key={`listen-${groupIdx}-${itemIdx}`}
              ...
              <div className={`flex ${isMobile ? 'flex-col' : 'flex-row'} items-center justify-center gap-6 md:gap-10`}>
                ...
                <MedioAmbienteCard item={item} size={isMobile ? 200 : 280} mode="word" />
          ...
          {phase === 'build' && item && (
            <motion.div key={`build-${groupIdx}-${itemIdx}`}
              ...
              <div className={`flex ${isMobile ? 'flex-col' : 'flex-row'} items-center justify-center gap-6 md:gap-10`}>
                ...
                <MedioAmbienteCard item={item} size={isMobile ? 220 : 280} mode="sentence" />
                ...
                <div className={`flex flex-col items-center gap-3 ${isMobile ? 'w-full max-w-sm' : 'flex-1 max-w-md'}`}>
                  ...
                  <div className="px-3 py-1.5 rounded-xl border flex items-center gap-2 text-center"
                    ...
                    <span className="text-[10px] font-bold text-orange-100">{item.grammarHint.rule}</span>
                  </div>
                  ...
                  {status === 'correct' && (
                    <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }}
                      ...
                      <span className="font-black text-white text-sm">¡Protege la Tierra!</span>
                    </motion.div>
                  )}
                </div>
              </div>
          ...
          {phase === 'group-success' && (
            <motion.div key="success" initial={{ scale: 0.5, opacity: 0 }} animate={{ scale: 1, opacity: 1 }}
              ...
              <h2 className="text-3xl md:text-5xl font-black text-white">¡Eres un héroe ecológico!</h2>
              <p className="text-lg md:text-xl text-orange-400 font-bold">أنهيت {group.titleEs}</p>
              ...
              <button 
                onClick={() => {
                  if (groupIdx < SPANISH_MEDIOAMBIENTE_GROUPS.length - 1) {
                    setGroupIdx(g => g + 1); 
                    setItemIdx(0); 
                    setPhase('listen');
                    setCorrectInGroup(0);
                ...
                style={{ background: 'linear-gradient(135deg, #F97316, #22C55E)', boxShadow: '0 10px 30px rgba(249,115,22,0.4)' }}
              >
                المجموعة التالية 🚀
              </button>
            </motion.div>
          ...
          {phase === 'all-done' && (
            <motion.div key="done" initial={{ opacity: 0 }} animate={{ opacity: 1 }}
              ...
              <div className="flex flex-wrap justify-center gap-2 max-w-2xl mx-auto mb-4">
                {SPANISH_MEDIOAMBIENTE_ITEMS.map((s, i) => (
              ...
              <h1 className="text-3xl md:text-5xl font-black text-white">¡Teide Tenerife completo!</h1>
              <p className="text-lg md:text-xl text-white/60 max-w-md mx-auto">تعلمت 15 كلمة عن البيئة والطبيعة + 15 جملة كاملة!</p>
              <p className="text-sm text-orange-300/80 max-w-md mx-auto">
                دلوقتي تقدر تحمي البيئة بالأسباني! 🌋🌱
              </p>
              ...
              <button 
                onClick={() => { 
                  ...
                  router.push('/spanish-character-and-map?map=5'); 
                }}
      ...
      <div className="fixed bottom-0 left-0 right-0 z-30 px-2 md:px-4 pb-1.5 pointer-events-none"
        ...
        <div className={`mx-auto pointer-events-auto ${isMobile ? 'max-w-md' : 'max-w-[1500px]'}`}>
          <div className="relative rounded-xl px-4 md:px-6 py-1.5 md:py-2"
            style={{
              background: 'linear-gradient(135deg, rgba(90,45,15,0.85), rgba(60,30,10,0.9))',
              backdropFilter: 'blur(30px)',
              border: '1.5px solid rgba(255,255,255,0.2)',
              boxShadow: `0 10px 30px rgba(0,0,0,0.5), 0 0 25px rgba(249,115,22,0.2)`,
            }}>
      ...
      <ToroBull mood={toroMood} message={toroMessage} idleGlowColor={item?.color || '#F97316'} />
      <ConfettiBurst trigger={confettiTrigger} x={confettiPos.x} y={confettiPos.y} 
        colors={[item?.color || '#F97316', '#22C55E', '#ffffff']} />
    </div>
  );
}

export default function SpanishMedioAmbienteLessonPage() {
  return (
    <Suspense fallback={<div className="min-h-screen bg-[#07090D] flex items-center justify-center text-white">Loading...</div>}>
      <SpanishMedioAmbienteLessonInner />
    </Suspense>
  );
}