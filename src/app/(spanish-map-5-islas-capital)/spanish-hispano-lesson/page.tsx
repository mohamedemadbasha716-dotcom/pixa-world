...
import { 
  SPANISH_HISPANO_ITEMS, 
  SPANISH_HISPANO_GROUPS, 
  generateSpanishHispanoChoices,
  generateHispanoSentenceWordPool,
  checkHispanoSentenceOrder,
  type SpanishHispanoItem
} from '@/data/spanish-5/hispano';
...
const LESSON_ID = 'es-america-hispano';
...
function getCategoryStyle(category: 'Paises' | 'Cultura' | 'Diversidad') {
  switch (category) {
    case 'Paises':
      return { bg: 'rgba(6,182,212,0.7)', border: '#06B6D4', icon: '🌎', label: 'دول' };
    case 'Cultura':
      return { bg: 'rgba(236,72,153,0.7)', border: '#EC4899', icon: '🎭', label: 'ثقافة' };
    case 'Diversidad':
      return { bg: 'rgba(139,92,246,0.7)', border: '#8B5CF6', icon: '🌐', label: 'تنوع' };
  }
}

// ═══════════════════════════════════════
// 🌍 Rotating Globe - كرة أرضية دوّارة
// ═══════════════════════════════════════
function RotatingGlobe({ size = 32 }: { size?: number }) {
  const globes = ['🌍', '🌎', '🌏'];
...
// ═══════════════════════════════════════
// ✈️ Flying Airplane - طيارة تطير
// ═══════════════════════════════════════
function FlyingPlane({ size = 26 }: { size?: number }) {
  return (
    <motion.div
      animate={{ 
        x: [-15, 15, -15],
  ...
  return (
    ...
    >
      ...
      <div 
        ...
        style={{
          background: `linear-gradient(180deg, ${item.gradient[0]}44, ${item.gradient[1]}77, #2e0a1a)`,
      ...
      >
        ...
        <div 
          ...
          style={{
            backgroundImage: `
              radial-gradient(circle at 15% 25%, rgba(220,38,38,0.6) 8px, transparent 12px),
              radial-gradient(circle at 85% 30%, rgba(34,197,94,0.6) 8px, transparent 12px),
              radial-gradient(circle at 25% 75%, rgba(59,130,246,0.6) 8px, transparent 12px),
              radial-gradient(circle at 75% 80%, rgba(234,179,8,0.6) 8px, transparent 12px)
            `,
            filter: 'blur(6px)',
          }}
        />
        ...
        <div className="absolute top-3 left-1/2 -translate-x-1/2 z-20 flex flex-col items-center gap-1">
          ...
          >
            <Globe size={12} className="text-cyan-300" /> العالم الأسباني
          </div>
        ...
        {mode === 'word' ? (
          ...
          <div className="px-3 pb-3 flex flex-col items-center gap-2 z-10 w-full">
            ...
            >
              <BookOpen size={10} className="text-pink-300" />
              <span className="text-[10px] font-black text-pink-200" dir="ltr">
...
function SpanishHispanoLessonInner() {
  ...
  const [itemChoices, setItemChoices] = useState<SpanishHispanoItem[]>([]);
  ...
  const group = SPANISH_HISPANO_GROUPS[groupIdx];
  ...
  useEffect(() => {
    if (item && phase === 'listen') {
      setItemChoices(generateSpanishHispanoChoices(item.word, 3));
  ...
  useEffect(() => {
    if (item && phase === 'build') {
      setAvailableWords(generateHispanoSentenceWordPool(item));
      setSelectedWords([]);
      setStatus('idle');
  ...
  const handleItemChoice = (choice: SpanishHispanoItem, e: React.MouseEvent) => {
  ...
  const handleSelectWord = (word: string, idx: number) => {
    ...
    if (newSelected.length === item.sentenceWords.length) {
      const isCorrect = checkHispanoSentenceOrder(newSelected, item.sentenceWords);
      if (isCorrect) {
        setStatus('correct');
        setTimeout(() => {
    ...
    <div className="relative min-h-screen overflow-hidden" 
      ...
      <div className="fixed inset-0 pointer-events-none" style={{ zIndex: 0 }}>
        ...
        <div className="absolute inset-0" style={{
          background: isMobile 
            ? 'linear-gradient(180deg, rgba(50,10,35,0.8) 0%, rgba(35,5,25,0.92) 100%)'
            : 'radial-gradient(ellipse at 20% 20%, rgba(90,15,60,0.9) 0%, rgba(60,10,45,0.94) 50%, rgba(35,5,25,0.98) 100%)',
        }} />
        <motion.div
            ...
            {phase !== 'group-success' && phase !== 'all-done' && (
              <div className="flex justify-center mt-1.5">
                <div className="flex items-center gap-1 px-2 py-1 rounded-lg bg-black/50 backdrop-blur-md border border-white/15">
                  <Stepper currentStep={itemIdx} totalSteps={group?.items.length || 5} isMobile color={item?.color || '#EC4899'} />
                </div>
              </div>
            )}
          </>
          ...
          <div className="max-w-[1600px] mx-auto flex items-center justify-between gap-4">
            ...
            {phase !== 'group-success' && phase !== 'all-done' && (
              <div className="flex items-center gap-1 px-4 py-2 rounded-2xl bg-black/50 backdrop-blur-xl border-2 border-white/15">
                <Stepper currentStep={itemIdx} totalSteps={group?.items.length || 5} isMobile={false} color={item?.color || '#EC4899'} />
              </div>
            )}
                ...
                <div className="flex items-center gap-2">
                  <div className="w-20 h-2 bg-white/15 rounded-full overflow-hidden border border-white/20">
                    <motion.div className="h-full rounded-full" 
                      style={{ background: 'linear-gradient(to right, #EC4899, #06B6D4)' }}
        ...
        <AnimatePresence mode="wait">
          ...
          {phase === 'listen' && item && (
            <motion.div key={`listen-${groupIdx}-${itemIdx}`}
              ...
              <div className={`flex ${isMobile ? 'flex-col' : 'flex-row'} items-center justify-center gap-6 md:gap-10`}>
                ...
                <HispanoCard item={item} size={isMobile ? 200 : 280} mode="word" />
          ...
          {phase === 'build' && item && (
            <motion.div key={`build-${groupIdx}-${itemIdx}`}
              ...
              <div className={`flex ${isMobile ? 'flex-col' : 'flex-row'} items-center justify-center gap-6 md:gap-10`}>
                ...
                <HispanoCard item={item} size={isMobile ? 220 : 280} mode="sentence" />
                ...
                <div className={`flex flex-col items-center gap-3 ${isMobile ? 'w-full max-w-sm' : 'flex-1 max-w-md'}`}>
                  ...
                  <div className="px-3 py-1.5 rounded-xl border flex items-center gap-2 text-center"
                    ...
                    <BookOpen size={12} className="text-pink-300 flex-shrink-0" />
                    <span className="text-[10px] font-bold text-pink-100">{item.grammarHint.rule}</span>
                  </div>
                  ...
                  {status === 'correct' && (
                    <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }}
                      ...
                      <span className="font-black text-white text-sm">¡Mundo unido!</span>
                    </motion.div>
                  )}
                </div>
              </div>
              ...
              <div className={`flex ${isMobile ? 'flex-col' : 'flex-row'} items-center justify-center gap-6 md:gap-10`}>
                ...
                <div className={`flex flex-col items-center gap-4 ${isMobile ? 'w-full max-w-sm' : 'flex-1 max-w-md'}`}>
                  ...
                  <div className="w-full p-3 rounded-2xl border-2"
                    ...
                    <div className="text-[10px] text-pink-300 font-black mb-2 text-center">
          ...
          {phase === 'group-success' && (
            <motion.div key="success" initial={{ scale: 0.5, opacity: 0 }} animate={{ scale: 1, opacity: 1 }}
              ...
              <h2 className="text-3xl md:text-5xl font-black text-white">¡Ciudadano del mundo!</h2>
              <p className="text-lg md:text-xl text-pink-400 font-bold">أنهيت {group.titleEs}</p>
              ...
              <button 
                onClick={() => {
                  if (groupIdx < SPANISH_HISPANO_GROUPS.length - 1) {
                    setGroupIdx(g => g + 1); 
                    setItemIdx(0); 
                    setPhase('listen');
                    setCorrectInGroup(0);
                ...
                style={{ background: 'linear-gradient(135deg, #EC4899, #06B6D4)', boxShadow: '0 10px 30px rgba(236,72,153,0.4)' }}
              >
                المجموعة التالية 🚀
              </button>
            </motion.div>
          ...
          {phase === 'all-done' && (
            <motion.div key="done" initial={{ opacity: 0 }} animate={{ opacity: 1 }}
              ...
              <h1 className="text-3xl md:text-5xl font-black text-white">¡Casa de América completa!</h1>
              <p className="text-lg md:text-xl text-white/60 max-w-md mx-auto">تعلمت 15 كلمة عن العالم الأسباني + 15 جملة كاملة!</p>
              <p className="text-sm text-pink-300/80 max-w-md mx-auto">
                دلوقتي تعرف الدول الأسبانية وثقافتها! 🌍🌎🌏
              </p>
      ...
      <div className="fixed bottom-0 left-0 right-0 z-30 px-2 md:px-4 pb-1.5 pointer-events-none"
        ...
        <div className={`mx-auto pointer-events-auto ${isMobile ? 'max-w-md' : 'max-w-[1500px]'}`}>
          <div className="relative rounded-xl px-4 md:px-6 py-1.5 md:py-2"
            style={{
              background: 'linear-gradient(135deg, rgba(90,15,60,0.85), rgba(60,10,45,0.9))',
              backdropFilter: 'blur(30px)',
              border: '1.5px solid rgba(255,255,255,0.2)',
              boxShadow: `0 10px 30px rgba(0,0,0,0.5), 0 0 25px rgba(236,72,153,0.2)`,
            }}>
      ...
      <ToroBull mood={toroMood} message={toroMessage} idleGlowColor={item?.color || '#EC4899'} />
      <ConfettiBurst trigger={confettiTrigger} x={confettiPos.x} y={confettiPos.y} 
        colors={[item?.color || '#EC4899', '#06B6D4', '#ffffff']} />
    </div>
  );
}

export default function SpanishHispanoLessonPage() {
  return (
    <Suspense fallback={<div className="min-h-screen bg-[#07090D] flex items-center justify-center text-white">Loading...</div>}>
      <SpanishHispanoLessonInner />
    </Suspense>
  );
}