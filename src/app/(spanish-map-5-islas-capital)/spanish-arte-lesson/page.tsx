...
import { 
  SPANISH_ARTE_ITEMS, 
  SPANISH_ARTE_GROUPS, 
  generateSpanishArteChoices,
  generateArteSentenceWordPool,
  checkArteSentenceOrder,
  type SpanishArteItem
} from '@/data/spanish-5/arte';
...
function getCategoryStyle(category: 'Clasico' | 'Cultura' | 'Museo') {
  switch (category) {
    case 'Clasico':
      return { bg: 'rgba(124,58,237,0.7)', border: '#7C3AED', icon: '🎨', label: 'كلاسيكي' };
    case 'Cultura':
      return { bg: 'rgba(219,39,119,0.7)', border: '#DB2777', icon: '🎭', label: 'ثقافة' };
    case 'Museo':
      return { bg: 'rgba(59,130,246,0.7)', border: '#3B82F6', icon: '🏛️', label: 'متحف' };
  }
}

// ═══════════════════════════════════════
// 🎨 Swaying Palette - بالتة رسم متأرجحة
// ═══════════════════════════════════════
function SwayingPalette({ size = 32 }: { size?: number }) {
  return (
    <motion.div
      animate={{ 
        rotate: [-10, 10, -10],
...
// ═══════════════════════════════════════
// 🖌️ Paint Brush - فرشاة رسم متحركة
// ═══════════════════════════════════════
function PaintBrush({ size = 26 }: { size?: number }) {
  return (
    <motion.div
      animate={{ 
        rotate: [0, 25, -25, 0],
  ...
  return (
    ...
    >
      ...
      <div 
        ...
        style={{
          background: `linear-gradient(180deg, ${item.gradient[0]}44, ${item.gradient[1]}77, #1a0a2e)`,
      ...
      >
        <div className="absolute top-3 right-3 opacity-40">
          <SwayingPalette size={28} />
        </div>
        <div className="absolute bottom-16 left-3 opacity-50">
          <PaintBrush size={24} />
        </div>
        
        {/* بقع ألوان فنية في الخلفية */}
        <div 
          className="absolute inset-0 opacity-15 pointer-events-none"
          style={{
            backgroundImage: `
              radial-gradient(circle at 20% 25%, rgba(252,211,77,0.9) 15px, transparent 20px),
              radial-gradient(circle at 80% 30%, rgba(236,72,153,0.9) 12px, transparent 18px),
              radial-gradient(circle at 30% 75%, rgba(139,92,246,0.9) 18px, transparent 22px),
              radial-gradient(circle at 75% 80%, rgba(59,130,246,0.9) 10px, transparent 15px),
              radial-gradient(circle at 50% 50%, rgba(220,38,38,0.7) 8px, transparent 12px)
            `,
            filter: 'blur(10px)',
          }}
        />
        ...
        <div className="absolute top-3 left-1/2 -translate-x-1/2 z-20 flex flex-col items-center gap-1">
          ...
          >
            <Palette size={12} className="text-yellow-300" /> الفن والثقافة
          </div>
        ...
        {mode === 'word' ? (
          ...
          <div className="px-3 pb-3 flex flex-col items-center gap-2 z-10 w-full">
            ...
            >
              <BookOpen size={10} className="text-purple-300" />
              <span className="text-[10px] font-black text-purple-200" dir="ltr">
...
function SpanishArteLessonInner() {
  ...
  const [itemChoices, setItemChoices] = useState<SpanishArteItem[]>([]);
  ...
  const group = SPANISH_ARTE_GROUPS[groupIdx];
  ...
  useEffect(() => {
    if (item && phase === 'listen') {
      setItemChoices(generateSpanishArteChoices(item.word, 3));
  ...
  useEffect(() => {
    if (item && phase === 'build') {
      setAvailableWords(generateArteSentenceWordPool(item));
      setSelectedWords([]);
      setStatus('idle');
  ...
  const handleItemChoice = (choice: SpanishArteItem, e: React.MouseEvent) => {
  ...
  const handleSelectWord = (word: string, idx: number) => {
    ...
    if (newSelected.length === item.sentenceWords.length) {
      const isCorrect = checkArteSentenceOrder(newSelected, item.sentenceWords);
      if (isCorrect) {
        setStatus('correct');
        setTimeout(() => {
    ...
    <div className="relative min-h-screen overflow-hidden" 
      ...
      <div className="fixed inset-0 pointer-events-none" style={{ zIndex: 0 }}>
        ...
        <div className="absolute inset-0" style={{
          background: isMobile 
            ? 'linear-gradient(180deg, rgba(30,10,50,0.8) 0%, rgba(20,5,40,0.92) 100%)'
            : 'radial-gradient(ellipse at 20% 20%, rgba(60,20,90,0.9) 0%, rgba(40,15,70,0.94) 50%, rgba(20,5,40,0.98) 100%)',
        }} />
        <motion.div
            ...
            {phase !== 'group-success' && phase !== 'all-done' && (
              <div className="flex justify-center mt-1.5">
                <div className="flex items-center gap-1 px-2 py-1 rounded-lg bg-black/50 backdrop-blur-md border border-white/15">
                  <Stepper currentStep={itemIdx} totalSteps={group?.items.length || 5} isMobile color={item?.color || '#7C3AED'} />
                </div>
              </div>
            )}
          </>
          ...
          <div className="max-w-[1600px] mx-auto flex items-center justify-between gap-4">
            ...
            {phase !== 'group-success' && phase !== 'all-done' && (
              <div className="flex items-center gap-1 px-4 py-2 rounded-2xl bg-black/50 backdrop-blur-xl border-2 border-white/15">
                <Stepper currentStep={itemIdx} totalSteps={group?.items.length || 5} isMobile={false} color={item?.color || '#7C3AED'} />
              </div>
            )}
                ...
                <div className="flex items-center gap-2">
                  <div className="w-20 h-2 bg-white/15 rounded-full overflow-hidden border border-white/20">
                    <motion.div className="h-full rounded-full" 
                      style={{ background: 'linear-gradient(to right, #7C3AED, #FCD34D)' }}
        ...
        <AnimatePresence mode="wait">
          ...
          {phase === 'listen' && item && (
            <motion.div key={`listen-${groupIdx}-${itemIdx}`}
              ...
              <div className={`flex ${isMobile ? 'flex-col' : 'flex-row'} items-center justify-center gap-6 md:gap-10`}>
                ...
                <ArteCard item={item} size={isMobile ? 200 : 280} mode="word" />
          ...
          {phase === 'build' && item && (
            <motion.div key={`build-${groupIdx}-${itemIdx}`}
              ...
              <div className={`flex ${isMobile ? 'flex-col' : 'flex-row'} items-center justify-center gap-6 md:gap-10`}>
                ...
                <ArteCard item={item} size={isMobile ? 220 : 280} mode="sentence" />
                ...
                <div className={`flex flex-col items-center gap-3 ${isMobile ? 'w-full max-w-sm' : 'flex-1 max-w-md'}`}>
                  ...
                  <div className="px-3 py-1.5 rounded-xl border flex items-center gap-2 text-center"
                    ...
                    <span className="text-[10px] font-bold text-purple-100">{item.grammarHint.rule}</span>
                  </div>
                  ...
                  {status === 'correct' && (
                    <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }}
                      ...
                      <span className="font-black text-white text-sm">¡Obra maestra!</span>
                    </motion.div>
                  )}
                </div>
              </div>
              ...
              <div className={`flex ${isMobile ? 'flex-col' : 'flex-row'} items-center justify-center gap-6 md:gap-10`}>
                ...
                <div className={`flex flex-col items-center gap-4 ${isMobile ? 'w-full max-w-sm' : 'flex-1 max-w-md'}`}>
                  ...
                  <div className="w-full p-3 rounded-2xl border-2"
                    ...
                    <div className="text-[10px] text-purple-300 font-black mb-2 text-center">
          ...
          {phase === 'group-success' && (
            <motion.div key="success" initial={{ scale: 0.5, opacity: 0 }} animate={{ scale: 1, opacity: 1 }}
              ...
              <h2 className="text-3xl md:text-5xl font-black text-white">¡Eres un artista!</h2>
              <p className="text-lg md:text-xl text-purple-400 font-bold">أنهيت {group.titleEs}</p>
              ...
              <button 
                onClick={() => {
                  if (groupIdx < SPANISH_ARTE_GROUPS.length - 1) {
                    setGroupIdx(g => g + 1); 
                    setItemIdx(0); 
                    setPhase('listen');
                    setCorrectInGroup(0);
                ...
                style={{ background: 'linear-gradient(135deg, #7C3AED, #FCD34D)', boxShadow: '0 10px 30px rgba(124,58,237,0.4)' }}
              >
                المجموعة التالية 🚀
              </button>
            </motion.div>
          ...
          {phase === 'all-done' && (
            <motion.div key="done" initial={{ opacity: 0 }} animate={{ opacity: 1 }}
              ...
              <h1 className="text-3xl md:text-5xl font-black text-white">¡Museo del Prado completo!</h1>
              <p className="text-lg md:text-xl text-white/60 max-w-md mx-auto">تعلمت 15 كلمة عن الفن والثقافة + 15 جملة كاملة!</p>
              <p className="text-sm text-purple-300/80 max-w-md mx-auto">
                دلوقتي تقدر تتكلم عن الفن الأسباني! 🖼️🎨
              </p>
      ...
      <div className="fixed bottom-0 left-0 right-0 z-30 px-2 md:px-4 pb-1.5 pointer-events-none"
        ...
        <div className={`mx-auto pointer-events-auto ${isMobile ? 'max-w-md' : 'max-w-[1500px]'}`}>
          <div className="relative rounded-xl px-4 md:px-6 py-1.5 md:py-2"
            style={{
              background: 'linear-gradient(135deg, rgba(60,20,90,0.85), rgba(40,15,70,0.9))',
              backdropFilter: 'blur(30px)',
              border: '1.5px solid rgba(255,255,255,0.2)',
              boxShadow: `0 10px 30px rgba(0,0,0,0.5), 0 0 25px rgba(124,58,237,0.2)`,
            }}>
      ...
      <ToroBull mood={toroMood} message={toroMessage} idleGlowColor={item?.color || '#7C3AED'} />
      <ConfettiBurst trigger={confettiTrigger} x={confettiPos.x} y={confettiPos.y} 
        colors={[item?.color || '#7C3AED', '#FCD34D', '#ffffff']} />
    </div>
  );
}

export default function SpanishArteLessonPage() {
  return (
    <Suspense fallback={<div className="min-h-screen bg-[#07090D] flex items-center justify-center text-white">Loading...</div>}>
      <SpanishArteLessonInner />
    </Suspense>
  );
}