...
import { 
  Check, Volume2, Trophy, Home, Flame, Gem, Mic, SkipForward, Sparkles, ShoppingBasket
} from 'lucide-react';

import { 
  SPANISH_FRUITS, 
  SPANISH_FRUIT_GROUPS, 
  generateSpanishFruitChoices,
  compareSpanishFruitWords,
  type SpanishFruit
} from '@/data/spanish/fruits';
...
const LESSON_ID = 'es-ribera-fruits';
...
// 🛒 Market Card - الكارت الفخم على شكل صندوق سوق
function MarketCard({ fruit, size = 280, showSentence = true }: { 
  fruit: SpanishFruit; size?: number; showSentence?: boolean;
}) {
  const isFruta = fruit.type === 'Fruta';
  ...
  return (
    ...
    >
      <div 
        ...
        style={{ background: `radial-gradient(circle, ${fruit.color}aa, transparent 70%)` }} 
      />
      
      <div 
        className="relative w-full rounded-[2rem] overflow-hidden border-4 flex flex-col items-center"
        style={{
          background: `linear-gradient(180deg, ${fruit.gradient[0]}44, ${fruit.gradient[1]}77, #1a0a2e)`,
          borderColor: fruit.color,
          boxShadow: `0 20px 60px ${fruit.color}66, inset 0 2px 0 rgba(255,255,255,0.3)`,
        }}
      >
        {/* خطوط الصندوق الخشبي (decoration) */}
        <div className="absolute top-0 left-0 right-0 h-6 z-0"
          style={{
            background: `repeating-linear-gradient(90deg, ${fruit.color}33 0px, ${fruit.color}33 8px, transparent 8px, transparent 16px)`,
          }}
        />
        
        {/* Header - بطاقة العنوان */}
        <div className="absolute top-3 left-1/2 -translate-x-1/2 z-20 flex flex-col items-center gap-1">
          ...
          >
            <ShoppingBasket size={12} className="text-red-700" /> السوق
          </div>
          
          {/* شارة النوع (Fruta / Verdura) */}
          <div 
          ...
          >
            {isFruta ? '🍓' : '🥬'} {fruit.type} • {fruit.typeAr}
          </div>
        </div>

        {/* الإيموجي الكبير */}
        <div className="flex-1 flex items-center justify-center w-full pt-20 pb-4">
          <motion.div
            ...
            style={{ 
              fontSize: size * 0.5,
              filter: `drop-shadow(0 10px 25px ${fruit.color}cc)`,
              lineHeight: 1,
            }}
          >
            {fruit.emoji}
        ...
        {/* الأسماء */}
        <div className="px-4 pb-2 flex flex-col items-center gap-1.5 z-10">
          {/* أداة التعريف + الكلمة */}
          <div className="flex items-end gap-1.5" dir="ltr">
            <div 
              className="px-2 py-0.5 rounded-lg font-black text-white text-xs"
              style={{
            ...
            <div 
              ...
              style={{
                ...
                borderColor: 'rgba(255,255,255,0.5)',
                boxShadow: `0 4px 15px ${fruit.color}88`,
              }}
            >
              {fruit.word}
            </div>
          </div>
          <div className="text-base font-bold text-white/90">{fruit.wordAr}</div>
        </div>
...
function SpanishFruitsLessonInner() {
  ...
  const [fruitChoices, setFruitChoices] = useState<SpanishFruit[]>([]);
  ...
  const group = SPANISH_FRUIT_GROUPS[groupIdx];
  const fruit = group?.fruits[fruitIdx];
  ...
  useEffect(() => {
    const load = async () => {
      ...
      if (progress && !progress.completed) {
        ...
        setFruitIdx(progress.current_letter || 0);
      }
      setIsLoading(false);
    };
    load();
  ...
  useEffect(() => {
    if (fruit) {
      setFruitChoices(generateSpanishFruitChoices(fruit.word, 3));
      setTimeout(() => speakSpanishWord(fruit.word), 500);
    }
  }, [fruitIdx, groupIdx, phase]);
  ...
  const triggerCorrect = (cx: number, cy: number) => {
    ...
    setTimeout(() => {
      if (phase === 'listen') { setPhase('write'); setInput(''); setStatus('idle'); }
        ...
        if (fruitIdx < group.fruits.length - 1) {
          setFruitIdx(n => n + 1); setPhase('listen');
        } else {
  ...
  const handleFruitChoice = (choice: SpanishFruit, e: React.MouseEvent) => {
    if (status === 'correct') return;
    if (choice.word === fruit.word) {
      setStatus('correct');
      speakSpanishWord(fruit.word);
  ...
  const handleWriteCheck = (e?: any) => {
    if (compareSpanishFruitWords(input, fruit.word)) {
      setStatus('correct');
      speakSpanishWord(fruit.word);
    ...
    <div className="relative min-h-screen overflow-hidden" 
      ...
      <div className="fixed inset-0 pointer-events-none" style={{ zIndex: 0 }}>
        ...
        <motion.div
          ...
          style={{ background: `radial-gradient(ellipse 80% 50% at 50% 0%, ${fruit?.color}33, transparent 70%)` }}
        ...
        {isMobile ? (
          <>
            ...
            <div className="flex justify-center mt-1.5">
              <div className="flex items-center gap-1 px-2 py-1 rounded-lg bg-black/50 backdrop-blur-md border border-white/15">
                <Stepper currentStep={fruitIdx} totalSteps={group?.fruits.length || 5} isMobile color={fruit?.color || '#DC2626'} />
              </div>
            </div>
          </>
        ) : (
          <div className="max-w-[1600px] mx-auto flex items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-1 px-4 py-2 rounded-2xl bg-black/50 backdrop-blur-xl border-2 border-white/15">
              <Stepper currentStep={fruitIdx} totalSteps={group?.fruits.length || 5} isMobile={false} color={fruit?.color || '#DC2626'} />
            </div>
      ...
      <main className="relative z-10 min-h-screen flex items-center justify-center px-3 pt-24 md:pt-28 pb-28">
        <AnimatePresence mode="wait">
          ...
          {phase === 'listen' && fruit && (
            <motion.div key={`listen-${groupIdx}-${fruitIdx}`}
              ...
              <div className={`flex ${isMobile ? 'flex-col' : 'flex-row'} items-center justify-center gap-6 md:gap-10`}>
                ...
                <MarketCard fruit={fruit} size={isMobile ? 200 : 280} showSentence={false} />
                ...
                <div className={`flex flex-col items-center gap-4 ${isMobile ? 'w-full max-w-sm' : 'flex-1 max-w-md'}`}>
                  ...
                  <div className="px-5 py-2 rounded-2xl backdrop-blur-md border-2"
                    style={{ 
                      ...
                      borderColor: `${fruit.color}66`,
                    }}>
                    <span className="font-black text-white text-sm md:text-base">استمع واختر الصنف الصحيح</span>
                  </div>

                  <SoundButton 
                    onClick={() => speakSpanishWord(fruit.word)} 
                    color={fruit.color} 
                    size={isMobile ? 55 : 70}
                  />

                  <div className="flex flex-col gap-2 md:gap-3 w-full mt-2">
                    {fruitChoices.map((choice, idx) => {
                      const isWrong = wrongChoice === choice.word;
                      const isCorrect = status === 'correct' && choice.word === fruit.word;
                      return (
                        <motion.button
                          key={`${fruit.word}-${choice.word}-${idx}`}
  ...
  isWrong 
    ...
    : { delay: idx * 0.1, type: 'spring' }
                          ...
                          onClick={(e) => handleFruitChoice(choice, e)}
                          ...
                          <div className="flex-1 flex items-center justify-between gap-2">
                            <div className="text-left" dir="ltr">
                              ...
                              <div className="text-[9px] text-white/50 font-bold mt-0.5">
                                {choice.type === 'Fruta' ? '🍓 Fruta' : '🥬 Verdura'}
                              </div>
                            </div>
          ...
          {phase === 'write' && fruit && (
            <motion.div key={`write-${groupIdx}-${fruitIdx}`}
              ...
              <div className={`flex ${isMobile ? 'flex-col' : 'flex-row'} items-center justify-center gap-6 md:gap-10`}>
                ...
                <MarketCard fruit={fruit} size={isMobile ? 200 : 280} showSentence />
                ...
                <div className={`flex flex-col items-center gap-4 ${isMobile ? 'w-full max-w-sm' : 'flex-1 max-w-md'}`}>
                  ...
                  <div className="px-5 py-2 rounded-2xl backdrop-blur-md border-2"
                    ...
                    <span className="font-black text-white text-sm md:text-base">اكتب اسم الصنف بالإسباني</span>
                  </div>

                  {/* تلميح أداة التعريف */}
                  <div className="px-4 py-1.5 rounded-xl border-2 flex items-center gap-2"
                    style={{
                      background: fruit.article === 'la' ? 'rgba(236,72,153,0.2)' : 'rgba(59,130,246,0.2)',
                      borderColor: fruit.article === 'la' ? 'rgba(236,72,153,0.5)' : 'rgba(59,130,246,0.5)',
                    }}>
                    <span className="text-xs font-bold text-white/70">أداة التعريف:</span>
                    <span className="font-black text-white text-base" dir="ltr">{fruit.article}</span>
                    <span className="text-[10px] text-white/60">
                      ({fruit.article === 'la' ? 'مؤنث ♀' : 'مذكر ♂'})
                    </span>
                  </div>
                  ...
                  <div className="w-full space-y-3">
                    <GhostInput 
                      ...
                      ghostText={fruit.word} color={fruit.color} status={status}
                      fontSize={isMobile ? '1.5rem' : '1.8rem'}
                    />
                    
                    {getRequiredSpanishSpecialChars(fruit.word).length > 0 && (
                      <SpanishCharsKeyboard 
                        chars={getRequiredSpanishSpecialChars(fruit.word)}
                        onChar={(c) => setInput(prev => prev + c)} color={fruit.color} 
                      />
                    )}
                  </div>
          ...
          {phase === 'speak' && fruit && (
            <motion.div key={`speak-${groupIdx}-${fruitIdx}`}
              ...
              <div className="rounded-[2rem] p-6 md:p-10 border-2 backdrop-blur-xl flex flex-col items-center gap-5"
                style={{
                  ...
                  borderColor: `${fruit.color}66`,
                  boxShadow: `0 20px 60px rgba(0,0,0,0.5), 0 0 40px ${fruit.color}33`,
                }}>
                
                <div className="text-4xl md:text-5xl">🎤</div>
                
                <h3 className="text-xl md:text-2xl font-black text-white text-center">انطق اسم الصنف</h3>

                <MarketCard fruit={fruit} size={isMobile ? 160 : 200} showSentence={false} />

                <button onClick={() => speakSpanishSentence(fruit.exampleEs)}
                  ...
                  <Volume2 size={16} /> 
                  <span dir="ltr">"{fruit.exampleEs}"</span>
                </button>
                ...
                <motion.button 
                  ...
                  style={{
                    ...
                    boxShadow: `0 0 50px ${fruit.color}66`,
                  }}
                >
                  <Mic size={isMobile ? 40 : 50} className="text-white" />
                </motion.button>

                <button onClick={() => speakSpanishWord(fruit.word)}
          ...
          {phase === 'group-success' && (
            <motion.div key="success" initial={{ scale: 0.5, opacity: 0 }} animate={{ scale: 1, opacity: 1 }}
              ...
              <h2 className="text-3xl md:text-5xl font-black text-white">¡Delicioso!</h2>
              <p className="text-lg md:text-xl text-red-400 font-bold">أنهيت {group.titleEs}</p>
              <button 
                onClick={() => {
                  if (groupIdx < SPANISH_FRUIT_GROUPS.length - 1) {
                    setGroupIdx(g => g + 1); setFruitIdx(0); setPhase('listen');
                    setCorrectInGroup(0);
                  } else {
                    setPhase('all-done');
                  }
          ...
          {phase === 'all-done' && (
            <motion.div key="done" initial={{ opacity: 0 }} animate={{ opacity: 1 }}
              ...
              <div className="flex flex-wrap justify-center gap-3 max-w-lg mx-auto mb-4">
                {SPANISH_FRUITS.map((f, i) => (
                  <motion.div key={f.word}
                    ...
                    style={{
                      ...
                      boxShadow: `0 4px 15px ${f.color}88`,
                      fontSize: isMobile ? '1.5rem' : '1.8rem',
                    }}
                  >
                    {f.emoji}
              ...
              <h1 className="text-3xl md:text-5xl font-black text-white">¡Mercado completo!</h1>
              <p className="text-lg md:text-xl text-white/60 max-w-md mx-auto">تعلمت 15 صنف من الفواكه والخضروات!</p>
      ...
      <ToroBull mood={toroMood} message={toroMessage} idleGlowColor={fruit?.color || '#DC2626'} />
      <ConfettiBurst trigger={confettiTrigger} x={confettiPos.x} y={confettiPos.y} 
        colors={[fruit?.color || '#DC2626', '#FFD700', '#ffffff']} />
    </div>
  );
}

export default function SpanishFruitsLessonPage() {
  return (
    <Suspense fallback={<div className="min-h-screen bg-[#07090D] flex items-center justify-center text-white">Loading...</div>}>
      <SpanishFruitsLessonInner />
    </Suspense>
  );
}