...
import { 
  Check, Volume2, Trophy, Home, Flame, Gem, Mic, SkipForward, Sparkles, Palette
} from 'lucide-react';

import { 
  SPANISH_COLORS, 
  SPANISH_COLOR_GROUPS, 
  generateSpanishColorChoices,
  compareSpanishColorWords,
  shuffleSpanishColorLetters,
  type SpanishColor
} from '@/data/spanish/colors';
...
const LESSON_ID = 'es-catedrales-colors';
...
function ColorCard({ color, size = 280, showSentence = true }: { 
  color: SpanishColor; size?: number; showSentence?: boolean;
}) {
  return (
    <motion.div
      animate={{ y: [0, -8, 0] }}
    ...
    >
      <div 
        ...
        style={{ background: `radial-gradient(circle, ${color.color}aa, transparent 70%)` }} 
      />
      
      <div 
        className="relative w-full rounded-[2rem] overflow-hidden border-4 flex flex-col items-center"
        style={{
          background: `linear-gradient(180deg, ${color.gradient[0]}44, ${color.gradient[1]}77, #1a0a2e)`,
          borderColor: color.color,
          boxShadow: `0 20px 60px ${color.color}66, inset 0 2px 0 rgba(255,255,255,0.3)`,
        }}
      >
        <div className="absolute top-3 left-1/2 -translate-x-1/2 z-20 flex flex-col items-center gap-1">
          ...
          >
            <Palette size={12} className="text-white" /> الألوان
          </div>
        </div>
        ...
        <div className="flex-1 flex items-center justify-center w-full pt-16 pb-4">
          <motion.div
            ...
            className="relative rounded-full border-4 border-white/40 flex items-center justify-center"
            style={{ 
              width: size * 0.5,
              height: size * 0.5,
              background: color.colorHex,
              boxShadow: `0 10px 30px ${color.color}cc, inset 0 -10px 30px rgba(0,0,0,0.3)`,
            }}
          >
            <span style={{ fontSize: size * 0.2 }}>{color.emoji}</span>
          </motion.div>
        </div>
        ...
        <div className="px-4 pb-3 flex flex-col items-center gap-1.5 z-10">
          <div 
            ...
            style={{
              ...
              boxShadow: `0 4px 15px ${color.color}88`,
              direction: 'ltr',
            }}
          >
            {color.word}
          </div>
          <div className="text-base font-bold text-white/90">{color.wordAr}</div>
        </div>
...
function SpanishColorsLessonInner() {
  ...
  const [colorChoices, setColorChoices] = useState<SpanishColor[]>([]);
  ...
  const group = SPANISH_COLOR_GROUPS[groupIdx];
  const color = group?.colors[colorIdx];
  ...
  useEffect(() => {
    const load = async () => {
      ...
      if (progress && !progress.completed) {
        ...
        setColorIdx(progress.current_letter || 0);
      }
      setIsLoading(false);
    };
    load();
  ...
  useEffect(() => {
    if (color) {
      setColorChoices(generateSpanishColorChoices(color.word, 3));
      setShuffledLetters(shuffleSpanishColorLetters(color.word));
      setUsedLetterIndices([]);
      setInput('');
      setTimeout(() => speakSpanishWord(color.word), 500);
    }
  }, [colorIdx, groupIdx, phase]);
  ...
  const triggerCorrect = (cx: number, cy: number) => {
    ...
    setTimeout(() => {
      if (phase === 'listen') { setPhase('write'); setInput(''); setStatus('idle'); }
        ...
        if (colorIdx < group.colors.length - 1) {
          setColorIdx(n => n + 1); setPhase('listen');
        } else {
  ...
  const handleColorChoice = (choice: SpanishColor, e: React.MouseEvent) => {
    if (status === 'correct') return;
    if (choice.word === color.word) {
      setStatus('correct');
      speakSpanishWord(color.word);
  ...
  const handleLetterClick = (letter: string, idx: number) => {
    ...
    if (newInput.length === color.word.length) {
      setTimeout(() => {
        if (compareSpanishColorWords(newInput, color.word)) {
          setStatus('correct');
          speakSpanishWord(color.word);
  ...
  const handleWriteCheck = (e?: any) => {
    if (compareSpanishColorWords(input, color.word)) {
      setStatus('correct');
      speakSpanishWord(color.word);
    ...
    <div className="relative min-h-screen overflow-hidden" 
      ...
      <div className="fixed inset-0 pointer-events-none" style={{ zIndex: 0 }}>
        ...
        <motion.div
          ...
          style={{ background: `radial-gradient(ellipse 80% 50% at 50% 0%, ${color?.color}33, transparent 70%)` }}
        ...
        {isMobile ? (
          <>
            ...
            <div className="flex justify-center mt-1.5">
              <div className="flex items-center gap-1 px-2 py-1 rounded-lg bg-black/50 backdrop-blur-md border border-white/15">
                <Stepper currentStep={colorIdx} totalSteps={group?.colors.length || 4} isMobile color={color?.color || '#DC2626'} />
              </div>
            </div>
          </>
        ) : (
          <div className="max-w-[1600px] mx-auto flex items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-1 px-4 py-2 rounded-2xl bg-black/50 backdrop-blur-xl border-2 border-white/15">
              <Stepper currentStep={colorIdx} totalSteps={group?.colors.length || 4} isMobile={false} color={color?.color || '#DC2626'} />
            </div>
      ...
      <main className="relative z-10 min-h-screen flex items-center justify-center px-3 pt-24 md:pt-28 pb-28">
        <AnimatePresence mode="wait">
          ...
          {phase === 'listen' && color && (
            <motion.div key={`listen-${groupIdx}-${colorIdx}`}
              ...
              <div className={`flex ${isMobile ? 'flex-col' : 'flex-row'} items-center justify-center gap-6 md:gap-10`}>
                ...
                <ColorCard color={color} size={isMobile ? 200 : 280} showSentence={false} />
                ...
                <div className={`flex flex-col items-center gap-4 ${isMobile ? 'w-full max-w-sm' : 'flex-1 max-w-md'}`}>
                  ...
                  <div className="px-5 py-2 rounded-2xl backdrop-blur-md border-2"
                    style={{ 
                      ...
                      borderColor: `${color.color}66`,
                    }}>
                    <span className="font-black text-white text-sm md:text-base">استمع واختر اللون</span>
                  </div>

                  <SoundButton 
                    onClick={() => speakSpanishWord(color.word)} 
                    color={color.color} 
                    size={isMobile ? 55 : 70}
                  />
                  ...
                  <div className="flex flex-col gap-2 md:gap-3 w-full mt-2">
                    {colorChoices.map((choice, idx) => {
                      ...
                      const isCorrect = status === 'correct' && choice.word === color.word;
                      return (
                        <motion.button
                          key={`${color.word}-${choice.word}-${idx}`}
                          ...
                          onClick={(e) => handleColorChoice(choice, e)}
                        ...
                        >
                          <div 
                            className="rounded-full flex-shrink-0 border-2 border-white/40"
                            style={{
                              width: isMobile ? 44 : 56,
                              height: isMobile ? 44 : 56,
                              background: choice.colorHex,
                              boxShadow: `0 4px 15px ${choice.color}88, inset 0 -5px 15px rgba(0,0,0,0.3)`,
                            }}
                          />
          ...
          {phase === 'write' && color && (
            <motion.div key={`write-${groupIdx}-${colorIdx}`}
              ...
              <div className={`flex ${isMobile ? 'flex-col' : 'flex-row'} items-center justify-center gap-6 md:gap-10`}>
                ...
                <ColorCard color={color} size={isMobile ? 200 : 280} showSentence />
                ...
                <div className={`flex flex-col items-center gap-4 ${isMobile ? 'w-full max-w-sm' : 'flex-1 max-w-md'}`}>
                  ...
                  <div className="px-5 py-2 rounded-2xl backdrop-blur-md border-2"
                    ...
                    <span className="font-black text-white text-sm md:text-base">
                      {isMobile ? 'رتّب الحروف لتكوين الكلمة' : 'اكتب اسم اللون'}
                    </span>
                  </div>
                    ...
                    <div className="w-full space-y-4">
                      ...
                      >
                        <div className="flex gap-1 flex-wrap justify-center" dir="ltr">
                          {color.word.split('').map((targetLetter, i) => {
                              ...
                              <motion.div
                                ...
                                style={{
                                  ...
                                  borderColor: isFilled 
                                    ...
                                    : `${color.color}33`,
                                  color: isFilled ? 'white' : `${color.color}55`,
                                  boxShadow: isFilled ? `0 4px 12px ${color.color}88` : 'none',
                                }}
                              >
                            ...
                            <motion.button
                              ...
                              style={{
                                background: isUsed 
                                  ? 'rgba(255,255,255,0.05)' 
                                  : `linear-gradient(135deg, ${color.gradient[0]}, ${color.gradient[1]})`,
                                borderColor: isUsed ? 'rgba(255,255,255,0.1)' : color.color,
                                color: isUsed ? 'rgba(255,255,255,0.2)' : 'white',
                                boxShadow: isUsed ? 'none' : `0 4px 12px ${color.color}66`,
                    ...
                    <>
                      <div className="w-full space-y-3">
                        <GhostInput 
                          ...
                          ghostText={color.word} color={color.color} status={status}
                          fontSize={'1.8rem'}
                        />
                        
                        {getRequiredSpanishSpecialChars(color.word).length > 0 && (
                          <SpanishCharsKeyboard 
                            chars={getRequiredSpanishSpecialChars(color.word)}
                            onChar={(c) => setInput(prev => prev + c)} color={color.color} 
                          />
                        )}
                      </div>
          ...
          {phase === 'speak' && color && (
            <motion.div key={`speak-${groupIdx}-${colorIdx}`}
              ...
              <div className="rounded-[2rem] p-6 md:p-10 border-2 backdrop-blur-xl flex flex-col items-center gap-5"
                style={{
                  ...
                  borderColor: `${color.color}66`,
                  boxShadow: `0 20px 60px rgba(0,0,0,0.5), 0 0 40px ${color.color}33`,
                }}>
                
                <div className="text-4xl md:text-5xl">🎤</div>
                
                <h3 className="text-xl md:text-2xl font-black text-white text-center">انطق اسم اللون</h3>

                <ColorCard color={color} size={isMobile ? 160 : 200} showSentence={false} />

                <button onClick={() => speakSpanishSentence(color.exampleEs)}
                  className="px-5 py-2.5 rounded-xl backdrop-blur-md border-2 flex items-center gap-2 text-white text-sm font-bold"
                  style={{
                    background: 'rgba(255,255,255,0.05)',
                    borderColor: `${color.color}44`,
                  }}>
                  <Volume2 size={16} /> 
                  <span dir="ltr">"{color.exampleEs}"</span>
                </button>
                ...
                <motion.button 
                  ...
                  style={{
                    ...
                    boxShadow: `0 0 50px ${color.color}66`,
                  }}
                >
                  <Mic size={isMobile ? 40 : 50} className="text-white" />
                </motion.button>

                <button onClick={() => speakSpanishWord(color.word)}
          ...
          {phase === 'group-success' && (
            <motion.div key="success" initial={{ scale: 0.5, opacity: 0 }} animate={{ scale: 1, opacity: 1 }}
              ...
              <h2 className="text-3xl md:text-5xl font-black text-white">¡Qué colores!</h2>
              <p className="text-lg md:text-xl text-red-400 font-bold">أنهيت {group.titleEs}</p>
              <button 
                onClick={() => {
                  if (groupIdx < SPANISH_COLOR_GROUPS.length - 1) {
                    setGroupIdx(g => g + 1); setColorIdx(0); setPhase('listen');
                    setCorrectInGroup(0);
                  } else {
                    setPhase('all-done');
                  }
          ...
          {phase === 'all-done' && (
            <motion.div key="done" initial={{ opacity: 0 }} animate={{ opacity: 1 }}
              ...
              <div className="flex flex-wrap justify-center gap-3 max-w-lg mx-auto mb-4">
                {SPANISH_COLORS.map((c, i) => (
                  <motion.div key={c.word}
                    initial={{ scale: 0 }}
                    animate={{ scale: 1 }}
                    transition={{ delay: i * 0.05, type: 'spring' }}
                    className="w-12 h-12 md:w-14 md:h-14 rounded-full border-2 border-white/40"
                    style={{
                      background: c.colorHex,
                      boxShadow: `0 4px 15px ${c.color}88`,
                    }}
                  />
                ))}
              </div>
              ...
              <h1 className="text-3xl md:text-5xl font-black text-white">¡Colores completos!</h1>
              <p className="text-lg md:text-xl text-white/60 max-w-md mx-auto">تعلمت 8 ألوان بالإسباني!</p>
      ...
      <ToroBull mood={toroMood} message={toroMessage} idleGlowColor={color?.color || '#DC2626'} />
      <ConfettiBurst trigger={confettiTrigger} x={confettiPos.x} y={confettiPos.y} 
        colors={[color?.color || '#DC2626', '#FFD700', '#ffffff']} />
    </div>
  );
}

export default function SpanishColorsLessonPage() {
  return (
    <Suspense fallback={<div className="min-h-screen bg-[#07090D] flex items-center justify-center text-white">Loading...</div>}>
      <SpanishColorsLessonInner />
    </Suspense>
  );
}