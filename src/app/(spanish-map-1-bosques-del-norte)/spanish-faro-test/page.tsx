...
// 🏮 Lighthouse Component - منارة متحركة
function AnimatedLighthouse({ size = 200 }: { size?: number }) {
  return (
    <motion.div
      className="relative"
    ...
    >
      {/* الضوء الدوار */}
      <motion.div
        className="absolute top-[15%] left-1/2 -translate-x-1/2 rounded-full blur-2xl"
        style={{
          width: size * 1.5,
          height: size * 0.4,
          background: 'radial-gradient(ellipse, rgba(255,215,0,0.9), transparent 70%)',
        }}
        animate={{ 
          rotate: [0, 360],
          opacity: [0.6, 1, 0.6],
      ...
      <motion.div
        ...
        className="absolute bottom-0 left-1/2 -translate-x-1/2 text-3xl opacity-60"
      >
        🌊
      </motion.div>
    </motion.div>
    ...
    <div className="w-full max-w-md mx-auto">
      ...
      <div className="w-full h-2.5 md:h-3 bg-black/40 rounded-full overflow-hidden border border-white/20">
        <motion.div 
          ...
          style={{ background: 'linear-gradient(to right, #FCD34D, #FBBF24, #F59E0B)' }}
          initial={{ width: 0 }}
...
function SpanishFaroTestInner() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      // اختبار: ما نحفظش position، نبدأ من الأول كل مرة
      setIsLoading(false);
    };
    load();
  }, []);

  // نطق الكلمة لما السؤال يتغير في جولة Recognize أو Speak
  useEffect(() => {
  ...
  const handleCorrect = (cx: number, cy: number) => {
    ...
    setPoints(p => p + 20); // 20 نقطة لكل إجابة صحيحة
    setStreak(s => s + 1);
  ...
  // 🎤 جولة Speak - يعتبر صحيح لو دوس زر النطق
    ...
    <div className="relative min-h-screen overflow-hidden" 
      ...
      <div className="fixed inset-0 pointer-events-none" style={{ zIndex: 0 }}>
        ...
        <div className="absolute inset-0" style={{
          background: isMobile 
            ? 'linear-gradient(180deg, rgba(10,5,30,0.85) 0%, rgba(10,5,30,0.92) 100%)'
            : 'radial-gradient(ellipse at 20% 20%, rgba(70,20,20,0.9) 0%, rgba(40,10,15,0.95) 50%, rgba(20,5,10,0.98) 100%)',
        }} />
        {/* تأثير منارة في الخلفية */}
        <motion.div
          className="absolute inset-0 opacity-20"
          style={{ background: `radial-gradient(ellipse 100% 60% at 50% 30%, rgba(255,215,0,0.4), transparent 70%)` }}
          animate={{ opacity: [0.15, 0.3, 0.15] }}
          transition={{ duration: 3, repeat: Infinity, ease: 'easeInOut' }}
        />
      </div>

      {/* Top HUD - يظهر فقط في مرحلة الأسئلة */}
      {phase === 'question' && (
        ...
        <AnimatePresence mode="wait">
          ...
          {phase === 'intro' && (
            <motion.div key="intro"
              ...
              <AnimatedLighthouse size={isMobile ? 150 : 200} />
              ...
              <p className="text-sm md:text-base text-white/70 max-w-md mx-auto">
                💡 الاختبار بيغطي كل اللي اتعلمته في الخريطة الأولى. تقدر تحصل على 3 نجوم لو جاوبت صح على معظم الأسئلة!
              </p>
          ...
          {/* ═══════ Phase: Round Intro (مقدمة كل جولة) ═══════ */}
                ...
                return (
                  <>
                    ...
                    <motion.div
                      ...
                      transition={{ type: 'spring', duration: 1 }}
                    >
                      <AnimatedLighthouse size={isMobile ? 130 : 180} />
                    </motion.div>
                    ...
                    {/* رسالة خاصة لو حصل على 3 نجوم */}
                    {isExcellent && (
                      <motion.div
                        initial={{ y: 20, opacity: 0 }}
                        animate={{ y: 0, opacity: 1 }}
                        transition={{ delay: 1 }}
                        className="bg-gradient-to-r from-yellow-500/30 to-red-500/30 backdrop-blur-md rounded-2xl p-4 border-2 border-yellow-400/50"
                      >
                        <p className="text-base md:text-lg font-black text-yellow-300 mb-1">
                          🏆 إنجاز رائع! 🏆
                        </p>
                        <p className="text-xs md:text-sm text-white">
                          أنهيت <span className="font-black text-yellow-300">الخريطة الأولى</span> بنجاح! جاهز للخريطة الثانية: قلاع قشتالة 🏰
                        </p>
                      </motion.div>
                    )}
