...
import { 
  SPANISH_TIME_ITEMS, 
  SPANISH_TIME_GROUPS, 
  generateSpanishTimeChoices,
  generateTimeSentenceWordPool,
  checkTimeSentenceOrder,
  type SpanishTimeItem
} from '@/data/spanish-3/time';
...
const LESSON_ID = 'es-portvell-time';
...
function getCategoryStyle(category: 'Momento' | 'Hora' | 'Dia') {
  switch (category) {
    case 'Momento':
      return { bg: 'rgba(245,158,11,0.7)', border: '#F59E0B', icon: '🌅', label: 'أوقات' };
    case 'Hora':
      return { bg: 'rgba(14,165,233,0.7)', border: '#0EA5E9', icon: '🕐', label: 'ساعات' };
    case 'Dia':
      return { bg: 'rgba(220,38,38,0.7)', border: '#DC2626', icon: '📅', label: 'أيام' };
  }
}

// ═══════════════════════════════════════
// 🕐 Interactive Clock Component - ساعة تفاعلية
// ═══════════════════════════════════════
    ...
    >
      ...
      <div 
        ...
        style={{
          ...
          borderColor: '#78350F',
          boxShadow: `0 15px 35px ${color}88, inset 0 4px 8px rgba(255,255,255,0.5), inset 0 -4px 8px rgba(0,0,0,0.2)`,
        }}
      />
      ...
      <div 
        ...
        style={{
          ...
          boxShadow: 'inset 0 2px 6px rgba(120,53,15,0.3)',
        }}
      />
      ...
      >
        {/* الشرطات الصغيرة (كل 5 دقايق) */}
        {Array.from({ length: 12 }).map((_, i) => {
          const angle = (i * 30) - 90;
          const rad = (angle * Math.PI) / 180;
          const isMain = i % 3 === 0;
          const outerR = 44;
          const innerR = isMain ? 38 : 41;
          
          const x1 = 50 + Math.cos(rad) * outerR;
          const y1 = 50 + Math.sin(rad) * outerR;
          const x2 = 50 + Math.cos(rad) * innerR;
          const y2 = 50 + Math.sin(rad) * innerR;

          return (
            <line
              key={i}
              x1={x1} y1={y1} x2={x2} y2={y2}
              stroke={isMain ? '#78350F' : '#A16207'}
...
function TimeCard({ 
  ...
  const hasClock = item.clockHour !== undefined;
  ...
  return (
    ...
    >
      ...
      <div 
        ...
        style={{
          background: `linear-gradient(180deg, ${item.gradient[0]}44, ${item.gradient[1]}77, #0a1628)`,
          borderColor: item.color,
          boxShadow: `0 20px 60px ${item.color}66, inset 0 2px 0 rgba(255,255,255,0.3)`,
        }}
      >
        {/* ديكور: برج الساعة والبحر */}
        <div className="absolute top-2 right-2 text-2xl opacity-20 select-none pointer-events-none">🏛️</div>
        <div className="absolute bottom-16 left-2 text-xl opacity-15 select-none pointer-events-none">⚓</div>
        ...
        <div className="absolute top-3 left-1/2 -translate-x-1/2 z-20 flex flex-col items-center gap-1">
          ...
          >
            <Clock size={12} className="text-white" /> الوقت
          </div>
        ...
        {mode === 'word' ? (
          ...
          <div className="px-3 pb-3 flex flex-col items-center gap-2 z-10 w-full">
            ...
            >
              <BookOpen size={10} className="text-sky-300" />
              <span className="text-[10px] font-black text-sky-200" dir="ltr">
...
function SpanishTimeLessonInner() {
  ...
  const [itemChoices, setItemChoices] = useState<SpanishTimeItem[]>([]);
  ...
  const group = SPANISH_TIME_GROUPS[groupIdx];
  ...
  useEffect(() => {
    if (item && phase === 'listen') {
      setItemChoices(generateSpanishTimeChoices(item.word, 3));
  ...
  useEffect(() => {
    if (item && phase === 'build') {
      setAvailableWords(generateTimeSentenceWordPool(item));
      setSelectedWords([]);
      setStatus('idle');
  ...
  const handleItemChoice = (choice: SpanishTimeItem, e: React.MouseEvent) => {
  ...
  const handleSelectWord = (word: string, idx: number) => {
    ...
    if (newSelected.length === item.sentenceWords.length) {
      const isCorrect = checkTimeSentenceOrder(newSelected, item.sentenceWords);
      if (isCorrect) {
        setStatus('correct');
        setTimeout(() => {
    ...
    <div className="relative min-h-screen overflow-hidden" 
      ...
      <div className="fixed inset-0 pointer-events-none" style={{ zIndex: 0 }}>
        ...
        <div className="absolute inset-0" style={{
          background: isMobile 
            ? 'linear-gradient(180deg, rgba(10,20,40,0.75) 0%, rgba(5,15,35,0.9) 100%)'
            : 'radial-gradient(ellipse at 20% 20%, rgba(20,50,90,0.85) 0%, rgba(10,30,60,0.92) 50%, rgba(5,20,45,0.95) 100%)',
        }} />
        <motion.div
            ...
            {phase !== 'group-success' && phase !== 'all-done' && (
              <div className="flex justify-center mt-1.5">
                <div className="flex items-center gap-1 px-2 py-1 rounded-lg bg-black/50 backdrop-blur-md border border-white/15">
                  <Stepper currentStep={itemIdx} totalSteps={group?.items.length || 5} isMobile color={item?.color || '#0EA5E9'} />
                </div>
              </div>
            )}
          </>
          ...
          <div className="max-w-[1600px] mx-auto flex items-center justify-between gap-4">
            ...
            {phase !== 'group-success' && phase !== 'all-done' && (
              <div className="flex items-center gap-1 px-4 py-2 rounded-2xl bg-black/50 backdrop-blur-xl border-2 border-white/15">
                <Stepper currentStep={itemIdx} totalSteps={group?.items.length || 5} isMobile={false} color={item?.color || '#0EA5E9'} />
              </div>
            )}
                ...
                <div className="flex items-center gap-2">
                  <div className="w-20 h-2 bg-white/15 rounded-full overflow-hidden border border-white/20">
                    <motion.div className="h-full rounded-full" 
                      style={{ background: 'linear-gradient(to right, #0EA5E9, #FFD700)' }}
        ...
        <AnimatePresence mode="wait">
          ...
          {phase === 'listen' && item && (
            <motion.div key={`listen-${groupIdx}-${itemIdx}`}
              ...
              <div className={`flex ${isMobile ? 'flex-col' : 'flex-row'} items-center justify-center gap-6 md:gap-10`}>
                ...
                <TimeCard item={item} size={isMobile ? 220 : 300} mode="word" />
                ...
                <div className={`flex flex-col items-center gap-4 ${isMobile ? 'w-full max-w-sm' : 'flex-1 max-w-md'}`}>
                  ...
                  <div className="px-5 py-2 rounded-2xl backdrop-blur-md border-2"
                    ...
                    <span className="font-black text-white text-sm md:text-base">🎧 استمع واختر الوقت الصحيح</span>
                  </div>
          ...
          {phase === 'build' && item && (
            <motion.div key={`build-${groupIdx}-${itemIdx}`}
              ...
              <div className={`flex ${isMobile ? 'flex-col' : 'flex-row'} items-center justify-center gap-6 md:gap-10`}>
                ...
                <TimeCard item={item} size={isMobile ? 240 : 300} mode="sentence" />
                ...
                <div className={`flex flex-col items-center gap-3 ${isMobile ? 'w-full max-w-sm' : 'flex-1 max-w-md'}`}>
                  ...
                  <div className="px-3 py-1.5 rounded-xl border flex items-center gap-2 text-center"
                    ...
                    <BookOpen size={12} className="text-sky-300 flex-shrink-0" />
                    <span className="text-[10px] font-bold text-sky-100">{item.grammarHint.rule}</span>
                  </div>
                  ...
                  {status === 'correct' && (
                    <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }}
                      ...
                      <span className="font-black text-white text-sm">¡Excelente!</span>
                    </motion.div>
                  )}
                </div>
              </div>
              ...
              <div className={`flex ${isMobile ? 'flex-col' : 'flex-row'} items-center justify-center gap-6 md:gap-10`}>
                ...
                <div className={`flex flex-col items-center gap-4 ${isMobile ? 'w-full max-w-sm' : 'flex-1 max-w-md'}`}>
                  ...
                  <div className="w-full p-3 rounded-2xl border-2"
                    ...
                    <div className="text-[10px] text-sky-300 font-black mb-2 text-center">
          ...
          {phase === 'group-success' && (
            <motion.div key="success" initial={{ scale: 0.5, opacity: 0 }} animate={{ scale: 1, opacity: 1 }}
              ...
              <h2 className="text-3xl md:text-5xl font-black text-white">¡Perfecto tiempo!</h2>
              <p className="text-lg md:text-xl text-sky-400 font-bold">أنهيت {group.titleEs}</p>
              ...
              <button 
                onClick={() => {
                  if (groupIdx < SPANISH_TIME_GROUPS.length - 1) {
                    setGroupIdx(g => g + 1); 
                    setItemIdx(0); 
                    setPhase('listen');
                    setCorrectInGroup(0);
                ...
                style={{ background: 'linear-gradient(135deg, #0EA5E9, #FFD700)', boxShadow: '0 10px 30px rgba(14,165,233,0.4)' }}
              >
                المجموعة التالية 🚀
              </button>
            </motion.div>
          ...
          {phase === 'all-done' && (
            <motion.div key="done" initial={{ opacity: 0 }} animate={{ opacity: 1 }}
              ...
              <h1 className="text-3xl md:text-5xl font-black text-white">¡Torre del Reloj completa!</h1>
              <p className="text-lg md:text-xl text-white/60 max-w-md mx-auto">تعلمت 15 كلمة عن الوقت والأيام + 15 جملة كاملة!</p>
              <p className="text-sm text-sky-300/80 max-w-md mx-auto">
                دلوقتي تقدر تسأل وتقول الساعة بالأسباني! 🕐⏰
              </p>
              ...
              <button 
                ...
                style={{ 
                  background: 'linear-gradient(135deg, #0EA5E9, #FFD700)', 
      ...
      <div className="fixed bottom-0 left-0 right-0 z-30 px-2 md:px-4 pb-1.5 pointer-events-none"
        ...
        <div className={`mx-auto pointer-events-auto ${isMobile ? 'max-w-md' : 'max-w-[1500px]'}`}>
          <div className="relative rounded-xl px-4 md:px-6 py-1.5 md:py-2"
            style={{
              background: 'linear-gradient(135deg, rgba(20,40,80,0.85), rgba(10,25,55,0.9))',
              backdropFilter: 'blur(30px)',
              border: '1.5px solid rgba(255,255,255,0.2)',
              boxShadow: `0 10px 30px rgba(0,0,0,0.5), 0 0 25px rgba(14,165,233,0.2)`,
            }}>
      ...
      <ToroBull mood={toroMood} message={toroMessage} idleGlowColor={item?.color || '#0EA5E9'} />
      <ConfettiBurst trigger={confettiTrigger} x={confettiPos.x} y={confettiPos.y} 
        colors={[item?.color || '#0EA5E9', '#FFD700', '#ffffff']} />
    </div>
  );
}

export default function SpanishTimeLessonPage() {
  return (
    <Suspense fallback={<div className="min-h-screen bg-[#07090D] flex items-center justify-center text-white">Loading...</div>}>
      <SpanishTimeLessonInner />
    </Suspense>
  );
}