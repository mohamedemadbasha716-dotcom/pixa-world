...
import { 
  SPANISH_TRANSPORT_ITEMS, 
  SPANISH_TRANSPORT_GROUPS, 
  generateSpanishTransportChoices,
  generateTransportSentenceWordPool,
  checkTransportSentenceOrder,
  type SpanishTransportItem
} from '@/data/spanish-3/transport';
...
const LESSON_ID = 'es-metrovalencia-transport';
...
function getCategoryStyle(category: 'Vehiculo' | 'Calle' | 'Viajar') {
  switch (category) {
    case 'Vehiculo':
      return { bg: 'rgba(124,58,237,0.7)', border: '#7C3AED', icon: '🚗', label: 'مواصلات' };
    case 'Calle':
      return { bg: 'rgba(234,179,8,0.7)', border: '#EAB308', icon: '🚦', label: 'شارع' };
    case 'Viajar':
      return { bg: 'rgba(59,130,246,0.7)', border: '#3B82F6', icon: '✈️', label: 'سفر' };
  }
}

// ═══════════════════════════════════════
// 🚇 Moving Train - قطر متحرك
// ═══════════════════════════════════════
function MovingTrain({ size = 30 }: { size?: number }) {
  return (
    <motion.div
      animate={{ 
        x: [-15, 15, -15],
...
// ═══════════════════════════════════════
function TrafficLight({ size = 26 }: { size?: number }) {
  ...
  return (
    ...
    >
      <div style={{ fontSize: size * 0.5, lineHeight: 1 }}>
        {lights[lightIdx]}
      </div>
...
// ═══════════════════════════════════════
// 🚇 Transport Card - بطاقة المواصلات
// ═══════════════════════════════════════
    ...
    >
      ...
      >
        ...
        <div className="absolute top-3 left-1/2 -translate-x-1/2 z-20 flex flex-col items-center gap-1">
          ...
          >
            <Train size={12} className="text-white" /> المواصلات
          </div>
...
function SpanishTransportLessonInner() {
  ...
  const [itemChoices, setItemChoices] = useState<SpanishTransportItem[]>([]);
  ...
  const group = SPANISH_TRANSPORT_GROUPS[groupIdx];
  ...
  useEffect(() => {
    if (item && phase === 'listen') {
      setItemChoices(generateSpanishTransportChoices(item.word, 3));
  ...
  useEffect(() => {
    if (item && phase === 'build') {
      setAvailableWords(generateTransportSentenceWordPool(item));
      setSelectedWords([]);
      setStatus('idle');
  ...
  const handleItemChoice = (choice: SpanishTransportItem, e: React.MouseEvent) => {
  ...
  const handleSelectWord = (word: string, idx: number) => {
    ...
    if (newSelected.length === item.sentenceWords.length) {
      const isCorrect = checkTransportSentenceOrder(newSelected, item.sentenceWords);
      if (isCorrect) {
        setStatus('correct');
        setTimeout(() => {
    ...
    <div className="relative min-h-screen overflow-hidden" 
      ...
      <div className="fixed inset-0 pointer-events-none" style={{ zIndex: 0 }}>
        ...
        <div className="absolute inset-0" style={{
          background: isMobile 
            ? 'linear-gradient(180deg, rgba(30,15,50,0.8) 0%, rgba(20,10,40,0.92) 100%)'
            : 'radial-gradient(ellipse at 20% 20%, rgba(60,25,90,0.9) 0%, rgba(40,15,70,0.94) 50%, rgba(20,10,40,0.98) 100%)',
        }} />
        <motion.div
        ...
        <AnimatePresence mode="wait">
          ...
          {phase === 'listen' && item && (
            <motion.div key={`listen-${groupIdx}-${itemIdx}`}
              ...
              <div className={`flex ${isMobile ? 'flex-col' : 'flex-row'} items-center justify-center gap-6 md:gap-10`}>
                ...
                <TransportCard item={item} size={isMobile ? 200 : 280} mode="word" />
          ...
          {phase === 'build' && item && (
            <motion.div key={`build-${groupIdx}-${itemIdx}`}
              ...
              <div className={`flex ${isMobile ? 'flex-col' : 'flex-row'} items-center justify-center gap-6 md:gap-10`}>
                ...
                <TransportCard item={item} size={isMobile ? 220 : 280} mode="sentence" />
          ...
          {phase === 'group-success' && (
            <motion.div key="success" initial={{ scale: 0.5, opacity: 0 }} animate={{ scale: 1, opacity: 1 }}
              ...
              <h2 className="text-3xl md:text-5xl font-black text-white">¡Buen viaje!</h2>
              ...
              <button 
                onClick={() => {
                  if (groupIdx < SPANISH_TRANSPORT_GROUPS.length - 1) {
                    setGroupIdx(g => g + 1); 
                    setItemIdx(0); 
                    setPhase('listen');
                    setCorrectInGroup(0);
          ...
          {phase === 'all-done' && (
            <motion.div key="done" initial={{ opacity: 0 }} animate={{ opacity: 1 }}
              ...
              <div className="flex flex-wrap justify-center gap-2 max-w-2xl mx-auto mb-4">
                {SPANISH_TRANSPORT_ITEMS.map((s, i) => (
              ...
              <h1 className="text-3xl md:text-5xl font-black text-white">¡Metro Valencia completo!</h1>
              <p className="text-lg md:text-xl text-white/60 max-w-md mx-auto">تعلمت 15 كلمة عن المواصلات والسفر + 15 جملة كاملة!</p>
              <p className="text-sm text-purple-300/80 max-w-md mx-auto">
                دلوقتي تقدر تسافر بالمترو والقطر بالأسباني! 🚇🚂
              </p>
      ...
      <div className="fixed bottom-0 left-0 right-0 z-30 px-2 md:px-4 pb-1.5 pointer-events-none"
        ...
        <div className={`mx-auto pointer-events-auto ${isMobile ? 'max-w-md' : 'max-w-[1500px]'}`}>
          <div className="relative rounded-xl px-4 md:px-6 py-1.5 md:py-2"
            style={{
              background: 'linear-gradient(135deg, rgba(60,25,90,0.85), rgba(40,15,70,0.9))',
