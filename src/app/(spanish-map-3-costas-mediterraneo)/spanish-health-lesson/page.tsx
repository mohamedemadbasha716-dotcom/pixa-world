...
import { 
  ...
  Heart, X, BookOpen, Plus
} from 'lucide-react';

import { 
  SPANISH_HEALTH_ITEMS, 
  SPANISH_HEALTH_GROUPS, 
  generateSpanishHealthChoices,
  generateHealthSentenceWordPool,
  checkHealthSentenceOrder,
  type SpanishHealthItem
} from '@/data/spanish-3/health';
...
const LESSON_ID = 'es-santpau-health';
...
function getCategoryStyle(category: 'Sintoma' | 'Hospital' | 'Salud') {
  switch (category) {
    case 'Sintoma':
      return { bg: 'rgba(220,38,38,0.7)', border: '#DC2626', icon: '🤒', label: 'أعراض' };
    case 'Hospital':
      return { bg: 'rgba(59,130,246,0.7)', border: '#3B82F6', icon: '🏥', label: 'مستشفى' };
    case 'Salud':
      return { bg: 'rgba(34,197,94,0.7)', border: '#22C55E', icon: '💪', label: 'صحة' };
  }
}

// ═══════════════════════════════════════
// ⚕️ Medical Cross - صليب طبي متحرك
// ═══════════════════════════════════════
function MedicalCross({ color = '#22C55E', size = 40 }: { color?: string; size?: number }) {
  return (
    <motion.div
      animate={{ 
        scale: [1, 1.1, 1],
    ...
    >
      <div className="relative w-full h-full">
        ...
        />
        {/* الصليب */}
        <svg viewBox="0 0 24 24" className="relative w-full h-full">
          <path
            d="M9 3v6H3v6h6v6h6v-6h6V9h-6V3z"
            fill={color}
            stroke="white"
            strokeWidth="1.5"
  ...
  return (
    ...
    >
      ...
      <div 
        ...
        style={{
          background: `linear-gradient(180deg, ${item.gradient[0]}44, ${item.gradient[1]}77, #052e16)`,
      ...
      >
        ...
        <div className="absolute top-3 right-3 opacity-30">
          <MedicalCross color="#FFFFFF" size={24} />
        </div>
        <div className="absolute bottom-16 left-3 text-xl opacity-15 select-none pointer-events-none">💉</div>
        ...
        <div className="absolute top-3 left-1/2 -translate-x-1/2 z-20 flex flex-col items-center gap-1">
          ...
          >
            <Heart size={12} className="text-white" fill="white" /> الصحة
          </div>
        ...
        {mode === 'word' ? (
          ...
          <div className="px-3 pb-3 flex flex-col items-center gap-2 z-10 w-full">
            ...
            >
              ...
              <span className="text-[10px] font-black text-green-200" dir="ltr">
...
function SpanishHealthLessonInner() {
  ...
  const [itemChoices, setItemChoices] = useState<SpanishHealthItem[]>([]);
  ...
  const group = SPANISH_HEALTH_GROUPS[groupIdx];
  ...
  useEffect(() => {
    if (item && phase === 'listen') {
      setItemChoices(generateSpanishHealthChoices(item.word, 3));
  ...
  useEffect(() => {
    if (item && phase === 'build') {
      setAvailableWords(generateHealthSentenceWordPool(item));
      setSelectedWords([]);
      setStatus('idle');
  ...
  const handleItemChoice = (choice: SpanishHealthItem, e: React.MouseEvent) => {
  ...
  const handleSelectWord = (word: string, idx: number) => {
    ...
    if (newSelected.length === item.sentenceWords.length) {
      const isCorrect = checkHealthSentenceOrder(newSelected, item.sentenceWords);
      if (isCorrect) {
        setStatus('correct');
        setTimeout(() => {
    ...
    <div className="relative min-h-screen overflow-hidden" 
      ...
      <div className="fixed inset-0 pointer-events-none" style={{ zIndex: 0 }}>
        ...
        <div className="absolute inset-0" style={{
          background: isMobile 
            ? 'linear-gradient(180deg, rgba(5,40,20,0.75) 0%, rgba(5,30,15,0.9) 100%)'
            : 'radial-gradient(ellipse at 20% 20%, rgba(15,60,30,0.85) 0%, rgba(10,45,20,0.92) 50%, rgba(5,30,15,0.95) 100%)',
        }} />
        <motion.div
            ...
            {phase !== 'group-success' && phase !== 'all-done' && (
              <div className="flex justify-center mt-1.5">
                <div className="flex items-center gap-1 px-2 py-1 rounded-lg bg-black/50 backdrop-blur-md border border-white/15">
                  <Stepper currentStep={itemIdx} totalSteps={group?.items.length || 5} isMobile color={item?.color || '#22C55E'} />
                </div>
              </div>
            )}
          </>
          ...
          <div className="max-w-[1600px] mx-auto flex items-center justify-between gap-4">
            ...
            {phase !== 'group-success' && phase !== 'all-done' && (
              <div className="flex items-center gap-1 px-4 py-2 rounded-2xl bg-black/50 backdrop-blur-xl border-2 border-white/15">
                <Stepper currentStep={itemIdx} totalSteps={group?.items.length || 5} isMobile={false} color={item?.color || '#22C55E'} />
              </div>
            )}
                ...
                <div className="flex items-center gap-2">
                  <div className="w-20 h-2 bg-white/15 rounded-full overflow-hidden border border-white/20">
                    <motion.div className="h-full rounded-full" 
                      style={{ background: 'linear-gradient(to right, #22C55E, #FFD700)' }}
        ...
        <AnimatePresence mode="wait">
          ...
          {phase === 'listen' && item && (
            <motion.div key={`listen-${groupIdx}-${itemIdx}`}
              ...
              <div className={`flex ${isMobile ? 'flex-col' : 'flex-row'} items-center justify-center gap-6 md:gap-10`}>
                ...
                <HealthCard item={item} size={isMobile ? 200 : 280} mode="word" />
          ...
          {phase === 'build' && item && (
            <motion.div key={`build-${groupIdx}-${itemIdx}`}
              ...
              <div className={`flex ${isMobile ? 'flex-col' : 'flex-row'} items-center justify-center gap-6 md:gap-10`}>
                ...
                <HealthCard item={item} size={isMobile ? 220 : 280} mode="sentence" />
              ...
              <div className={`flex ${isMobile ? 'flex-col' : 'flex-row'} items-center justify-center gap-6 md:gap-10`}>
                ...
                <div className={`flex flex-col items-center gap-4 ${isMobile ? 'w-full max-w-sm' : 'flex-1 max-w-md'}`}>
                  ...
                  <div className="w-full p-3 rounded-2xl border-2"
                    ...
                    <div className="text-[10px] text-green-300 font-black mb-2 text-center">
          ...
          {phase === 'group-success' && (
            <motion.div key="success" initial={{ scale: 0.5, opacity: 0 }} animate={{ scale: 1, opacity: 1 }}
              ...
              <h2 className="text-3xl md:text-5xl font-black text-white">¡Muy saludable!</h2>
              <p className="text-lg md:text-xl text-green-400 font-bold">أنهيت {group.titleEs}</p>
              ...
              <button 
                onClick={() => {
                  if (groupIdx < SPANISH_HEALTH_GROUPS.length - 1) {
                    setGroupIdx(g => g + 1); 
                    setItemIdx(0); 
                    setPhase('listen');
                    setCorrectInGroup(0);
                ...
                style={{ background: 'linear-gradient(135deg, #22C55E, #FFD700)', boxShadow: '0 10px 30px rgba(34,197,94,0.4)' }}
              >
                المجموعة التالية 🚀
              </button>
            </motion.div>
          ...
          {phase === 'all-done' && (
            <motion.div key="done" initial={{ opacity: 0 }} animate={{ opacity: 1 }}
              ...
              <h1 className="text-3xl md:text-5xl font-black text-white">¡Hospital Sant Pau completo!</h1>
              <p className="text-lg md:text-xl text-white/60 max-w-md mx-auto">تعلمت 15 كلمة عن الصحة والمستشفى + 15 جملة كاملة!</p>
              <p className="text-sm text-green-300/80 max-w-md mx-auto">
                دلوقتي تقدر تتكلم مع الدكتور بالأسباني! 🏥💚
              </p>
      ...
      <div className="fixed bottom-0 left-0 right-0 z-30 px-2 md:px-4 pb-1.5 pointer-events-none"
        ...
        <div className={`mx-auto pointer-events-auto ${isMobile ? 'max-w-md' : 'max-w-[1500px]'}`}>
          <div className="relative rounded-xl px-4 md:px-6 py-1.5 md:py-2"
            style={{
              background: 'linear-gradient(135deg, rgba(15,60,30,0.85), rgba(10,45,20,0.9))',
              backdropFilter: 'blur(30px)',
              border: '1.5px solid rgba(255,255,255,0.2)',
              boxShadow: `0 10px 30px rgba(0,0,0,0.5), 0 0 25px rgba(34,197,94,0.2)`,
            }}>
      ...
      <ToroBull mood={toroMood} message={toroMessage} idleGlowColor={item?.color || '#22C55E'} />
      <ConfettiBurst trigger={confettiTrigger} x={confettiPos.x} y={confettiPos.y} 
        colors={[item?.color || '#22C55E', '#FFD700', '#ffffff']} />
    </div>
  );
}

export default function SpanishHealthLessonPage() {
  return (
    <Suspense fallback={<div className="min-h-screen bg-[#07090D] flex items-center justify-center text-white">Loading...</div>}>
      <SpanishHealthLessonInner />
    </Suspense>
  );
}