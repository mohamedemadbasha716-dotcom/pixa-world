...
import { 
  SPANISH_ART_ITEMS, 
  SPANISH_ART_GROUPS, 
  generateSpanishArtChoices,
  generateArtSentenceWordPool,
  checkArtSentenceOrder,
  type SpanishArtItem
} from '@/data/spanish-3/art';
...
function getCategoryStyle(category: 'Herramienta' | 'TipoArte' | 'Color') {
  switch (category) {
    case 'Herramienta':
      return { bg: 'rgba(236,72,153,0.7)', border: '#EC4899', icon: '🖌️', label: 'أدوات' };
    case 'TipoArte':
      return { bg: 'rgba(139,92,246,0.7)', border: '#8B5CF6', icon: '🎨', label: 'فنون' };
    case 'Color':
      return { bg: 'rgba(234,179,8,0.7)', border: '#EAB308', icon: '🌈', label: 'ألوان' };
  }
}

// ═══════════════════════════════════════
// 🎨 Color Palette - بالتة ألوان متحركة
// ═══════════════════════════════════════
function ColorPalette({ size = 40 }: { size?: number }) {
  const palettes = ['🎨', '🖼️', '🎭'];
  ...
  return (
    ...
    >
      ...
      <div 
        ...
        style={{
          background: `linear-gradient(180deg, ${item.gradient[0]}44, ${item.gradient[1]}77, #1e0a1a)`,
      ...
      >
        ...
        {/* بقع ألوان في الخلفية (زي لوحة فنية) */}
        <div 
          className="absolute inset-0 opacity-15 pointer-events-none"
          style={{
            backgroundImage: `
              radial-gradient(circle at 15% 25%, #FBBF24 20px, transparent 22px),
              radial-gradient(circle at 85% 30%, #EC4899 15px, transparent 18px),
              radial-gradient(circle at 25% 75%, #8B5CF6 18px, transparent 20px),
              radial-gradient(circle at 75% 80%, #06B6D4 12px, transparent 15px)
            `,
        ...
        <div className="absolute top-3 left-1/2 -translate-x-1/2 z-20 flex flex-col items-center gap-1">
          ...
          >
            <Palette size={12} className="text-white" /> الفن
          </div>
...
function SpanishArtLessonInner() {
  ...
  const [itemChoices, setItemChoices] = useState<SpanishArtItem[]>([]);
  ...
  const group = SPANISH_ART_GROUPS[groupIdx];
  ...
  useEffect(() => {
    if (item && phase === 'listen') {
      setItemChoices(generateSpanishArtChoices(item.word, 3));
  ...
  useEffect(() => {
    if (item && phase === 'build') {
      setAvailableWords(generateArtSentenceWordPool(item));
      setSelectedWords([]);
      setStatus('idle');
  ...
  const handleItemChoice = (choice: SpanishArtItem, e: React.MouseEvent) => {
  ...
  const handleSelectWord = (word: string, idx: number) => {
    ...
    if (newSelected.length === item.sentenceWords.length) {
      const isCorrect = checkArtSentenceOrder(newSelected, item.sentenceWords);
      if (isCorrect) {
        setStatus('correct');
        setTimeout(() => {
    ...
    <div className="relative min-h-screen overflow-hidden" 
      ...
      <div className="fixed inset-0 pointer-events-none" style={{ zIndex: 0 }}>
        ...
        <div className="absolute inset-0" style={{
          background: isMobile 
            ? 'linear-gradient(180deg, rgba(50,15,40,0.8) 0%, rgba(35,10,30,0.92) 100%)'
            : 'radial-gradient(ellipse at 20% 20%, rgba(90,20,70,0.9) 0%, rgba(60,15,55,0.94) 50%, rgba(35,10,30,0.98) 100%)',
        }} />
        <motion.div
                ...
                <div className="flex items-center gap-2">
                  <div className="w-20 h-2 bg-white/15 rounded-full overflow-hidden border border-white/20">
                    <motion.div className="h-full rounded-full" 
                      style={{ background: 'linear-gradient(to right, #EC4899, #FFD700)' }}
        ...
        <AnimatePresence mode="wait">
          ...
          {phase === 'listen' && item && (
            <motion.div key={`listen-${groupIdx}-${itemIdx}`}
              ...
              <div className={`flex ${isMobile ? 'flex-col' : 'flex-row'} items-center justify-center gap-6 md:gap-10`}>
                ...
                <ArtCard item={item} size={isMobile ? 200 : 280} mode="word" />
          ...
          {phase === 'build' && item && (
            <motion.div key={`build-${groupIdx}-${itemIdx}`}
              ...
              <div className={`flex ${isMobile ? 'flex-col' : 'flex-row'} items-center justify-center gap-6 md:gap-10`}>
                ...
                <ArtCard item={item} size={isMobile ? 220 : 280} mode="sentence" />
                ...
                <div className={`flex flex-col items-center gap-3 ${isMobile ? 'w-full max-w-sm' : 'flex-1 max-w-md'}`}>
                  ...
                  {status === 'correct' && (
                    <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }}
                      ...
                      <span className="font-black text-white text-sm">¡Bravo!</span>
                    </motion.div>
                  )}
                </div>
              </div>
          ...
          {phase === 'group-success' && (
            <motion.div key="success" initial={{ scale: 0.5, opacity: 0 }} animate={{ scale: 1, opacity: 1 }}
              ...
              <h2 className="text-3xl md:text-5xl font-black text-white">¡Como Dalí!</h2>
              ...
              <button 
                onClick={() => {
                  if (groupIdx < SPANISH_ART_GROUPS.length - 1) {
                    setGroupIdx(g => g + 1); 
                    setItemIdx(0); 
                    setPhase('listen');
                    setCorrectInGroup(0);
                ...
                style={{ background: 'linear-gradient(135deg, #EC4899, #FFD700)', boxShadow: '0 10px 30px rgba(236,72,153,0.4)' }}
              >
                المجموعة التالية 🚀
              </button>
            </motion.div>
          ...
          {phase === 'all-done' && (
            <motion.div key="done" initial={{ opacity: 0 }} animate={{ opacity: 1 }}
              ...
              <h1 className="text-3xl md:text-5xl font-black text-white">¡Museo Dalí completo!</h1>
              <p className="text-lg md:text-xl text-white/60 max-w-md mx-auto">تعلمت 15 كلمة عن الفن والألوان + 15 جملة كاملة!</p>
              <p className="text-sm text-pink-300/80 max-w-md mx-auto">
                دلوقتي تقدر تتكلم عن الفن والألوان بالأسباني! 🎨🖌️
              </p>
      ...
      <div className="fixed bottom-0 left-0 right-0 z-30 px-2 md:px-4 pb-1.5 pointer-events-none"
        ...
        <div className={`mx-auto pointer-events-auto ${isMobile ? 'max-w-md' : 'max-w-[1500px]'}`}>
          <div className="relative rounded-xl px-4 md:px-6 py-1.5 md:py-2"
            style={{
              background: 'linear-gradient(135deg, rgba(90,20,70,0.85), rgba(60,15,55,0.9))',
