...
import { 
  ...
  ShoppingBag, X, BookOpen
} from 'lucide-react';

import { 
  SPANISH_SHOPPING_ITEMS, 
  SPANISH_SHOPPING_GROUPS, 
  generateSpanishShoppingChoices,
  generateShoppingSentenceWordPool,
  checkShoppingSentenceOrder,
  type SpanishShoppingItem
} from '@/data/spanish-3/shopping';
...
const LESSON_ID = 'es-boqueria-shopping';
...
function getCategoryStyle(category: 'Dinero' | 'Tienda' | 'Mercado') {
  switch (category) {
    case 'Dinero':
      return { bg: 'rgba(234,179,8,0.7)', border: '#EAB308', icon: '💰', label: 'فلوس' };
    case 'Tienda':
      return { bg: 'rgba(249,115,22,0.7)', border: '#F97316', icon: '🏪', label: 'محل' };
    case 'Mercado':
      return { bg: 'rgba(245,158,11,0.7)', border: '#F59E0B', icon: '🏛️', label: 'سوق' };
  }
}

// ═══════════════════════════════════════
// 🏷️ Price Tag - تاج سعر متحرك
// ═══════════════════════════════════════
function PriceTag({ size = 36 }: { size?: number }) {
  return (
    <motion.div
      animate={{ 
        rotate: [-8, 8, -8],
...
// ═══════════════════════════════════════
// 🛒 Shopping Cart - عربة تسوق متحركة
// ═══════════════════════════════════════
function ShoppingCart({ size = 28 }: { size?: number }) {
  return (
    <motion.div
      animate={{ 
        x: [-3, 3, -3],
  ...
  return (
    ...
    >
      ...
      <div 
        ...
        style={{
          background: `linear-gradient(180deg, ${item.gradient[0]}44, ${item.gradient[1]}77, #1a0e05)`,
          borderColor: item.color,
          boxShadow: `0 20px 60px ${item.color}66, inset 0 2px 0 rgba(255,255,255,0.3)`,
        }}
      >
        {/* ديكور: تاج سعر + عربة تسوق */}
        ...
        <div className="absolute top-3 left-1/2 -translate-x-1/2 z-20 flex flex-col items-center gap-1">
          ...
          >
            <ShoppingBag size={12} className="text-white" /> التسوق
          </div>
...
function SpanishShoppingLessonInner() {
  ...
  const [itemChoices, setItemChoices] = useState<SpanishShoppingItem[]>([]);
  ...
  const group = SPANISH_SHOPPING_GROUPS[groupIdx];
  ...
  useEffect(() => {
    if (item && phase === 'listen') {
      setItemChoices(generateSpanishShoppingChoices(item.word, 3));
  ...
  useEffect(() => {
    if (item && phase === 'build') {
      setAvailableWords(generateShoppingSentenceWordPool(item));
      setSelectedWords([]);
      setStatus('idle');
  ...
  const handleItemChoice = (choice: SpanishShoppingItem, e: React.MouseEvent) => {
  ...
  const handleSelectWord = (word: string, idx: number) => {
    ...
    if (newSelected.length === item.sentenceWords.length) {
      const isCorrect = checkShoppingSentenceOrder(newSelected, item.sentenceWords);
      if (isCorrect) {
        setStatus('correct');
        setTimeout(() => {
    ...
    <div className="relative min-h-screen overflow-hidden" 
      ...
      <div className="fixed inset-0 pointer-events-none" style={{ zIndex: 0 }}>
        ...
        <div className="absolute inset-0" style={{
          background: isMobile 
            ? 'linear-gradient(180deg, rgba(40,20,5,0.8) 0%, rgba(30,15,5,0.92) 100%)'
            : 'radial-gradient(ellipse at 20% 20%, rgba(90,50,15,0.9) 0%, rgba(60,35,10,0.94) 50%, rgba(30,15,5,0.98) 100%)',
        }} />
        <motion.div
        ...
        <AnimatePresence mode="wait">
          ...
          {phase === 'listen' && item && (
            <motion.div key={`listen-${groupIdx}-${itemIdx}`}
              ...
              <div className={`flex ${isMobile ? 'flex-col' : 'flex-row'} items-center justify-center gap-6 md:gap-10`}>
                ...
                <ShoppingCard item={item} size={isMobile ? 200 : 280} mode="word" />
          ...
          {phase === 'build' && item && (
            <motion.div key={`build-${groupIdx}-${itemIdx}`}
              ...
              <div className={`flex ${isMobile ? 'flex-col' : 'flex-row'} items-center justify-center gap-6 md:gap-10`}>
                ...
                <ShoppingCard item={item} size={isMobile ? 220 : 280} mode="sentence" />
          ...
          {phase === 'group-success' && (
            <motion.div key="success" initial={{ scale: 0.5, opacity: 0 }} animate={{ scale: 1, opacity: 1 }}
              ...
              <h2 className="text-3xl md:text-5xl font-black text-white">¡Buena compra!</h2>
              ...
              <button 
                onClick={() => {
                  if (groupIdx < SPANISH_SHOPPING_GROUPS.length - 1) {
                    setGroupIdx(g => g + 1); 
                    setItemIdx(0); 
                    setPhase('listen');
                    setCorrectInGroup(0);
          ...
          {phase === 'all-done' && (
            <motion.div key="done" initial={{ opacity: 0 }} animate={{ opacity: 1 }}
              ...
              <div className="flex flex-wrap justify-center gap-2 max-w-2xl mx-auto mb-4">
                {SPANISH_SHOPPING_ITEMS.map((s, i) => (
              ...
              <p className="text-lg md:text-xl text-white/60 max-w-md mx-auto">تعلمت 15 كلمة عن التسوق والفلوس + 15 جملة كاملة!</p>
              <p className="text-sm text-orange-300/80 max-w-md mx-auto">
                دلوقتي تقدر تشتري وتسأل عن الأسعار بالأسباني! 🛒💰
              </p>
      ...
      <div className="fixed bottom-0 left-0 right-0 z-30 px-2 md:px-4 pb-1.5 pointer-events-none"
        ...
        <div className={`mx-auto pointer-events-auto ${isMobile ? 'max-w-md' : 'max-w-[1500px]'}`}>
          <div className="relative rounded-xl px-4 md:px-6 py-1.5 md:py-2"
            style={{
              background: 'linear-gradient(135deg, rgba(90,50,15,0.85), rgba(60,35,10,0.9))',
