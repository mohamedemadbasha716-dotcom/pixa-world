...
import {
  SPANISH_VERBS_REGULAR_ITEMS,
  SPANISH_VERBS_REGULAR_GROUPS,
  VERB_TYPE_STYLES,
  generateVerbsRegularChoices,
  generateVerbsSentenceWordPool,
  checkVerbsSentenceOrder,
  type SpanishVerbsRegularItem,
} from '@/data/spanish-4/verbs-regular';
...
const LESSON_ID = 'es-sevilla-verbs-regular';
...
// ═══════════════════════════════════════
// 🏛️ Sevilla Tower - برج إشبيلية متحرك
// ═══════════════════════════════════════

function SevillaDecor({ size = 120 }: { size?: number }) {
  return (
    <motion.div
    ...
    >
      ...
      <motion.div
        ...
        style={{
          fontSize: size * 0.75,
          filter: 'drop-shadow(0 10px 30px rgba(251,146,60,0.7))',
          lineHeight: 1,
          position: 'relative',
          zIndex: 10,
        }}
...
function VerbCard({
  ...
  const typeStyle = VERB_TYPE_STYLES[item.verbType];
  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{
          background: `radial-gradient(circle, ${item.color}aa, transparent 70%)`,
        }}
      />
      ...
      <div
        ...
        style={{
          background: `linear-gradient(180deg, ${item.gradient[0]}44, ${item.gradient[1]}88, #0f0a00)`,
      ...
      >
        ...
        <div className="absolute top-3 left-1/2 -translate-x-1/2 z-20 flex flex-col items-center gap-1.5">
          <div
            className="px-4 py-1.5 rounded-2xl border-2 font-black text-white text-sm flex items-center gap-1.5"
            style={{
        ...
        <div className="flex-1 flex flex-col items-center justify-center w-full pt-20 pb-2 gap-2">
          ...
          {item.note && (
            <div
              className="px-2.5 py-0.5 rounded-full text-[9px] font-bold text-white backdrop-blur-md border flex items-center gap-1"
              style={{
        ...
        {mode === 'word' ? (
          ...
          <div className="px-3 pb-4 flex flex-col items-center gap-2 z-10 w-full">
            ...
            >
              <BookOpen size={10} className="text-orange-300" />
              <span
                className="text-[10px] font-black text-orange-200"
                dir="ltr"
              >
                {item.grammarHint.pattern}
              </span>
            ...
            <div
              className="text-sm font-bold text-white/80 text-center"
              style={{ fontFamily: "'Tajawal', sans-serif" }}
            >
              {item.sentenceAr}
            </div>
...
function SpanishVerbsRegularLessonInner() {
  ...
  const [itemChoices, setItemChoices] = useState<SpanishVerbsRegularItem[]>([]);
  ...
  const group = SPANISH_VERBS_REGULAR_GROUPS[groupIdx];
  ...
  useEffect(() => {
    if (item && phase === 'listen') {
      setItemChoices(generateVerbsRegularChoices(item.word, 3));
  ...
  useEffect(() => {
    if (item && phase === 'build') {
      setAvailableWords(generateVerbsSentenceWordPool(item));
      setSelectedWords([]);
      setStatus('idle');
  ...
  const handleItemChoice = (
    choice: SpanishVerbsRegularItem,
    e: React.MouseEvent
  ) => {
  ...
  const handleSelectWord = (word: string, idx: number) => {
    ...
    if (newSelected.length === item.sentenceWords.length) {
      const isCorrect = checkVerbsSentenceOrder(newSelected, item.sentenceWords);
      if (isCorrect) {
        setStatus('correct');
        setTimeout(() => {
  ...
  if (isLoading) {
    return (
      <div className="min-h-screen bg-[#0f0a00] flex items-center justify-center text-white font-black"
        style={{ fontFamily: "'Tajawal', sans-serif" }}>
        جاري التحميل...
      </div>
    );
  ...
  return (
    <div
      ...
      style={{ background: '#0f0a00', fontFamily: "'Tajawal', sans-serif" }}
    >
      {/* ═══ Background ═══ */}
      <div className="fixed inset-0 pointer-events-none" style={{ zIndex: 0 }}>
        <img
          src={isMobile ? '/maps/spanish-map-4-mob.webp' : '/maps/spanish-map-4-pc.webp'}
          alt="bg"
        ...
        <div
          ...
          style={{
            background: isMobile
              ? 'linear-gradient(180deg, rgba(30,15,0,0.82) 0%, rgba(20,10,0,0.93) 100%)'
              : 'radial-gradient(ellipse at 20% 20%, rgba(80,40,0,0.88) 0%, rgba(50,25,0,0.93) 50%, rgba(15,10,0,0.97) 100%)',
          }}
        />
        <motion.div
          className="absolute inset-0 opacity-30"
          style={{
            background: `radial-gradient(ellipse 80% 50% at 50% 0%, ${item?.color ?? '#F97316'}33, transparent 70%)`,
          }}
      ...
      {/* ═══ Header ═══ */}
      <div
        className="fixed top-0 left-0 right-0 z-30 px-2 md:px-6"
        ...
        {isMobile ? (
          <>
            <div className="flex items-center justify-between gap-1.5">
              <button
                onClick={() => router.push('/spanish-character-and-map?map=4')}
            ...
            {phase !== 'group-success' && phase !== 'all-done' && (
              <div className="flex justify-center mt-1.5">
                <div className="flex items-center gap-1 px-2 py-1 rounded-lg bg-black/50 backdrop-blur-md border border-white/15">
                  <Stepper
                    currentStep={itemIdx}
                    totalSteps={group?.items.length || 5}
                    isMobile
                ...
                <div className="flex items-center gap-2">
                  <div className="w-20 h-2 bg-white/15 rounded-full overflow-hidden border border-white/20">
                    <motion.div
                      ...
                      style={{ background: 'linear-gradient(to right, #F97316, #FFD700)' }}
      ...
      <main className="relative z-10 min-h-screen flex items-center justify-center px-3 pt-24 md:pt-28 pb-28">
        <AnimatePresence mode="wait">
          ...
          {/* ══════════════════════════════
              Phase 1: Listen & Choose
          ══════════════════════════════ */}
          {phase === 'listen' && item && (
            <motion.div
              key={`listen-${groupIdx}-${itemIdx}`}
            ...
            >
              <div className={`flex ${isMobile ? 'flex-col' : 'flex-row'} items-center justify-center gap-6 md:gap-10`}>
                ...
                <VerbCard item={item} size={isMobile ? 200 : 270} mode="word" />

                <div className={`flex flex-col items-center gap-4 ${isMobile ? 'w-full max-w-sm' : 'flex-1 max-w-md'}`}>

                  {/* تعليمات */}
                  <div
                    className="px-5 py-2 rounded-2xl backdrop-blur-md border-2"
                    style={{
                  ...
                  >
                    <span className="font-black text-white text-sm md:text-base">
                      🎧 استمع واختر الفعل الصحيح
                    </span>
                  </div>

                  <SoundButton
                    onClick={() => speakSpanishWord(item.conjugated)}
                    size={isMobile ? 55 : 70}
                  />
                  ...
                  <div className="flex flex-col gap-2 md:gap-3 w-full mt-1">
                    {itemChoices.map((choice, idx) => {
                      ...
                      const typeStyle = VERB_TYPE_STYLES[choice.verbType];
                      ...
                      return (
                        <motion.button
                          ...
                          style={{
                            ...
                            boxShadow: isCorrect ? `0 0 25px ${choice.color}55` : 'none',
                          }}
                        >
                          {/* إيموجي + نوع الفعل */}
                          <div
                            className="rounded-2xl flex-shrink-0 border-2 border-white/30 flex flex-col items-center justify-center gap-0.5"
                            style={{
                              width: isMobile ? 48 : 60,
                              height: isMobile ? 48 : 60,
                              background: `linear-gradient(135deg, ${choice.gradient[0]}99, ${choice.gradient[1]}99)`,
                              boxShadow: `0 4px 12px ${choice.color}66`,
                            }}
                          >
                            ...
                            <span
                              className="text-[8px] font-black text-white/80 px-1 rounded"
                          ...
                          <div className="flex-1 flex items-center justify-between gap-2">
                            <div dir="ltr">
                              ...
                              <div className="text-[10px] text-white/50 font-bold mt-0.5">
                                yo {choice.conjugated}
                              </div>
                            </div>
                            <div className="text-right" dir="rtl">
                              <div className={`font-bold text-white/70 ${isMobile ? 'text-xs' : 'text-sm'}`}>
                                {choice.wordAr}
                              </div>
                            </div>
                          </div>
                          ...
                          {isCorrect && (
                            <Check size={isMobile ? 20 : 24} className="text-white flex-shrink-0" strokeWidth={3} />
                          )}
                        </motion.button>
                      );
                    })}
          ...
          {phase === 'build' && item && (
            ...
            >
              <div className={`flex ${isMobile ? 'flex-col' : 'flex-row'} items-center justify-center gap-6 md:gap-10`}>
                ...
                <VerbCard item={item} size={isMobile ? 210 : 270} mode="sentence" />

                <div className={`flex flex-col items-center gap-3 ${isMobile ? 'w-full max-w-sm' : 'flex-1 max-w-md'}`}>

                  <div
                    className="px-5 py-2 rounded-2xl backdrop-blur-md border-2 text-center"
                    style={{
                      background: 'rgba(255,255,255,0.08)',
                      borderColor: `${item.color}55`,
                    }}
                  >
                    <span className="font-black text-white text-sm md:text-base">
                      🧱 كوّن الجملة بالترتيب الصحيح
                    </span>
                    <div className="text-[10px] text-white/55 mt-0.5">
                      دوس على الكلمات اللي تحت ⬇️
                    </div>
                  </div>

                  <button
                    onClick={() => speakSpanishSentence(item.sentenceEs)}
                    className="flex items-center gap-2 px-4 py-2 rounded-xl bg-white/10 border border-white/20 text-white text-sm font-bold hover:bg-white/20 transition"
                  >
                  ...
                  {/* قاعدة الجرامر */}
                  <div
                    className="px-3 py-1.5 rounded-xl border flex items-center gap-2 w-full"
                    style={{
                      background: 'rgba(249,115,22,0.15)',
                      borderColor: 'rgba(249,115,22,0.4)',
                    }}
                  >
                    <BookOpen size={12} className="text-orange-300 flex-shrink-0" />
                    <span className="text-[10px] font-bold text-orange-100">
                      {item.grammarHint.rule}
                    </span>
                  </div>
                  ...
                  {status === 'correct' && (
                    <motion.div
                      initial={{ scale: 0 }}
                      animate={{ scale: 1 }}
                      className="flex items-center gap-2 px-4 py-2 rounded-xl bg-green-500/25 border-2 border-green-400"
                    >
            ...
            >
              <div className={`flex ${isMobile ? 'flex-col' : 'flex-row'} items-center justify-center gap-6 md:gap-10`}>
                ...
                <div className={`flex flex-col items-center gap-4 ${isMobile ? 'w-full max-w-sm' : 'flex-1 max-w-md'}`}>
                  ...
                  >
                    <span className="font-black text-white text-sm md:text-base">
                      🎤 انطق الجملة كاملة بصوت واضح
                    </span>
                  </div>

                  {/* كلمات قابلة للاستماع */}
                  <div
                  ...
                  >
                    <div className="text-[10px] text-orange-300 font-black mb-2 text-center">
                    ...
                    <div className="flex flex-wrap justify-center gap-2">
                      {item.sentenceWords.map((word, i) => (
                        <button
                          ...
                          className="px-2.5 py-1.5 rounded-lg bg-white/10 hover:bg-white/20 border border-white/20 text-xs font-bold text-white transition"
                          dir="ltr"
                        >
                          🔊 {word}
                        </button>
                  ...
                  <button
                    onClick={() => speakSpanishSentence(item.sentenceEs)}
                    className="flex items-center gap-2 px-5 py-2 rounded-xl bg-white/5 border border-white/15 text-white/65 hover:bg-white/10 text-sm font-bold transition"
                  >
                    <Volume2 size={14} /> اسمع الجملة كاملة
                  </button>
                  ...
                  <button
                    onClick={handleSpeakDone}
                    className="flex items-center gap-2 px-4 py-2 rounded-xl text-white/40 hover:text-white text-sm font-bold transition"
                  >
          ...
          {phase === 'group-success' && (
            ...
            >
              ...
              {/* ملخص القاعدة */}
              <div
                className="px-5 py-4 rounded-2xl border-2 inline-block text-left"
                style={{
                  background: `linear-gradient(135deg, ${VERB_TYPE_STYLES[group.verbType].bg.replace('0.7', '0.25')}, rgba(0,0,0,0.3))`,
                  borderColor: 'rgba(255,255,255,0.2)',
                }}
              >
              ...
              <button
                onClick={() => {
                  if (groupIdx < SPANISH_VERBS_REGULAR_GROUPS.length - 1) {
                    setGroupIdx(g => g + 1);
                    setItemIdx(0);
                    setPhase('listen');
                    setCorrectInGroup(0);
          ...
          {phase === 'all-done' && (
            ...
            >
              ...
              <div className="flex flex-wrap justify-center gap-2 max-w-2xl mx-auto mb-4">
                {SPANISH_VERBS_REGULAR_ITEMS.map((v, i) => (
                  <motion.div
                    key={v.word}
                    initial={{ scale: 0, rotate: -180 }}
                    animate={{ scale: 1, rotate: 0 }}
                    transition={{ delay: i * 0.04, type: 'spring' }}
                    className="px-3 py-1.5 rounded-xl border-2 border-white/35 flex items-center gap-1.5"
                    style={{
              ...
              <SevillaDecor size={isMobile ? 110 : 150} />

              <h1 className="text-3xl md:text-5xl font-black text-white">
                ¡Eres un crack!
              </h1>
              <p className="text-lg md:text-xl text-white/60 max-w-md mx-auto">
                تعلمت 15 فعل منتظم + طريقة تصريفهم مع Yo!
              </p>
              <p className="text-sm text-orange-300/80 max-w-md mx-auto">
                دلوقتي تقدر تقول إيه اللي بتعمله بالأسباني! 💪🎉
              </p>

              {/* ملخص القواعد */}
              <div className="flex flex-wrap justify-center gap-3 max-w-lg mx-auto">
                {(['AR', 'ER', 'IR'] as const).map(type => {
                  const s = VERB_TYPE_STYLES[type];
                  return (
                    <div
                      key={type}
                      className="px-4 py-2 rounded-xl border-2 border-white/25 text-center"
                      style={{ background: s.bg }}
                    >
                      <div className="font-black text-white text-sm" dir="ltr">
                        -{type} → -o
                      </div>
              ...
              <button
                onClick={() => {
                  ...
                  router.push('/spanish-character-and-map?map=4');
                }}
      ...
      {/* ═══ Footer ═══ */}
      <div
        className="fixed bottom-0 left-0 right-0 z-30 px-2 md:px-4 pb-1.5 pointer-events-none"
        style={{ paddingBottom: 'calc(env(safe-area-inset-bottom, 0px) + 4px)' }}
      >
        <div className={`mx-auto pointer-events-auto ${isMobile ? 'max-w-md' : 'max-w-[1500px]'}`}>
          <div
            className="relative rounded-xl px-4 md:px-6 py-1.5 md:py-2"
            style={{
              background: 'linear-gradient(135deg, rgba(40,20,0,0.88), rgba(25,12,0,0.92))',
              backdropFilter: 'blur(30px)',
              border: '1.5px solid rgba(255,255,255,0.15)',
              boxShadow: '0 10px 30px rgba(0,0,0,0.5), 0 0 25px rgba(249,115,22,0.15)',
            }}
          >
            ...
            <div className="flex items-end justify-around gap-2 md:gap-3">
              ...
              >
                ...
                />
                <span className="text-[8px] md:text-[9px] font-black text-orange-400">خريطة</span>
              </button>
              ...
              <motion.div
                ...
                transition={{
                  duration: 1.5,
                  repeat: treasureState === 'opend' ? Infinity : 0,
                }}
                className="flex flex-col items-center gap-0.5"
              >
                <img
              ...
              <div className="flex flex-col items-center gap-0.5 opacity-70">
                <img
                  ...
                  style={{ filter: 'drop-shadow(0 2px 8px rgba(249,115,22,0.6))' }}
                />
                <span className="text-[8px] md:text-[9px] font-black text-orange-400">طاقة</span>
              </div>
              ...
              >
                <div className="relative">
                  <img
                    src="/treasuer/HINT.svg"
                    alt="hint"
                    className="w-9 h-9 md:w-11 md:h-11 object-contain"
                    style={{ filter: 'drop-shadow(0 2px 8px rgba(255,215,0,0.6))' }}
                  />
                  {hints > 0 && (
                    <div className="absolute -top-1 -right-1 w-4 h-4 rounded-full bg-red-500 text-white text-[9px] font-black flex items-center justify-center border border-black/90">
                      {hints}
                    </div>
                  )}
                </div>
      ...
      <ToroBull
        mood={toroMood}
        message={toroMessage}
        idleGlowColor={item?.color || '#F97316'}
      />
      <ConfettiBurst
        trigger={confettiTrigger}
        x={confettiPos.x}
        y={confettiPos.y}
        colors={[item?.color || '#F97316', '#FFD700', '#ffffff']}
      />
    </div>
  );
}

export default function SpanishVerbsRegularLessonPage() {
  return (
    <Suspense
      fallback={
        <div className="min-h-screen bg-[#0f0a00] flex items-center justify-center text-white">
          Loading...
        </div>
      }
    >
      <SpanishVerbsRegularLessonInner />
    </Suspense>
  );
}