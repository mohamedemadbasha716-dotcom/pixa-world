...
import { 
  SPANISH_COMMUNICATION_ITEMS, 
  SPANISH_COMMUNICATION_GROUPS, 
  generateSpanishCommunicationChoices,
  generateCommunicationSentenceWordPool,
  checkCommunicationSentenceOrder,
  type SpanishCommunicationItem
} from '@/data/spanish-4/communication';
...
const LESSON_ID = 'es-correos-communication';
...
function getCategoryStyle(category: 'Carta' | 'Telefono' | 'Conversacion') {
  switch (category) {
    case 'Carta':
      return { bg: 'rgba(59,130,246,0.7)', border: '#3B82F6', icon: '✉️', label: 'بريد' };
    case 'Telefono':
      return { bg: 'rgba(14,165,233,0.7)', border: '#0EA5E9', icon: '📱', label: 'موبايل' };
    case 'Conversacion':
      return { bg: 'rgba(124,58,237,0.7)', border: '#7C3AED', icon: '💬', label: 'حوار' };
  }
}

// ═══════════════════════════════════════
// 📮 Shaking Mailbox - صندوق بريد يهتز
// ═══════════════════════════════════════
function ShakingMailbox({ size = 32 }: { size?: number }) {
  return (
    <motion.div
      animate={{ 
        rotate: [-8, 8, -8],
...
// ═══════════════════════════════════════
// ✉️ Flying Envelope - مغلف طائر
// ═══════════════════════════════════════
function FlyingEnvelope({ size = 26 }: { size?: number }) {
  return (
    <motion.div
      animate={{ 
        x: [-10, 10, -10],
  ...
  return (
    ...
    >
      ...
      <div 
        ...
        style={{
          background: `linear-gradient(180deg, ${item.gradient[0]}44, ${item.gradient[1]}77, #0a1428)`,
      ...
      >
        <div className="absolute top-3 right-3 opacity-40">
          <ShakingMailbox size={26} />
        </div>
        <div className="absolute bottom-16 left-3 opacity-50">
          <FlyingEnvelope size={22} />
        </div>
        
        {/* خطوط بريدية في الخلفية */}
        <div 
          className="absolute inset-0 opacity-10 pointer-events-none"
          style={{
            backgroundImage: `
              repeating-linear-gradient(45deg, transparent, transparent 15px, rgba(255,255,255,0.3) 15px, rgba(255,255,255,0.3) 17px),
              repeating-linear-gradient(-45deg, transparent, transparent 15px, rgba(59,130,246,0.3) 15px, rgba(59,130,246,0.3) 17px)
            `,
          }}
        />
        ...
        <div className="absolute top-3 left-1/2 -translate-x-1/2 z-20 flex flex-col items-center gap-1">
          ...
          >
            <Mail size={12} className="text-white" /> التواصل
          </div>
        ...
        {mode === 'word' ? (
          ...
          <div className="px-3 pb-3 flex flex-col items-center gap-2 z-10 w-full">
            ...
            >
              ...
              <span className="text-[10px] font-black text-blue-200" dir="ltr">
...
function SpanishCommunicationLessonInner() {
  ...
  const [itemChoices, setItemChoices] = useState<SpanishCommunicationItem[]>([]);
  ...
  const group = SPANISH_COMMUNICATION_GROUPS[groupIdx];
  ...
  useEffect(() => {
    if (item && phase === 'listen') {
      setItemChoices(generateSpanishCommunicationChoices(item.word, 3));
  ...
  useEffect(() => {
    if (item && phase === 'build') {
      setAvailableWords(generateCommunicationSentenceWordPool(item));
      setSelectedWords([]);
      setStatus('idle');
  ...
  const handleItemChoice = (choice: SpanishCommunicationItem, e: React.MouseEvent) => {
  ...
  const handleSelectWord = (word: string, idx: number) => {
    ...
    if (newSelected.length === item.sentenceWords.length) {
      const isCorrect = checkCommunicationSentenceOrder(newSelected, item.sentenceWords);
      if (isCorrect) {
        setStatus('correct');
        setTimeout(() => {
    ...
    <div className="relative min-h-screen overflow-hidden" 
      ...
      <div className="fixed inset-0 pointer-events-none" style={{ zIndex: 0 }}>
        ...
        <div className="absolute inset-0" style={{
          background: isMobile 
            ? 'linear-gradient(180deg, rgba(10,25,50,0.8) 0%, rgba(5,15,40,0.92) 100%)'
            : 'radial-gradient(ellipse at 20% 20%, rgba(15,50,90,0.9) 0%, rgba(10,35,70,0.94) 50%, rgba(5,15,40,0.98) 100%)',
        }} />
        <motion.div
            ...
            {phase !== 'group-success' && phase !== 'all-done' && (
              <div className="flex justify-center mt-1.5">
                <div className="flex items-center gap-1 px-2 py-1 rounded-lg bg-black/50 backdrop-blur-md border border-white/15">
                  <Stepper currentStep={itemIdx} totalSteps={group?.items.length || 5} isMobile color={item?.color || '#3B82F6'} />
                </div>
              </div>
            )}
          </>
          ...
          <div className="max-w-[1600px] mx-auto flex items-center justify-between gap-4">
            ...
            {phase !== 'group-success' && phase !== 'all-done' && (
              <div className="flex items-center gap-1 px-4 py-2 rounded-2xl bg-black/50 backdrop-blur-xl border-2 border-white/15">
                <Stepper currentStep={itemIdx} totalSteps={group?.items.length || 5} isMobile={false} color={item?.color || '#3B82F6'} />
              </div>
            )}
                ...
                <div className="flex items-center gap-2">
                  <div className="w-20 h-2 bg-white/15 rounded-full overflow-hidden border border-white/20">
                    <motion.div className="h-full rounded-full" 
                      style={{ background: 'linear-gradient(to right, #3B82F6, #FFD700)' }}
        ...
        <AnimatePresence mode="wait">
          ...
          {phase === 'listen' && item && (
            <motion.div key={`listen-${groupIdx}-${itemIdx}`}
              ...
              <div className={`flex ${isMobile ? 'flex-col' : 'flex-row'} items-center justify-center gap-6 md:gap-10`}>
                ...
                <CommunicationCard item={item} size={isMobile ? 200 : 280} mode="word" />
          ...
          {phase === 'build' && item && (
            <motion.div key={`build-${groupIdx}-${itemIdx}`}
              ...
              <div className={`flex ${isMobile ? 'flex-col' : 'flex-row'} items-center justify-center gap-6 md:gap-10`}>
                ...
                <CommunicationCard item={item} size={isMobile ? 220 : 280} mode="sentence" />
                ...
                <div className={`flex flex-col items-center gap-3 ${isMobile ? 'w-full max-w-sm' : 'flex-1 max-w-md'}`}>
                  ...
                  <div className="px-3 py-1.5 rounded-xl border flex items-center gap-2 text-center"
                    ...
                    <BookOpen size={12} className="text-blue-300 flex-shrink-0" />
                    <span className="text-[10px] font-bold text-blue-100">{item.grammarHint.rule}</span>
                  </div>
                  ...
                  {status === 'correct' && (
                    <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }}
                      ...
                      <span className="font-black text-white text-sm">¡Enviado!</span>
                    </motion.div>
                  )}
                </div>
              </div>
              ...
              <div className={`flex ${isMobile ? 'flex-col' : 'flex-row'} items-center justify-center gap-6 md:gap-10`}>
                ...
                <div className={`flex flex-col items-center gap-4 ${isMobile ? 'w-full max-w-sm' : 'flex-1 max-w-md'}`}>
                  ...
                  <div className="w-full p-3 rounded-2xl border-2"
                    ...
                    <div className="text-[10px] text-blue-300 font-black mb-2 text-center">
          ...
          {phase === 'group-success' && (
            <motion.div key="success" initial={{ scale: 0.5, opacity: 0 }} animate={{ scale: 1, opacity: 1 }}
              ...
              <h2 className="text-3xl md:text-5xl font-black text-white">¡Mensaje enviado!</h2>
              <p className="text-lg md:text-xl text-blue-400 font-bold">أنهيت {group.titleEs}</p>
              ...
              <button 
                onClick={() => {
                  if (groupIdx < SPANISH_COMMUNICATION_GROUPS.length - 1) {
                    setGroupIdx(g => g + 1); 
                    setItemIdx(0); 
                    setPhase('listen');
                    setCorrectInGroup(0);
                ...
                style={{ background: 'linear-gradient(135deg, #3B82F6, #FFD700)', boxShadow: '0 10px 30px rgba(59,130,246,0.4)' }}
              >
                المجموعة التالية 🚀
              </button>
            </motion.div>
          ...
          {phase === 'all-done' && (
            <motion.div key="done" initial={{ opacity: 0 }} animate={{ opacity: 1 }}
              ...
              <div className="flex flex-wrap justify-center gap-2 max-w-2xl mx-auto mb-4">
                {SPANISH_COMMUNICATION_ITEMS.map((s, i) => (
              ...
              <h1 className="text-3xl md:text-5xl font-black text-white">¡Casa de Correos completa!</h1>
              <p className="text-lg md:text-xl text-white/60 max-w-md mx-auto">تعلمت 15 كلمة عن التواصل والرسائل + 15 جملة كاملة!</p>
              <p className="text-sm text-blue-300/80 max-w-md mx-auto">
                دلوقتي تقدر تتواصل بالأسباني بالرسائل والتليفون! 📮📱
              </p>
      ...
      <div className="fixed bottom-0 left-0 right-0 z-30 px-2 md:px-4 pb-1.5 pointer-events-none"
        ...
        <div className={`mx-auto pointer-events-auto ${isMobile ? 'max-w-md' : 'max-w-[1500px]'}`}>
          <div className="relative rounded-xl px-4 md:px-6 py-1.5 md:py-2"
            style={{
              background: 'linear-gradient(135deg, rgba(15,50,90,0.85), rgba(10,35,70,0.9))',
              backdropFilter: 'blur(30px)',
              border: '1.5px solid rgba(255,255,255,0.2)',
              boxShadow: `0 10px 30px rgba(0,0,0,0.5), 0 0 25px rgba(59,130,246,0.2)`,
            }}>
      ...
      <ToroBull mood={toroMood} message={toroMessage} idleGlowColor={item?.color || '#3B82F6'} />
      <ConfettiBurst trigger={confettiTrigger} x={confettiPos.x} y={confettiPos.y} 
        colors={[item?.color || '#3B82F6', '#FFD700', '#ffffff']} />
    </div>
  );
}

export default function SpanishCommunicationLessonPage() {
  return (
    <Suspense fallback={<div className="min-h-screen bg-[#07090D] flex items-center justify-center text-white">Loading...</div>}>
      <SpanishCommunicationLessonInner />
    </Suspense>
  );
}