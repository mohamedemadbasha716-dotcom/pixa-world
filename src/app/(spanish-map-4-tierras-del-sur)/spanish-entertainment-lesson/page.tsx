...
import { 
  SPANISH_ENTERTAINMENT_ITEMS, 
  SPANISH_ENTERTAINMENT_GROUPS, 
  generateSpanishEntertainmentChoices,
  generateEntertainmentSentenceWordPool,
  checkEntertainmentSentenceOrder,
  type SpanishEntertainmentItem
} from '@/data/spanish-4/entertainment';
...
const LESSON_ID = 'es-malaga-entertainment';
...
function getCategoryStyle(category: 'Cine' | 'Casa' | 'Diversion') {
  switch (category) {
    case 'Cine':
      return { bg: 'rgba(252,211,77,0.7)', border: '#FCD34D', icon: '🎬', label: 'سينما' };
    case 'Casa':
      return { bg: 'rgba(59,130,246,0.7)', border: '#3B82F6', icon: '📺', label: 'بيت' };
    case 'Diversion':
      return { bg: 'rgba(34,197,94,0.7)', border: '#22C55E', icon: '🎉', label: 'متعة' };
  }
}

// ═══════════════════════════════════════
// 🎬 Spinning Film Reel - بكرة فيلم دوّارة
// ═══════════════════════════════════════
function SpinningFilmReel({ size = 32 }: { size?: number }) {
  return (
    <motion.div
      animate={{ 
        rotate: 360,
      ...
      style={{ 
        ...
        fontSize: size,
        lineHeight: 1,
        filter: 'drop-shadow(0 4px 12px rgba(252,211,77,0.7))',
      }}
    >
      🎞️
    </motion.div>
...
// ═══════════════════════════════════════
// 🎥 Movie Camera - كاميرا سينما متحركة
// ═══════════════════════════════════════
function MovieCamera({ size = 26 }: { size?: number }) {
  return (
    <motion.div
      animate={{ 
        y: [0, -4, 0],
...
// ═══════════════════════════════════════
// 🎬 Entertainment Card - بطاقة الترفيه
// ═══════════════════════════════════════
function EntertainmentCard({ 
  ...
  return (
    ...
    >
      ...
      >
        {/* ديكور: بكرة فيلم + كاميرا */}
        <div className="absolute top-3 right-3 opacity-40">
          <SpinningFilmReel size={26} />
        </div>
        <div className="absolute bottom-16 left-3 opacity-50">
          <MovieCamera size={22} />
        </div>
        
        {/* أضواء كشافات في الخلفية */}
        <div 
          className="absolute inset-0 opacity-15 pointer-events-none"
          style={{
            backgroundImage: `
              radial-gradient(ellipse at 15% 25%, rgba(252,211,77,0.8) 0px, transparent 40px),
              radial-gradient(ellipse at 85% 30%, rgba(252,211,77,0.6) 0px, transparent 35px),
              radial-gradient(ellipse at 50% 90%, rgba(252,211,77,0.7) 0px, transparent 50px)
            `,
            filter: 'blur(15px)',
          }}
        />
        ...
        <div className="absolute top-3 left-1/2 -translate-x-1/2 z-20 flex flex-col items-center gap-1">
          ...
          >
            <Film size={12} className="text-amber-900" /> الترفيه
          </div>
        ...
        {mode === 'word' ? (
          ...
          <div className="px-3 pb-3 flex flex-col items-center gap-2 z-10 w-full">
            ...
            >
              <BookOpen size={10} className="text-yellow-300" />
              <span className="text-[10px] font-black text-yellow-200" dir="ltr">
...
function SpanishEntertainmentLessonInner() {
  ...
  const [itemChoices, setItemChoices] = useState<SpanishEntertainmentItem[]>([]);
  ...
  const group = SPANISH_ENTERTAINMENT_GROUPS[groupIdx];
  ...
  useEffect(() => {
    if (item && phase === 'listen') {
      setItemChoices(generateSpanishEntertainmentChoices(item.word, 3));
  ...
  useEffect(() => {
    if (item && phase === 'build') {
      setAvailableWords(generateEntertainmentSentenceWordPool(item));
      setSelectedWords([]);
      setStatus('idle');
  ...
  const handleItemChoice = (choice: SpanishEntertainmentItem, e: React.MouseEvent) => {
  ...
  const handleSelectWord = (word: string, idx: number) => {
    ...
    if (newSelected.length === item.sentenceWords.length) {
      const isCorrect = checkEntertainmentSentenceOrder(newSelected, item.sentenceWords);
      if (isCorrect) {
        setStatus('correct');
        setTimeout(() => {
    ...
    <div className="relative min-h-screen overflow-hidden" 
      ...
      <div className="fixed inset-0 pointer-events-none" style={{ zIndex: 0 }}>
        ...
        <div className="absolute inset-0" style={{
          background: isMobile 
            ? 'linear-gradient(180deg, rgba(40,25,10,0.8) 0%, rgba(30,15,5,0.92) 100%)'
            : 'radial-gradient(ellipse at 20% 20%, rgba(80,50,20,0.9) 0%, rgba(50,30,10,0.94) 50%, rgba(30,15,5,0.98) 100%)',
        }} />
        <motion.div
        ...
        {isMobile ? (
          <>
            <div className="flex items-center justify-between gap-1.5">
              <button onClick={() => router.push('/spanish-character-and-map?map=4')}
            ...
            {phase !== 'group-success' && phase !== 'all-done' && (
              <div className="flex justify-center mt-1.5">
                <div className="flex items-center gap-1 px-2 py-1 rounded-lg bg-black/50 backdrop-blur-md border border-white/15">
                  <Stepper currentStep={itemIdx} totalSteps={group?.items.length || 5} isMobile color={item?.color || '#FCD34D'} />
                </div>
              </div>
            )}
          </>
          ...
          <div className="max-w-[1600px] mx-auto flex items-center justify-between gap-4">
            ...
            {phase !== 'group-success' && phase !== 'all-done' && (
              <div className="flex items-center gap-1 px-4 py-2 rounded-2xl bg-black/50 backdrop-blur-xl border-2 border-white/15">
                <Stepper currentStep={itemIdx} totalSteps={group?.items.length || 5} isMobile={false} color={item?.color || '#FCD34D'} />
              </div>
            )}
                ...
                <div className="flex items-center gap-2">
                  <div className="w-20 h-2 bg-white/15 rounded-full overflow-hidden border border-white/20">
                    <motion.div className="h-full rounded-full" 
                      style={{ background: 'linear-gradient(to right, #FCD34D, #DC2626)' }}
        ...
        <AnimatePresence mode="wait">
          ...
          {phase === 'listen' && item && (
            <motion.div key={`listen-${groupIdx}-${itemIdx}`}
              ...
              <div className={`flex ${isMobile ? 'flex-col' : 'flex-row'} items-center justify-center gap-6 md:gap-10`}>
                ...
                <EntertainmentCard item={item} size={isMobile ? 200 : 280} mode="word" />
          ...
          {phase === 'build' && item && (
            <motion.div key={`build-${groupIdx}-${itemIdx}`}
              ...
              <div className={`flex ${isMobile ? 'flex-col' : 'flex-row'} items-center justify-center gap-6 md:gap-10`}>
                ...
                <EntertainmentCard item={item} size={isMobile ? 220 : 280} mode="sentence" />
                ...
                <div className={`flex flex-col items-center gap-3 ${isMobile ? 'w-full max-w-sm' : 'flex-1 max-w-md'}`}>
                  ...
                  <div className="px-3 py-1.5 rounded-xl border flex items-center gap-2 text-center"
                    ...
                    <BookOpen size={12} className="text-yellow-300 flex-shrink-0" />
                    <span className="text-[10px] font-bold text-yellow-100">{item.grammarHint.rule}</span>
                  </div>
                  ...
                  {status === 'correct' && (
                    <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }}
                      ...
                      <span className="font-black text-white text-sm">¡Acción!</span>
                    </motion.div>
                  )}
                </div>
              </div>
          ...
          {phase === 'group-success' && (
            <motion.div key="success" initial={{ scale: 0.5, opacity: 0 }} animate={{ scale: 1, opacity: 1 }}
              ...
              <h2 className="text-3xl md:text-5xl font-black text-white">¡Como una estrella!</h2>
              <p className="text-lg md:text-xl text-yellow-400 font-bold">أنهيت {group.titleEs}</p>
              ...
              <button 
                onClick={() => {
                  if (groupIdx < SPANISH_ENTERTAINMENT_GROUPS.length - 1) {
                    setGroupIdx(g => g + 1); 
                    setItemIdx(0); 
                    setPhase('listen');
                    setCorrectInGroup(0);
                ...
                style={{ background: 'linear-gradient(135deg, #FCD34D, #DC2626)', boxShadow: '0 10px 30px rgba(252,211,77,0.4)' }}
              >
                المجموعة التالية 🚀
              </button>
            </motion.div>
          ...
          {phase === 'all-done' && (
            <motion.div key="done" initial={{ opacity: 0 }} animate={{ opacity: 1 }}
              ...
              <div className="flex flex-wrap justify-center gap-2 max-w-2xl mx-auto mb-4">
                {SPANISH_ENTERTAINMENT_ITEMS.map((s, i) => (
              ...
              <h1 className="text-3xl md:text-5xl font-black text-white">¡Festival completo!</h1>
              <p className="text-lg md:text-xl text-white/60 max-w-md mx-auto">تعلمت 15 كلمة عن الترفيه والأفلام + 15 جملة كاملة!</p>
              <p className="text-sm text-yellow-300/80 max-w-md mx-auto">
                دلوقتي تقدر تتكلم عن الأفلام والبرامج بالأسباني! 🎬🎥
              </p>
      ...
      <div className="fixed bottom-0 left-0 right-0 z-30 px-2 md:px-4 pb-1.5 pointer-events-none"
        ...
        <div className={`mx-auto pointer-events-auto ${isMobile ? 'max-w-md' : 'max-w-[1500px]'}`}>
          <div className="relative rounded-xl px-4 md:px-6 py-1.5 md:py-2"
            style={{
              background: 'linear-gradient(135deg, rgba(80,50,20,0.85), rgba(50,30,10,0.9))',
              backdropFilter: 'blur(30px)',
              border: '1.5px solid rgba(255,255,255,0.2)',
              boxShadow: `0 10px 30px rgba(0,0,0,0.5), 0 0 25px rgba(252,211,77,0.2)`,
            }}>
      ...
      <ToroBull mood={toroMood} message={toroMessage} idleGlowColor={item?.color || '#FCD34D'} />
      <ConfettiBurst trigger={confettiTrigger} x={confettiPos.x} y={confettiPos.y} 
        colors={[item?.color || '#FCD34D', '#FFD700', '#ffffff']} />
    </div>
  );
}

export default function SpanishEntertainmentLessonPage() {
  return (
    <Suspense fallback={<div className="min-h-screen bg-[#07090D] flex items-center justify-center text-white">Loading...</div>}>
      <SpanishEntertainmentLessonInner />
    </Suspense>
  );
}