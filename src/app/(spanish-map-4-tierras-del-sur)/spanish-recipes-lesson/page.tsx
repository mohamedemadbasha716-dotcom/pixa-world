...
import { 
  SPANISH_RECIPES_ITEMS, 
  SPANISH_RECIPES_GROUPS, 
  generateSpanishRecipesChoices,
  generateRecipesSentenceWordPool,
  checkRecipesSentenceOrder,
  type SpanishRecipesItem
} from '@/data/spanish-4/recipes';
...
const LESSON_ID = 'es-triana-recipes';
...
function getCategoryStyle(category: 'Ingrediente' | 'Cocinar' | 'Mesa') {
  switch (category) {
    case 'Ingrediente':
      return { bg: 'rgba(249,115,22,0.7)', border: '#F97316', icon: '🥘', label: 'مكونات' };
    case 'Cocinar':
      return { bg: 'rgba(234,179,8,0.7)', border: '#EAB308', icon: '👨‍🍳', label: 'طبخ' };
    case 'Mesa':
      return { bg: 'rgba(220,38,38,0.7)', border: '#DC2626', icon: '🍽️', label: 'سفرة' };
  }
}

// ═══════════════════════════════════════
// 🍳 Sizzling Frying Pan - مقلاة تتأرجح
// ═══════════════════════════════════════
function SizzlingPan({ size = 32 }: { size?: number }) {
  return (
    <motion.div
      animate={{ 
        rotate: [-15, 15, -15],
...
// ═══════════════════════════════════════
// 👨‍🍳 Chef Hat - قبعة شيف متحركة
// ═══════════════════════════════════════
function ChefHatEmoji({ size = 28 }: { size?: number }) {
  return (
    <motion.div
      animate={{ 
        y: [0, -4, 0],
  ...
  return (
    ...
    >
      ...
      <div 
        ...
        style={{
          background: `linear-gradient(180deg, ${item.gradient[0]}44, ${item.gradient[1]}77, #2e0f05)`,
          borderColor: item.color,
          boxShadow: `0 20px 60px ${item.color}66, inset 0 2px 0 rgba(255,255,255,0.3)`,
        }}
      >
        {/* ديكور: مقلاة + قبعة شيف */}
        <div className="absolute top-3 right-3 opacity-40">
          <SizzlingPan size={28} />
        </div>
        <div className="absolute bottom-16 left-3 opacity-50">
          <ChefHatEmoji size={24} />
        </div>
        
        {/* بخار الطبخ في الخلفية */}
        <div 
          className="absolute inset-0 opacity-15 pointer-events-none"
          style={{
            backgroundImage: `
              radial-gradient(ellipse at 25% 20%, rgba(255,200,150,0.8) 0px, transparent 30px),
              radial-gradient(ellipse at 75% 25%, rgba(255,180,100,0.7) 0px, transparent 25px),
              radial-gradient(ellipse at 50% 85%, rgba(255,220,180,0.6) 0px, transparent 40px)
            `,
            filter: 'blur(12px)',
          }}
        />
        ...
        <div className="absolute top-3 left-1/2 -translate-x-1/2 z-20 flex flex-col items-center gap-1">
          ...
          >
            <ChefHat size={12} className="text-white" /> الوصفات
          </div>
...
function SpanishRecipesLessonInner() {
  ...
  const [itemChoices, setItemChoices] = useState<SpanishRecipesItem[]>([]);
  ...
  const group = SPANISH_RECIPES_GROUPS[groupIdx];
  ...
  useEffect(() => {
    if (item && phase === 'listen') {
      setItemChoices(generateSpanishRecipesChoices(item.word, 3));
  ...
  useEffect(() => {
    if (item && phase === 'build') {
      setAvailableWords(generateRecipesSentenceWordPool(item));
      setSelectedWords([]);
      setStatus('idle');
  ...
  const handleItemChoice = (choice: SpanishRecipesItem, e: React.MouseEvent) => {
  ...
  const handleSelectWord = (word: string, idx: number) => {
    ...
    if (newSelected.length === item.sentenceWords.length) {
      const isCorrect = checkRecipesSentenceOrder(newSelected, item.sentenceWords);
      if (isCorrect) {
        setStatus('correct');
        setTimeout(() => {
    ...
    <div className="relative min-h-screen overflow-hidden" 
      ...
      <div className="fixed inset-0 pointer-events-none" style={{ zIndex: 0 }}>
        ...
        <div className="absolute inset-0" style={{
          background: isMobile 
            ? 'linear-gradient(180deg, rgba(50,20,10,0.8) 0%, rgba(35,10,5,0.92) 100%)'
            : 'radial-gradient(ellipse at 20% 20%, rgba(90,40,15,0.9) 0%, rgba(60,25,10,0.94) 50%, rgba(30,10,5,0.98) 100%)',
        }} />
        <motion.div
        ...
        <AnimatePresence mode="wait">
          ...
          {phase === 'listen' && item && (
            <motion.div key={`listen-${groupIdx}-${itemIdx}`}
              ...
              <div className={`flex ${isMobile ? 'flex-col' : 'flex-row'} items-center justify-center gap-6 md:gap-10`}>
                ...
                <RecipesCard item={item} size={isMobile ? 200 : 280} mode="word" />
          ...
          {phase === 'build' && item && (
            <motion.div key={`build-${groupIdx}-${itemIdx}`}
              ...
              <div className={`flex ${isMobile ? 'flex-col' : 'flex-row'} items-center justify-center gap-6 md:gap-10`}>
                ...
                <RecipesCard item={item} size={isMobile ? 220 : 280} mode="sentence" />
                ...
                <div className={`flex flex-col items-center gap-3 ${isMobile ? 'w-full max-w-sm' : 'flex-1 max-w-md'}`}>
                  ...
                  {status === 'correct' && (
                    <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }}
                      ...
                      <span className="font-black text-white text-sm">¡Delicioso!</span>
                    </motion.div>
                  )}
                </div>
              </div>
          ...
          {phase === 'group-success' && (
            <motion.div key="success" initial={{ scale: 0.5, opacity: 0 }} animate={{ scale: 1, opacity: 1 }}
              ...
              <h2 className="text-3xl md:text-5xl font-black text-white">¡Chef estrella!</h2>
              ...
              <button 
                onClick={() => {
                  if (groupIdx < SPANISH_RECIPES_GROUPS.length - 1) {
                    setGroupIdx(g => g + 1); 
                    setItemIdx(0); 
                    setPhase('listen');
                    setCorrectInGroup(0);
                ...
                style={{ background: 'linear-gradient(135deg, #F97316, #FFD700)', boxShadow: '0 10px 30px rgba(249,115,22,0.4)' }}
              >
                المجموعة التالية 🚀
              </button>
            </motion.div>
          ...
          {phase === 'all-done' && (
            <motion.div key="done" initial={{ opacity: 0 }} animate={{ opacity: 1 }}
              ...
              <h1 className="text-3xl md:text-5xl font-black text-white">¡Taberna Triana completa!</h1>
              <p className="text-lg md:text-xl text-white/60 max-w-md mx-auto">تعلمت 15 كلمة عن الطبخ والوصفات + 15 جملة كاملة!</p>
              <p className="text-sm text-orange-300/80 max-w-md mx-auto">
                دلوقتي تقدر تطبخ وتتكلم عن الأكل بالأسباني! 🍳👨‍🍳
              </p>
      ...
      <div className="fixed bottom-0 left-0 right-0 z-30 px-2 md:px-4 pb-1.5 pointer-events-none"
        ...
        <div className={`mx-auto pointer-events-auto ${isMobile ? 'max-w-md' : 'max-w-[1500px]'}`}>
          <div className="relative rounded-xl px-4 md:px-6 py-1.5 md:py-2"
            style={{
              background: 'linear-gradient(135deg, rgba(90,40,15,0.85), rgba(60,25,10,0.9))',
