...
import { 
  SPANISH_PLACES_ITEMS, 
  SPANISH_PLACES_GROUPS, 
  generateSpanishPlacesChoices,
  generatePlacesSentenceWordPool,
  checkPlacesSentenceOrder,
  type SpanishPlacesItem
} from '@/data/spanish-4/places';
...
const LESSON_ID = 'es-mezquita-places';
...
function getCategoryStyle(category: 'Lugar' | 'Adjetivo' | 'Ubicacion') {
  switch (category) {
    case 'Lugar':
      return { bg: 'rgba(59,130,246,0.7)', border: '#3B82F6', icon: '🏛️', label: 'أماكن' };
    case 'Adjetivo':
      return { bg: 'rgba(236,72,153,0.7)', border: '#EC4899', icon: '✨', label: 'صفات' };
    case 'Ubicacion':
      return { bg: 'rgba(234,179,8,0.7)', border: '#EAB308', icon: '📍', label: 'موقع' };
  }
}

// ═══════════════════════════════════════
// 🕌 Rotating Dome - قبة إسلامية دوّارة
// ═══════════════════════════════════════
function RotatingDome({ size = 32 }: { size?: number }) {
  return (
    <motion.div
      animate={{ 
        rotate: [0, 5, -5, 0],
      ...
      style={{ 
        ...
        fontSize: size,
        lineHeight: 1,
        filter: 'drop-shadow(0 4px 12px rgba(185,28,28,0.6))',
      }}
    >
      🕌
    </motion.div>
...
// ═══════════════════════════════════════
// 🌙 Crescent Moon - هلال متلألئ
// ═══════════════════════════════════════
function CrescentMoon({ size = 24 }: { size?: number }) {
  return (
    <motion.div
      animate={{ 
        rotate: [0, 360],
    ...
    >
      ...
      >
        ...
        <div 
          ...
          style={{
            backgroundImage: `
              repeating-linear-gradient(90deg, transparent 0px, transparent 20px, rgba(185,28,28,0.4) 20px, rgba(185,28,28,0.4) 22px, transparent 22px, transparent 25px, rgba(254,243,199,0.4) 25px, rgba(254,243,199,0.4) 27px)
            `,
          }}
        />
        ...
        <div className="absolute top-3 left-1/2 -translate-x-1/2 z-20 flex flex-col items-center gap-1">
          ...
          >
            <MapPin size={12} className="text-yellow-300" /> الأماكن
          </div>
...
function SpanishPlacesLessonInner() {
  ...
  const [itemChoices, setItemChoices] = useState<SpanishPlacesItem[]>([]);
  ...
  const group = SPANISH_PLACES_GROUPS[groupIdx];
  ...
  useEffect(() => {
    if (item && phase === 'listen') {
      setItemChoices(generateSpanishPlacesChoices(item.word, 3));
  ...
  useEffect(() => {
    if (item && phase === 'build') {
      setAvailableWords(generatePlacesSentenceWordPool(item));
      setSelectedWords([]);
      setStatus('idle');
  ...
  const handleItemChoice = (choice: SpanishPlacesItem, e: React.MouseEvent) => {
  ...
  const handleSelectWord = (word: string, idx: number) => {
    ...
    if (newSelected.length === item.sentenceWords.length) {
      const isCorrect = checkPlacesSentenceOrder(newSelected, item.sentenceWords);
      if (isCorrect) {
        setStatus('correct');
        setTimeout(() => {
    ...
    <div className="relative min-h-screen overflow-hidden" 
      ...
      <div className="fixed inset-0 pointer-events-none" style={{ zIndex: 0 }}>
        ...
        <div className="absolute inset-0" style={{
          background: isMobile 
            ? 'linear-gradient(180deg, rgba(40,10,10,0.8) 0%, rgba(30,5,5,0.92) 100%)'
            : 'radial-gradient(ellipse at 20% 20%, rgba(90,15,15,0.9) 0%, rgba(60,10,10,0.94) 50%, rgba(30,5,5,0.98) 100%)',
        }} />
        <motion.div
            ...
            {phase !== 'group-success' && phase !== 'all-done' && (
              <div className="flex justify-center mt-1.5">
                <div className="flex items-center gap-1 px-2 py-1 rounded-lg bg-black/50 backdrop-blur-md border border-white/15">
                  <Stepper currentStep={itemIdx} totalSteps={group?.items.length || 5} isMobile color={item?.color || '#B91C1C'} />
                </div>
              </div>
            )}
          </>
          ...
          <div className="max-w-[1600px] mx-auto flex items-center justify-between gap-4">
            ...
            {phase !== 'group-success' && phase !== 'all-done' && (
              <div className="flex items-center gap-1 px-4 py-2 rounded-2xl bg-black/50 backdrop-blur-xl border-2 border-white/15">
                <Stepper currentStep={itemIdx} totalSteps={group?.items.length || 5} isMobile={false} color={item?.color || '#B91C1C'} />
              </div>
            )}
                ...
                <div className="flex items-center gap-2">
                  <div className="w-20 h-2 bg-white/15 rounded-full overflow-hidden border border-white/20">
                    <motion.div className="h-full rounded-full" 
                      style={{ background: 'linear-gradient(to right, #B91C1C, #FCD34D)' }}
        ...
        <AnimatePresence mode="wait">
          ...
          {phase === 'listen' && item && (
            <motion.div key={`listen-${groupIdx}-${itemIdx}`}
              ...
              <div className={`flex ${isMobile ? 'flex-col' : 'flex-row'} items-center justify-center gap-6 md:gap-10`}>
                ...
                <PlacesCard item={item} size={isMobile ? 200 : 280} mode="word" />
          ...
          {phase === 'build' && item && (
            <motion.div key={`build-${groupIdx}-${itemIdx}`}
              ...
              <div className={`flex ${isMobile ? 'flex-col' : 'flex-row'} items-center justify-center gap-6 md:gap-10`}>
                ...
                <PlacesCard item={item} size={isMobile ? 220 : 280} mode="sentence" />
                ...
                <div className={`flex flex-col items-center gap-3 ${isMobile ? 'w-full max-w-sm' : 'flex-1 max-w-md'}`}>
                  ...
                  {status === 'correct' && (
                    <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }}
                      ...
                      <span className="font-black text-white text-sm">¡Precioso!</span>
                    </motion.div>
                  )}
                </div>
              </div>
          ...
          {phase === 'group-success' && (
            <motion.div key="success" initial={{ scale: 0.5, opacity: 0 }} animate={{ scale: 1, opacity: 1 }}
              ...
              <h2 className="text-3xl md:text-5xl font-black text-white">¡Un lugar increíble!</h2>
              ...
              <button 
                onClick={() => {
                  if (groupIdx < SPANISH_PLACES_GROUPS.length - 1) {
                    setGroupIdx(g => g + 1); 
                    setItemIdx(0); 
                    setPhase('listen');
                    setCorrectInGroup(0);
                ...
                style={{ background: 'linear-gradient(135deg, #B91C1C, #FCD34D)', boxShadow: '0 10px 30px rgba(185,28,28,0.4)' }}
              >
                المجموعة التالية 🚀
              </button>
            </motion.div>
          ...
          {phase === 'all-done' && (
            <motion.div key="done" initial={{ opacity: 0 }} animate={{ opacity: 1 }}
              ...
              <h1 className="text-3xl md:text-5xl font-black text-white">¡Mezquita completa!</h1>
              <p className="text-lg md:text-xl text-white/60 max-w-md mx-auto">تعلمت 15 كلمة عن الأماكن والصفات + 15 جملة كاملة!</p>
              <p className="text-sm text-red-300/80 max-w-md mx-auto">
                دلوقتي تقدر توصف أي مكان بالأسباني! 🕌🏛️
              </p>
              ...
              <button 
                ...
                style={{ 
                  background: 'linear-gradient(135deg, #B91C1C, #FCD34D)', 
                  boxShadow: '0 15px 40px rgba(185,28,28,0.5)',
                  borderBottom: '5px solid #7F1D1D'
                }}
              >
                العودة للخريطة 🗺️
              </button>
      ...
      <div className="fixed bottom-0 left-0 right-0 z-30 px-2 md:px-4 pb-1.5 pointer-events-none"
        ...
        <div className={`mx-auto pointer-events-auto ${isMobile ? 'max-w-md' : 'max-w-[1500px]'}`}>
          <div className="relative rounded-xl px-4 md:px-6 py-1.5 md:py-2"
            style={{
              background: 'linear-gradient(135deg, rgba(90,15,15,0.85), rgba(60,10,10,0.9))',
              backdropFilter: 'blur(30px)',
              border: '1.5px solid rgba(255,255,255,0.2)',
              boxShadow: `0 10px 30px rgba(0,0,0,0.5), 0 0 25px rgba(185,28,28,0.2)`,
            }}>
      ...
      <ToroBull mood={toroMood} message={toroMessage} idleGlowColor={item?.color || '#B91C1C'} />
      <ConfettiBurst trigger={confettiTrigger} x={confettiPos.x} y={confettiPos.y} 
        colors={[item?.color || '#B91C1C', '#FCD34D', '#ffffff']} />
    </div>
  );
}

export default function SpanishPlacesLessonPage() {
  return (
    <Suspense fallback={<div className="min-h-screen bg-[#07090D] flex items-center justify-center text-white">Loading...</div>}>
      <SpanishPlacesLessonInner />
    </Suspense>
  );
}