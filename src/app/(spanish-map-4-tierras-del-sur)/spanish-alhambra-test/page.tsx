...
} from '@/data/spanish-4/alhambra-test';
...
const LESSON_ID = 'es-alhambra-test';
...
// 🏰 AnimatedAlhambra Component - قصر الحمراء متوهج
function AnimatedAlhambra({ size = 200 }: { size?: number }) {
  return (
    <motion.div
    ...
    >
      ...
      <motion.div
        ...
        style={{
          width: size * 1.6,
          height: size * 1.2,
          background: 'radial-gradient(circle, rgba(234,179,8,0.8), transparent 70%)',
        }}
        animate={{ 
          opacity: [0.5, 0.9, 0.5],
          scale: [1, 1.15, 1],
      ...
      <motion.div
        ...
        style={{
          width: '4px',
          height: size * 0.7,
          background: 'linear-gradient(to bottom, rgba(234,179,8,0.9), rgba(252,211,77,0.4), transparent)',
          filter: 'blur(2px)',
        }}
        animate={{
          opacity: [0.6, 1, 0.6],
      ...
      {/* زخارف نجمية حواليها */}
      {[
        { top: '15%', left: '15%', delay: 0, emoji: '⭐' },
        { top: '20%', right: '15%', delay: 0.5, emoji: '✨' },
        { top: '45%', left: '5%', delay: 1, emoji: '⭐' },
        { top: '40%', right: '5%', delay: 1.5, emoji: '✨' },
        { top: '65%', left: '15%', delay: 0.3, emoji: '💫' },
      ].map((star, i) => (
        <motion.div
          key={i}
          className="absolute text-yellow-300"
          style={{ ...star, fontSize: '18px' }}
          animate={{ 
            opacity: [0.3, 1, 0.3],
            scale: [0.8, 1.3, 0.8],
            rotate: [0, 360],
      ...
      <motion.div
        ...
        style={{ 
          fontSize: size * 0.95,
          filter: 'drop-shadow(0 15px 40px rgba(234,179,8,0.6))',
          lineHeight: 1,
        }}
      >
        🏰
      ...
      <motion.div
        ...
        className="absolute bottom-2 left-1/2 -translate-x-1/2 flex gap-2 text-2xl opacity-80"
      >
        <span>🦁</span>
        <span>👑</span>
        <span>🦁</span>
    ...
    <div className="w-full max-w-md mx-auto">
      ...
      <div className="w-full h-2.5 md:h-3 bg-black/40 rounded-full overflow-hidden border border-white/20">
        <motion.div 
          ...
          style={{ background: 'linear-gradient(to right, #FDE047, #EAB308, #A16207)' }}
          initial={{ width: 0 }}
...
function SpanishAlhambraTestInner() {
    ...
    <div className="relative min-h-screen overflow-hidden" 
      ...
      <div className="fixed inset-0 pointer-events-none" style={{ zIndex: 0 }}>
        ...
        <div className="absolute inset-0" style={{
          background: isMobile 
            ? 'linear-gradient(180deg, rgba(60,35,10,0.88) 0%, rgba(35,20,5,0.95) 100%)'
            : 'radial-gradient(ellipse at 20% 20%, rgba(100,60,15,0.9) 0%, rgba(70,40,10,0.94) 50%, rgba(35,20,5,0.98) 100%)',
        }} />
        <motion.div
          className="absolute inset-0 opacity-30"
          style={{ background: `radial-gradient(ellipse 100% 60% at 50% 30%, rgba(234,179,8,0.5), transparent 70%)` }}
        ...
        <AnimatePresence mode="wait">
          ...
          {phase === 'intro' && (
            <motion.div key="intro"
              ...
              <AnimatedAlhambra size={isMobile ? 150 : 200} />
              ...
              <p className="text-sm md:text-base text-white/70 max-w-md mx-auto">
                🏰 الاختبار بيغطي كل اللي اتعلمته في أراضي الجنوب: الأفعال، الطبيعة، الوصفات، التواصل، والأماكن!
              </p>
                ...
                return (
                  <>
                    <motion.div
                      ...
                      transition={{ type: 'spring', duration: 1 }}
                    >
                      <AnimatedAlhambra size={isMobile ? 130 : 180} />
                    </motion.div>
                    ...
                    {isExcellent && (
                      ...
                      >
                        ...
                        <p className="text-xs md:text-sm text-white">
                          أنهيت <span className="font-black text-yellow-300">أراضي الجنوب</span> بنجاح! جاهز للمرحلة الأخيرة: الجزر والعاصمة ✈️🏝️
                        </p>
                      </motion.div>
                    )}
      ...
      {phase === 'question' && (
        <ToroBull mood={toroMood} message={toroMessage} idleGlowColor={currentQuestion?.color || '#EAB308'} />
      )}
      
      <ConfettiBurst trigger={confettiTrigger} x={confettiPos.x} y={confettiPos.y} 
        colors={['#EAB308', '#FDE047', '#FCD34D', '#B91C1C', '#ffffff']} />
    </div>
  );
}

export default function SpanishAlhambraTestPage() {
  return (
    <Suspense fallback={<div className="min-h-screen bg-[#07090D] flex items-center justify-center text-white">Loading...</div>}>
      <SpanishAlhambraTestInner />
    </Suspense>
  );
}