...
import { QUESTIONS, QUESTION_GROUPS, type QuestionItem } from '@/data/german/map-5-culture/goethe-questions';
...
const TOTAL_ANSWERS_PER_LESSON = TOTAL_QUESTIONS * 3;
const LESSON_ID = 'goethe-house';
...
const DARK_COLORS: Record<string, string> = {
  ...
  '#06D6A0': '#064E3B', '#FBBF24': '#7D5310', '#EC4899': '#831843',
  '#3B82F6': '#1E3A8A', '#F77F00': '#9A3412', '#A78BFA': '#5B21B6',
  '#10B981': '#064E3B', '#0EA5E9': '#075985', '#F472B6': '#9D174D',
...
function checkAnswer(input: string, item: QuestionItem): boolean {
...
function generateChoices(correctId: string, all: QuestionItem[], count = 3): QuestionItem[] {
...
function getGroupBackground(groupIdx: number, isMobile: boolean): string {
  ...
  return `/card-image/questions/bg-group${Math.min(groupIdx + 1, 3)}-${suffix}.webp`;
}
...
// 🆕 Answer Card - مميز لكلمات السؤال
function AnswerCard({ item, isMobile }: { item: QuestionItem; isMobile?: boolean }) {
  ...
  return (
    <motion.div 
      ...
      className={`${isMobile ? 'p-2.5' : 'p-4'} rounded-2xl border-2 w-full`}
      style={{
        background: `linear-gradient(135deg, ${item.color}22, ${item.color}11)`,
        borderColor: `${item.color}66`,
      }}
    >
      <div className={`text-center font-black ${isMobile ? 'text-[10px]' : 'text-xs'} mb-1.5`} style={{ color: item.color }}>
        💡 الإجابة النموذجية
      </div>
      <div className="text-center" dir="ltr">
        <div className={`font-black text-white ${isMobile ? 'text-sm' : 'text-base'}`} style={{ textShadow: `0 1px 4px ${item.color}` }}>
          {item.answer.de}
        </div>
        <div className={`font-bold text-white/70 ${isMobile ? 'text-[10px]' : 'text-xs'} mt-0.5`} dir="rtl">
          {item.answer.ar}
        </div>
      </div>
    </motion.div>
...
function HeroDisplay({ item, isMobile, showWord = false }: { item: QuestionItem; isMobile?: boolean; showWord?: boolean }) {
...
function ChoiceCard({ item, allItems, isMobile, onCorrect, onWrong }: {
  item: QuestionItem; allItems: QuestionItem[]; isMobile: boolean;
  onCorrect: (cx: number, cy: number) => void; onWrong: () => void;
}) {
  const [choices, setChoices] = useState<QuestionItem[]>([]);
  ...
  const handleChoice = (c: QuestionItem, e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    <div className={`w-full ${isMobile ? 'max-w-md' : 'max-w-3xl'} mx-auto ${isMobile ? 'p-3' : 'p-6'} rounded-[1.5rem] md:rounded-[2rem] relative overflow-hidden`}
      ...
      <div className={`relative z-10 flex flex-col items-center ${isMobile ? 'gap-2.5' : 'gap-4'}`}>
        <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}
          ...
          <span className={`font-black ${isMobile ? 'text-xs' : 'text-base'} text-gray-800`}>استمع جيداً واختر السؤال الصحيح</span>
        </motion.div>
        ...
        <AnimatePresence>
          {status === 'correct' && item.answer && (
            <AnswerCard item={item} isMobile={isMobile} />
          )}
        </AnimatePresence>
        <AnimatePresence>
  ...
  item: QuestionItem; onComplete: (cx: number, cy: number) => void; onWrong: () => void;
}) {
        ...
        {isMobile ? (
          ...
          <div className="grid lg:grid-cols-5 gap-8 items-center">
            <div className="lg:col-span-3 flex flex-col items-center gap-4">
              ...
              <SoundButton onClick={() => speakGerman(item.de)} color={item.color} label="استمع للسؤال" />
              {item.answer && <AnswerCard item={item} />}
            </div>
            <div className="lg:col-span-2 space-y-4">
              <div className="text-center lg:text-right">
                ...
                <div className="text-2xl font-black text-white">اكتب السؤال</div>
            ...
            <div className={`flex flex-col items-center gap-${isMobile ? '2' : '4'} w-full ${isMobile ? '' : 'relative z-10'}`}>
              ...
              <div className="text-center">
                <h3 className={`font-black text-white flex items-center justify-center gap-1.5 ${isMobile ? 'text-base' : 'text-2xl'}`}>
                  انطق السؤال <motion.span animate={{ scale: [1, 1.2, 1] }} transition={{ duration: 1.5, repeat: Infinity }}>🎤</motion.span>
                </h3>
              </div>
...
function MatchGame({ group, onComplete, onCorrect, onKarlReact, onCombo }: any) {
  ...
  const [shuffledWords, setShuffledWords] = useState<QuestionItem[]>(() => shuffle(group));
  ...
  const tryMatch = (src: DragSource, tgt: DragSource, cx: number, cy: number) => {
    ...
    if (src.id === tgt.id) {
      const it = group.find((x: QuestionItem) => x.id === src.id)!;
  ...
  const renderCard = (item: QuestionItem, side: 'image' | 'word') => {
  ...
  return (
    <>
      <ConfettiBurst trigger={confettiTrigger} x={confettiPos.x} y={confettiPos.y} colors={['#FFD700', '#F77F00', '#EC4899', '#FFF']} />
      <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }}
        ...
        <div className="flex items-center gap-3 w-full max-w-md px-2">
          <div className="px-3 py-1 rounded-full flex items-center gap-1.5"
            style={{ background: 'linear-gradient(135deg,rgba(247,127,0,0.4),rgba(236,72,153,0.4))', border: '1.5px solid rgba(247,127,0,0.5)' }}>
            <Sparkles size={11} className="text-yellow-300" />
            <span className="text-[10px] font-black text-white">طابق السؤال بالرمز</span>
          </div>
          <div className="flex-1 flex items-center gap-1.5">
            <div className="flex-1 h-1.5 bg-white/10 rounded-full overflow-hidden">
              <motion.div className="h-full rounded-full" style={{ background: 'linear-gradient(to right,#58CC02,#F77F00,#EC4899)' }}
        ...
        <div className="w-full flex flex-col items-center gap-1">
          ...
          <div className="flex items-center justify-center gap-1.5 flex-wrap" dir="ltr">{group.map((c: QuestionItem) => renderCard(c, 'image'))}</div>
        </div>
        ...
        <div className="w-full flex flex-col items-center gap-1">
          <span className="text-[9px] text-pink-300/80 font-black uppercase">الأسئلة — اسحب</span>
...
function GermanQuestionsLessonInner() {
  ...
  const currentGroup = QUESTION_GROUPS[groupIdx];
  ...
  const calcRating = (s: number) => { const r = s / (QUESTIONS.length * 3); return r >= .67 ? 3 : r >= .34 ? 2 : 1; };
  ...
  const nextGroup = async () => {
    if (groupIdx < QUESTION_GROUPS.length - 1) {
    ...
    <div className="text-white relative" style={{ fontFamily: "'Tajawal',sans-serif", minHeight: '100vh' }} dir="rtl">
      ...
      {!(isMobile && isKeyboardOpen) && (
        <div style={{ transform: isMobile ? 'scale(0.4)' : 'scale(0.55)', transformOrigin: 'bottom right', position: 'fixed', bottom: isMobile ? 110 : 130, right: 0, zIndex: 25, pointerEvents: 'none' }}>
          <KarlEagle mood={karlMood} message={karlMessage} idleGlowColor="#F77F00" />
        </div>
      )}
      ...
      <div className="flex flex-col items-center justify-center relative px-3 md:px-6 mx-auto w-full"
        ...
        <AnimatePresence mode="wait">
          {phase === 'listen' && <ListenPhase key={`l-${groupIdx}-${itemIdx}`} item={currentItem} allItems={QUESTIONS} onDone={handleListenDone} onKarlReact={handleKarlReact} onCombo={handleCombo} onCorrect={handleCorrect} isMobile={isMobile} />}
          ...
          {testSuccess && (
            <motion.div key="success" initial={{ opacity: 0, scale: .85 }} animate={{ opacity: 1, scale: 1 }}
              ...
              <motion.button whileHover={{ scale: 1.04 }} whileTap={{ scale: .97 }} onClick={nextGroup}
                ...
                style={{ background: 'linear-gradient(135deg,#F77F00,#EC4899)', boxShadow: '0 10px 40px rgba(247,127,0,0.5)' }}>
                {groupIdx < QUESTION_GROUPS.length - 1 ? 'المجموعة التالية ←' : '🗺️ رجوع للخريطة'}
              </motion.button>
            </motion.div>
          )}
        </AnimatePresence>
...
export default function GermanQuestionsLesson() {
  return (
