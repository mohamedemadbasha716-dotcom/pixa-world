...
import { 
  SPANISH_FEELING_ITEMS, 
  SPANISH_FEELING_GROUPS, 
  generateSpanishFeelingChoices,
  generateFeelingSentenceWordPool,
  checkFeelingSentenceOrder,
  type SpanishFeelingItem
} from '@/data/spanish-2/feelings';
...
const LESSON_ID = 'es-greco-feelings';
...
function getCategoryStyle(category: 'Positiva' | 'Dificil') {
  switch (category) {
    case 'Positiva':
      return { bg: 'rgba(34,197,94,0.7)', border: '#22C55E', icon: '🌞', label: 'مريحة' };
    case 'Dificil':
      return { bg: 'rgba(99,102,241,0.7)', border: '#6366F1', icon: '🌧️', label: 'صعبة' };
  }
}

// ═══════════════════════════════════════
// 💖 Feeling Card - بطاقة المشاعر
// ═══════════════════════════════════════
    ...
    >
      ...
      >
        ...
        <div className="absolute top-3 left-1/2 -translate-x-1/2 z-20 flex flex-col items-center gap-1">
          ...
          >
            <Heart size={12} className="text-white" fill="white" /> المشاعر
          </div>
...
function SpanishFeelingsLessonInner() {
  ...
  const [itemChoices, setItemChoices] = useState<SpanishFeelingItem[]>([]);
  ...
  const group = SPANISH_FEELING_GROUPS[groupIdx];
  ...
  useEffect(() => {
    if (item && phase === 'listen') {
      setItemChoices(generateSpanishFeelingChoices(item.word, 3));
  ...
  useEffect(() => {
    if (item && phase === 'build') {
      setAvailableWords(generateFeelingSentenceWordPool(item));
      setSelectedWords([]);
      setStatus('idle');
  ...
  const handleItemChoice = (choice: SpanishFeelingItem, e: React.MouseEvent) => {
  ...
  const handleSelectWord = (word: string, idx: number) => {
    ...
    if (newSelected.length === item.sentenceWords.length) {
      const isCorrect = checkFeelingSentenceOrder(newSelected, item.sentenceWords);
      if (isCorrect) {
        setStatus('correct');
        setTimeout(() => {
    ...
    <div className="relative min-h-screen overflow-hidden" 
      ...
      <div className="fixed inset-0 pointer-events-none" style={{ zIndex: 0 }}>
        ...
        <div className="absolute inset-0" style={{
          background: isMobile 
            ? 'linear-gradient(180deg, rgba(40,15,40,0.7) 0%, rgba(30,10,35,0.85) 100%)'
            : 'radial-gradient(ellipse at 20% 20%, rgba(90,25,80,0.85) 0%, rgba(60,15,50,0.92) 50%, rgba(30,10,35,0.95) 100%)',
        }} />
        <motion.div
            ...
            {phase !== 'group-success' && phase !== 'all-done' && (
              <div className="flex justify-center mt-1.5">
                <div className="flex items-center gap-1 px-2 py-1 rounded-lg bg-black/50 backdrop-blur-md border border-white/15">
                  <Stepper currentStep={itemIdx} totalSteps={group?.items.length || 4} isMobile color={item?.color || '#EC4899'} />
                </div>
              </div>
            )}
          </>
          ...
          <div className="max-w-[1600px] mx-auto flex items-center justify-between gap-4">
            ...
            {phase !== 'group-success' && phase !== 'all-done' && (
              <div className="flex items-center gap-1 px-4 py-2 rounded-2xl bg-black/50 backdrop-blur-xl border-2 border-white/15">
                <Stepper currentStep={itemIdx} totalSteps={group?.items.length || 4} isMobile={false} color={item?.color || '#EC4899'} />
              </div>
            )}
                ...
                <div className="flex items-center gap-2">
                  <div className="w-20 h-2 bg-white/15 rounded-full overflow-hidden border border-white/20">
                    <motion.div className="h-full rounded-full" 
                      ...
                      animate={{ width: `${(itemIdx / (group?.items.length || 4)) * 100}%` }} />
                  </div>
        ...
        <AnimatePresence mode="wait">
          ...
          {phase === 'listen' && item && (
            <motion.div key={`listen-${groupIdx}-${itemIdx}`}
              ...
              <div className={`flex ${isMobile ? 'flex-col' : 'flex-row'} items-center justify-center gap-6 md:gap-10`}>
                ...
                <FeelingCard item={item} size={isMobile ? 200 : 280} mode="word" />
                ...
                <div className={`flex flex-col items-center gap-4 ${isMobile ? 'w-full max-w-sm' : 'flex-1 max-w-md'}`}>
                  ...
                  <div className="px-5 py-2 rounded-2xl backdrop-blur-md border-2"
                    ...
                    <span className="font-black text-white text-sm md:text-base">🎧 استمع واختر الشعور الصحيح</span>
                  </div>
          ...
          {phase === 'build' && item && (
            <motion.div key={`build-${groupIdx}-${itemIdx}`}
              ...
              <div className={`flex ${isMobile ? 'flex-col' : 'flex-row'} items-center justify-center gap-6 md:gap-10`}>
                ...
                <FeelingCard item={item} size={isMobile ? 220 : 280} mode="sentence" />
          ...
          {phase === 'group-success' && (
            <motion.div key="success" initial={{ scale: 0.5, opacity: 0 }} animate={{ scale: 1, opacity: 1 }}
              ...
              <h2 className="text-3xl md:text-5xl font-black text-white">¡Muy bien sentido!</h2>
              ...
              <button 
                onClick={() => {
                  if (groupIdx < SPANISH_FEELING_GROUPS.length - 1) {
                    setGroupIdx(g => g + 1); 
                    setItemIdx(0); 
                    setPhase('listen');
                    setCorrectInGroup(0);
          ...
          {phase === 'all-done' && (
            <motion.div key="done" initial={{ opacity: 0 }} animate={{ opacity: 1 }}
              ...
              <h1 className="text-3xl md:text-5xl font-black text-white">¡Museo del Greco completo!</h1>
              <p className="text-lg md:text-xl text-white/60 max-w-md mx-auto">تعلمت 8 مشاعر + 8 جمل كاملة!</p>
              <p className="text-sm text-pink-300/80 max-w-md mx-auto">
                دلوقتي تقدر تعبر عن مشاعرك بالأسباني! 💖
              </p>
      ...
      <div className="fixed bottom-0 left-0 right-0 z-30 px-2 md:px-4 pb-1.5 pointer-events-none"
        ...
        <div className={`mx-auto pointer-events-auto ${isMobile ? 'max-w-md' : 'max-w-[1500px]'}`}>
          <div className="relative rounded-xl px-4 md:px-6 py-1.5 md:py-2"
            style={{
              background: 'linear-gradient(135deg, rgba(70,20,60,0.85), rgba(50,15,45,0.9))',
