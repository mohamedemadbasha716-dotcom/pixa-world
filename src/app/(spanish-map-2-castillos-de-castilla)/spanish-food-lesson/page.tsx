...
import { 
  ...
  UtensilsCrossed, X, BookOpen
} from 'lucide-react';

import { 
  SPANISH_FOODS, 
  SPANISH_FOOD_GROUPS, 
  generateSpanishFoodChoices,
  generateFoodSentenceWordPool,
  checkFoodSentenceOrder,
  type SpanishFoodItem
} from '@/data/spanish-2/food';
...
const LESSON_ID = 'es-candido-food';
...
function getCategoryStyle(category: 'Principal' | 'Postre' | 'Bebida') {
  switch (category) {
    case 'Principal':
      return { bg: 'rgba(220,38,38,0.7)', border: '#DC2626', icon: '🍖', label: 'طبق رئيسي' };
    case 'Postre':
      return { bg: 'rgba(236,72,153,0.7)', border: '#EC4899', icon: '🍰', label: 'حلويات' };
    case 'Bebida':
      return { bg: 'rgba(14,165,233,0.7)', border: '#0EA5E9', icon: '🥤', label: 'مشروبات' };
  }
}

// 🎨 Helper: لون شارة الفعل
function getVerbStyle(verb: 'comer' | 'beber') {
  switch (verb) {
    case 'comer':
      return { bg: 'rgba(249,115,22,0.5)', icon: '🍴', label: 'يأكل' };
    case 'beber':
      return { bg: 'rgba(14,165,233,0.5)', icon: '🥤', label: 'يشرب' };
  }
}

// ═══════════════════════════════════════
// 🍽️ Food Card - بطاقة مطعم فخمة
// ═══════════════════════════════════════
function FoodCard({ 
  item, 
  size = 280, 
  ...
  const verbStyle = getVerbStyle(item.verb);
    ...
    >
      ...
      >
        ...
        <div className="absolute top-3 left-1/2 -translate-x-1/2 z-20 flex flex-col items-center gap-1">
          ...
          >
            <UtensilsCrossed size={12} className="text-amber-800" /> الطعام
          </div>
        ...
        <div className="flex-1 flex flex-col items-center justify-center w-full pt-20 pb-2 gap-2">
          ...
          <div className="flex gap-1.5">
            ...
            >
              {verbStyle.icon} {verbStyle.label}
            </div>
          </div>
        </div>
...
function SpanishFoodLessonInner() {
  ...
  const [itemChoices, setItemChoices] = useState<SpanishFoodItem[]>([]);
  ...
  const group = SPANISH_FOOD_GROUPS[groupIdx];
  ...
  useEffect(() => {
    if (item && phase === 'listen') {
      setItemChoices(generateSpanishFoodChoices(item.word, 3));
  ...
  useEffect(() => {
    if (item && phase === 'build') {
      setAvailableWords(generateFoodSentenceWordPool(item));
      setSelectedWords([]);
      setStatus('idle');
  ...
  const handleItemChoice = (choice: SpanishFoodItem, e: React.MouseEvent) => {
  ...
  const handleSelectWord = (word: string, idx: number) => {
    ...
    if (newSelected.length === item.sentenceWords.length) {
      const isCorrect = checkFoodSentenceOrder(newSelected, item.sentenceWords);
      if (isCorrect) {
        setStatus('correct');
        setTimeout(() => {
    ...
    <div className="relative min-h-screen overflow-hidden" 
      ...
      <div className="fixed inset-0 pointer-events-none" style={{ zIndex: 0 }}>
        ...
        <div className="absolute inset-0" style={{
          background: isMobile 
            ? 'linear-gradient(180deg, rgba(40,15,10,0.7) 0%, rgba(40,15,10,0.85) 100%)'
            : 'radial-gradient(ellipse at 20% 20%, rgba(90,40,20,0.85) 0%, rgba(60,25,15,0.92) 50%, rgba(30,15,10,0.95) 100%)',
        }} />
        <motion.div
        ...
        <AnimatePresence mode="wait">
          ...
          {phase === 'listen' && item && (
            <motion.div key={`listen-${groupIdx}-${itemIdx}`}
              ...
              <div className={`flex ${isMobile ? 'flex-col' : 'flex-row'} items-center justify-center gap-6 md:gap-10`}>
                ...
                <FoodCard item={item} size={isMobile ? 200 : 280} mode="word" />
          ...
          {phase === 'build' && item && (
            <motion.div key={`build-${groupIdx}-${itemIdx}`}
              ...
              <div className={`flex ${isMobile ? 'flex-col' : 'flex-row'} items-center justify-center gap-6 md:gap-10`}>
                ...
                <FoodCard item={item} size={isMobile ? 220 : 280} mode="sentence" />
          ...
          {phase === 'group-success' && (
            <motion.div key="success" initial={{ scale: 0.5, opacity: 0 }} animate={{ scale: 1, opacity: 1 }}
              ...
              <h2 className="text-3xl md:text-5xl font-black text-white">¡Buen provecho!</h2>
              ...
              <button 
                onClick={() => {
                  if (groupIdx < SPANISH_FOOD_GROUPS.length - 1) {
                    setGroupIdx(g => g + 1); 
                    setItemIdx(0); 
                    setPhase('listen');
                    setCorrectInGroup(0);
              ...
              <div className="flex flex-wrap justify-center gap-2 max-w-2xl mx-auto mb-4">
                {SPANISH_FOODS.map((f, i) => (
                  ...
                  >
                    ...
                    <span className="text-xs font-black text-white" dir="ltr">{f.word}</span>
                  </motion.div>
                ))}
              </div>

              <div className="text-7xl md:text-8xl">🍖</div>
              <h1 className="text-3xl md:text-5xl font-black text-white">¡Mesón completo!</h1>
              <p className="text-lg md:text-xl text-white/60 max-w-md mx-auto">تعلمت 15 صنف طعام + 15 جملة كاملة!</p>
              <p className="text-sm text-yellow-300/80 max-w-md mx-auto">
                دلوقتي تقدر تطلب أكل في أي مطعم إسباني! 🎉
              </p>
      ...
      <div className="fixed bottom-0 left-0 right-0 z-30 px-2 md:px-4 pb-1.5 pointer-events-none"
        ...
        <div className={`mx-auto pointer-events-auto ${isMobile ? 'max-w-md' : 'max-w-[1500px]'}`}>
          <div className="relative rounded-xl px-4 md:px-6 py-1.5 md:py-2"
            style={{
              background: 'linear-gradient(135deg, rgba(60,25,15,0.85), rgba(40,15,10,0.9))',
