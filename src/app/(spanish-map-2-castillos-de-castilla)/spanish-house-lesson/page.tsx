...
import { 
  SPANISH_HOUSE_ITEMS, 
  SPANISH_HOUSE_GROUPS, 
  generateSpanishHouseChoices,
  generateHouseSentenceWordPool,
  checkHouseSentenceOrder,
  type SpanishHouseItem
} from '@/data/spanish-2/house';
...
const LESSON_ID = 'es-cuenca-house';
...
function getCategoryStyle(category: 'Habitacion' | 'Mueble' | 'Parte') {
  switch (category) {
    case 'Habitacion':
      return { bg: 'rgba(161,98,7,0.7)', border: '#A16207', icon: '🏠', label: 'الغرف' };
    case 'Mueble':
      return { bg: 'rgba(146,64,14,0.7)', border: '#92400E', icon: '🪑', label: 'الأثاث' };
    case 'Parte':
      return { bg: 'rgba(22,163,74,0.7)', border: '#16A34A', icon: '🌳', label: 'أجزاء أخرى' };
  }
}

// ═══════════════════════════════════════
// 🏚️ House Card - بطاقة البيوت المعلقة
// ═══════════════════════════════════════
function HouseCard({ 
  item, 
  size = 280, 
  ...
  return (
    ...
    >
      ...
      >
        {/* ديكور: البيوت المعلقة */}
        <div className="absolute top-2 right-2 text-2xl opacity-20 select-none pointer-events-none">🏚️</div>
        <div className="absolute bottom-16 left-2 text-xl opacity-15 select-none pointer-events-none">⛰️</div>
        ...
        <div className="absolute top-3 left-1/2 -translate-x-1/2 z-20 flex flex-col items-center gap-1">
          ...
          >
            <House size={12} className="text-amber-800" /> المنزل
          </div>
        ...
        <div className="flex-1 flex flex-col items-center justify-center w-full pt-20 pb-2 gap-2">
          ...
          <div className="flex gap-1.5">
            <div 
              ...
              style={{
                background: item.gender === 'M' ? 'rgba(59,130,246,0.5)' : 'rgba(236,72,153,0.5)',
                borderColor: 'rgba(255,255,255,0.3)',
              }}
            >
              {item.gender === 'M' ? `♂ ${item.article}` : `♀ ${item.article}`}
            </div>
            <div 
              className="px-2.5 py-0.5 rounded-full text-[10px] font-bold text-white backdrop-blur-md border flex items-center gap-1"
              style={{
            ...
            >
              {item.indefiniteArticle === 'un' ? '1️⃣ un' : '1️⃣ una'}
            </div>
          </div>
        </div>
...
function SpanishHouseLessonInner() {
  ...
  const [itemChoices, setItemChoices] = useState<SpanishHouseItem[]>([]);
  ...
  const group = SPANISH_HOUSE_GROUPS[groupIdx];
  ...
  useEffect(() => {
    if (item && phase === 'listen') {
      setItemChoices(generateSpanishHouseChoices(item.word, 3));
  ...
  useEffect(() => {
    if (item && phase === 'build') {
      setAvailableWords(generateHouseSentenceWordPool(item));
      setSelectedWords([]);
      setStatus('idle');
  ...
  const handleItemChoice = (choice: SpanishHouseItem, e: React.MouseEvent) => {
  ...
  const handleSelectWord = (word: string, idx: number) => {
    ...
    if (newSelected.length === item.sentenceWords.length) {
      const isCorrect = checkHouseSentenceOrder(newSelected, item.sentenceWords);
      if (isCorrect) {
        setStatus('correct');
        setTimeout(() => {
    ...
    <div className="relative min-h-screen overflow-hidden" 
      ...
      <div className="fixed inset-0 pointer-events-none" style={{ zIndex: 0 }}>
        ...
        <div className="absolute inset-0" style={{
          background: isMobile 
            ? 'linear-gradient(180deg, rgba(35,20,8,0.7) 0%, rgba(35,20,8,0.85) 100%)'
            : 'radial-gradient(ellipse at 20% 20%, rgba(80,50,20,0.85) 0%, rgba(50,30,15,0.92) 50%, rgba(25,15,8,0.95) 100%)',
        }} />
        <motion.div
                ...
                <div className="flex items-center gap-2">
                  <div className="w-20 h-2 bg-white/15 rounded-full overflow-hidden border border-white/20">
                    <motion.div className="h-full rounded-full" 
                      style={{ background: 'linear-gradient(to right, #A16207, #FFD700)' }}
                      animate={{ width: `${(itemIdx / 5) * 100}%` }} />
                  </div>
        ...
        <AnimatePresence mode="wait">
          ...
          {phase === 'listen' && item && (
            <motion.div key={`listen-${groupIdx}-${itemIdx}`}
              ...
              <div className={`flex ${isMobile ? 'flex-col' : 'flex-row'} items-center justify-center gap-6 md:gap-10`}>
                ...
                <HouseCard item={item} size={isMobile ? 200 : 280} mode="word" />
                ...
                <div className={`flex flex-col items-center gap-4 ${isMobile ? 'w-full max-w-sm' : 'flex-1 max-w-md'}`}>
                  ...
                  <div className="px-5 py-2 rounded-2xl backdrop-blur-md border-2"
                    ...
                    <span className="font-black text-white text-sm md:text-base">🎧 استمع واختر اسم الغرفة/الشيء الصحيح</span>
                  </div>
          ...
          {phase === 'build' && item && (
            <motion.div key={`build-${groupIdx}-${itemIdx}`}
              ...
              <div className={`flex ${isMobile ? 'flex-col' : 'flex-row'} items-center justify-center gap-6 md:gap-10`}>
                ...
                <HouseCard item={item} size={isMobile ? 220 : 280} mode="sentence" />
                ...
                <div className={`flex flex-col items-center gap-3 ${isMobile ? 'w-full max-w-sm' : 'flex-1 max-w-md'}`}>
                  ...
                  {status === 'correct' && (
                    <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }}
                      ...
                      <span className="font-black text-white text-sm">¡Mi casa!</span>
                    </motion.div>
                  )}
                </div>
              </div>
          ...
          {phase === 'group-success' && (
            <motion.div key="success" initial={{ scale: 0.5, opacity: 0 }} animate={{ scale: 1, opacity: 1 }}
              ...
              <h2 className="text-3xl md:text-5xl font-black text-white">¡Bienvenido a casa!</h2>
              ...
              <button 
                onClick={() => {
                  if (groupIdx < SPANISH_HOUSE_GROUPS.length - 1) {
                    setGroupIdx(g => g + 1); 
                    setItemIdx(0); 
                    setPhase('listen');
                    setCorrectInGroup(0);
                ...
                style={{ background: 'linear-gradient(135deg, #A16207, #FFD700)', boxShadow: '0 10px 30px rgba(161,98,7,0.4)' }}
              >
                المجموعة التالية 🚀
              </button>
            </motion.div>
          ...
          {phase === 'all-done' && (
            <motion.div key="done" initial={{ opacity: 0 }} animate={{ opacity: 1 }}
              ...
              <div className="flex flex-wrap justify-center gap-2 max-w-2xl mx-auto mb-4">
                {SPANISH_HOUSE_ITEMS.map((h, i) => (
                  <motion.div key={h.word}
                    initial={{ scale: 0, rotate: -180 }}
                    animate={{ scale: 1, rotate: 0 }}
                    transition={{ delay: i * 0.05, type: 'spring' }}
                    className="px-3 py-1.5 rounded-xl border-2 border-white/40 flex items-center gap-1.5"
                    style={{
                      background: `linear-gradient(135deg, ${h.gradient[0]}, ${h.gradient[1]})`,
                      boxShadow: `0 4px 15px ${h.color}88`,
                    }}
                  >
                    <span className="text-lg">{h.emoji}</span>
                    <span className="text-xs font-black text-white" dir="ltr">{h.word}</span>
                  </motion.div>
                ))}
              </div>

              <div className="text-7xl md:text-8xl">🏚️</div>
              <h1 className="text-3xl md:text-5xl font-black text-white">¡Casa completa!</h1>
              <p className="text-lg md:text-xl text-white/60 max-w-md mx-auto">تعلمت 15 جزء من البيت + 15 جملة كاملة!</p>
              <p className="text-sm text-yellow-300/80 max-w-md mx-auto">
                دلوقتي تقدر توصف بيتك بالإسباني! 🎉
              </p>
              ...
              <button 
                ...
                style={{ 
                  background: 'linear-gradient(135deg, #A16207, #FFD700)', 
      ...
      <div className="fixed bottom-0 left-0 right-0 z-30 px-2 md:px-4 pb-1.5 pointer-events-none"
        ...
        <div className={`mx-auto pointer-events-auto ${isMobile ? 'max-w-md' : 'max-w-[1500px]'}`}>
          <div className="relative rounded-xl px-4 md:px-6 py-1.5 md:py-2"
            style={{
              background: 'linear-gradient(135deg, rgba(60,35,15,0.85), rgba(40,25,10,0.9))',
