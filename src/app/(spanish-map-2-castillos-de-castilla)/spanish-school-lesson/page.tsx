...
import { 
  ...
  GraduationCap, X, BookOpen
} from 'lucide-react';

import { 
  SPANISH_SCHOOL_ITEMS, 
  SPANISH_SCHOOL_GROUPS, 
  generateSpanishSchoolChoices,
  generateSchoolSentenceWordPool,
  checkSchoolSentenceOrder,
  type SpanishSchoolItem
} from '@/data/spanish-2/school';
...
const LESSON_ID = 'es-salamanca-school';
...
function getCategoryStyle(category: 'Util' | 'Lugar' | 'Persona') {
  switch (category) {
    case 'Util':
      return { bg: 'rgba(124,58,237,0.7)', border: '#7C3AED', icon: '📚', label: 'أدوات' };
    case 'Lugar':
      return { bg: 'rgba(59,130,246,0.7)', border: '#3B82F6', icon: '🏫', label: 'أماكن' };
    case 'Persona':
      return { bg: 'rgba(236,72,153,0.7)', border: '#EC4899', icon: '👨‍🏫', label: 'أشخاص/مواد' };
  }
}

// ═══════════════════════════════════════
// 🎓 School Card - بطاقة جامعية أكاديمية
// ═══════════════════════════════════════
function SchoolCard({ 
  item, 
  size = 280, 
    ...
    >
      ...
      >
        ...
        <div className="absolute top-3 left-1/2 -translate-x-1/2 z-20 flex flex-col items-center gap-1">
          ...
          >
            <GraduationCap size={12} className="text-amber-800" /> المدرسة
          </div>
        ...
        <div className="flex-1 flex flex-col items-center justify-center w-full pt-20 pb-2 gap-2">
          ...
          <div className="flex gap-1.5">
            ...
            >
              {item.indefiniteArticle === 'un' ? '1️⃣ un' : 
               item.indefiniteArticle === 'una' ? '1️⃣ una' : '➕ unas'}
            </div>
          </div>
        </div>
...
function SpanishSchoolLessonInner() {
  ...
  const [itemChoices, setItemChoices] = useState<SpanishSchoolItem[]>([]);
  ...
  const group = SPANISH_SCHOOL_GROUPS[groupIdx];
  ...
  useEffect(() => {
    if (item && phase === 'listen') {
      setItemChoices(generateSpanishSchoolChoices(item.word, 3));
  ...
  useEffect(() => {
    if (item && phase === 'build') {
      setAvailableWords(generateSchoolSentenceWordPool(item));
      setSelectedWords([]);
      setStatus('idle');
  ...
  const handleItemChoice = (choice: SpanishSchoolItem, e: React.MouseEvent) => {
  ...
  const handleSelectWord = (word: string, idx: number) => {
    ...
    if (newSelected.length === item.sentenceWords.length) {
      const isCorrect = checkSchoolSentenceOrder(newSelected, item.sentenceWords);
      if (isCorrect) {
        setStatus('correct');
        setTimeout(() => {
    ...
    <div className="relative min-h-screen overflow-hidden" 
      ...
      <div className="fixed inset-0 pointer-events-none" style={{ zIndex: 0 }}>
        ...
        <div className="absolute inset-0" style={{
          background: isMobile 
            ? 'linear-gradient(180deg, rgba(30,15,40,0.7) 0%, rgba(30,15,40,0.85) 100%)'
            : 'radial-gradient(ellipse at 20% 20%, rgba(70,35,90,0.85) 0%, rgba(45,20,60,0.92) 50%, rgba(25,10,35,0.95) 100%)',
        }} />
        <motion.div
                ...
                <div className="flex items-center gap-2">
                  <div className="w-20 h-2 bg-white/15 rounded-full overflow-hidden border border-white/20">
                    <motion.div className="h-full rounded-full" 
                      style={{ background: 'linear-gradient(to right, #7C3AED, #FFD700)' }}
        ...
        <AnimatePresence mode="wait">
          ...
          {phase === 'listen' && item && (
            <motion.div key={`listen-${groupIdx}-${itemIdx}`}
              ...
              <div className={`flex ${isMobile ? 'flex-col' : 'flex-row'} items-center justify-center gap-6 md:gap-10`}>
                ...
                <SchoolCard item={item} size={isMobile ? 200 : 280} mode="word" />
                ...
                <div className={`flex flex-col items-center gap-4 ${isMobile ? 'w-full max-w-sm' : 'flex-1 max-w-md'}`}>
                  ...
                  <div className="px-5 py-2 rounded-2xl backdrop-blur-md border-2"
                    ...
                    <span className="font-black text-white text-sm md:text-base">🎧 استمع واختر الكلمة الصحيحة</span>
                  </div>
          ...
          {phase === 'build' && item && (
            <motion.div key={`build-${groupIdx}-${itemIdx}`}
              ...
              <div className={`flex ${isMobile ? 'flex-col' : 'flex-row'} items-center justify-center gap-6 md:gap-10`}>
                ...
                <SchoolCard item={item} size={isMobile ? 220 : 280} mode="sentence" />
          ...
          {phase === 'group-success' && (
            <motion.div key="success" initial={{ scale: 0.5, opacity: 0 }} animate={{ scale: 1, opacity: 1 }}
              ...
              <h2 className="text-3xl md:text-5xl font-black text-white">¡Muy bien estudiado!</h2>
              ...
              <button 
                onClick={() => {
                  if (groupIdx < SPANISH_SCHOOL_GROUPS.length - 1) {
                    setGroupIdx(g => g + 1); 
                    setItemIdx(0); 
                    setPhase('listen');
                    setCorrectInGroup(0);
                ...
                style={{ background: 'linear-gradient(135deg, #7C3AED, #FFD700)', boxShadow: '0 10px 30px rgba(124,58,237,0.4)' }}
              >
                المجموعة التالية 🚀
              </button>
            </motion.div>
          ...
          {phase === 'all-done' && (
            <motion.div key="done" initial={{ opacity: 0 }} animate={{ opacity: 1 }}
              ...
              <h1 className="text-3xl md:text-5xl font-black text-white">¡Salamanca completa!</h1>
              <p className="text-lg md:text-xl text-white/60 max-w-md mx-auto">تعلمت 15 كلمة من عالم المدرسة + 15 جملة كاملة!</p>
              <p className="text-sm text-yellow-300/80 max-w-md mx-auto">
                دلوقتي تقدر تتكلم عن مدرستك وأدواتك ومعلميك! 🎉
              </p>
      ...
      <div className="fixed bottom-0 left-0 right-0 z-30 px-2 md:px-4 pb-1.5 pointer-events-none"
        ...
        <div className={`mx-auto pointer-events-auto ${isMobile ? 'max-w-md' : 'max-w-[1500px]'}`}>
          <div className="relative rounded-xl px-4 md:px-6 py-1.5 md:py-2"
            style={{
              background: 'linear-gradient(135deg, rgba(50,25,70,0.85), rgba(35,15,50,0.9))',
