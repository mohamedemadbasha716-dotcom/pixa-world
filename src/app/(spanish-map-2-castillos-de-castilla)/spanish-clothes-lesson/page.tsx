...
import { 
  Check, Volume2, Trophy, Home, Flame, Gem, Mic, SkipForward, Sparkles, Shirt, X, BookOpen
} from 'lucide-react';

import { 
  SPANISH_CLOTHES, 
  SPANISH_CLOTHES_GROUPS, 
  generateSpanishClothesChoices,
  generateClothesSentenceWordPool,
  checkClothesSentenceOrder,
  type SpanishClothingItem
} from '@/data/spanish-2/clothes';
...
const LESSON_ID = 'es-traje-clothes';
...
function getCategoryStyle(category: 'Superior' | 'Inferior' | 'Accesorios') {
  switch (category) {
    case 'Superior':
      return { bg: 'rgba(59,130,246,0.7)', border: '#3B82F6', icon: '👕' };
    case 'Inferior':
      return { bg: 'rgba(245,158,11,0.7)', border: '#F59E0B', icon: '👖' };
    case 'Accesorios':
      return { bg: 'rgba(168,85,247,0.7)', border: '#A855F7', icon: '🧤' };
  }
}
    ...
    >
      ...
      >
        ...
        <div className="absolute top-3 left-1/2 -translate-x-1/2 z-20 flex flex-col items-center gap-1">
          ...
          >
            <Shirt size={12} className="text-amber-800" /> الملابس
          </div>
          ...
          >
            {catStyle.icon} {item.category} • {item.categoryAr}
          </div>
        </div>
        ...
        <div className="flex-1 flex flex-col items-center justify-center w-full pt-20 pb-2 gap-2">
          ...
          >
            {item.gender === 'M' ? '♂ مذكر (un)' : '♀ مؤنث (una)'}
          </div>
        </div>
...
function SpanishClothesLessonInner() {
  ...
  const [itemChoices, setItemChoices] = useState<SpanishClothingItem[]>([]);
  ...
  const group = SPANISH_CLOTHES_GROUPS[groupIdx];
  ...
  useEffect(() => {
    if (item && phase === 'listen') {
      setItemChoices(generateSpanishClothesChoices(item.word, 3));
  ...
  useEffect(() => {
    if (item && phase === 'build') {
      setAvailableWords(generateClothesSentenceWordPool(item));
      setSelectedWords([]);
      setStatus('idle');
  ...
  const handleItemChoice = (choice: SpanishClothingItem, e: React.MouseEvent) => {
  ...
  const handleSelectWord = (word: string, idx: number) => {
    ...
    if (newSelected.length === item.sentenceWords.length) {
      const isCorrect = checkClothesSentenceOrder(newSelected, item.sentenceWords);
      if (isCorrect) {
        setStatus('correct');
        setTimeout(() => {
        ...
        <AnimatePresence mode="wait">
          ...
          {phase === 'listen' && item && (
            <motion.div key={`listen-${groupIdx}-${itemIdx}`}
              ...
              <div className={`flex ${isMobile ? 'flex-col' : 'flex-row'} items-center justify-center gap-6 md:gap-10`}>
                ...
                <ClothingCard item={item} size={isMobile ? 200 : 280} mode="word" />
                ...
                <div className={`flex flex-col items-center gap-4 ${isMobile ? 'w-full max-w-sm' : 'flex-1 max-w-md'}`}>
                  ...
                  <div className="px-5 py-2 rounded-2xl backdrop-blur-md border-2"
                    ...
                    <span className="font-black text-white text-sm md:text-base">🎧 استمع واختر قطعة الملابس الصحيحة</span>
                  </div>
          ...
          {phase === 'build' && item && (
            <motion.div key={`build-${groupIdx}-${itemIdx}`}
              ...
              <div className={`flex ${isMobile ? 'flex-col' : 'flex-row'} items-center justify-center gap-6 md:gap-10`}>
                ...
                <ClothingCard item={item} size={isMobile ? 220 : 280} mode="sentence" />
          ...
          {phase === 'group-success' && (
            <motion.div key="success" initial={{ scale: 0.5, opacity: 0 }} animate={{ scale: 1, opacity: 1 }}
              ...
              <h2 className="text-3xl md:text-5xl font-black text-white">¡Muy elegante!</h2>
              ...
              <button 
                onClick={() => {
                  if (groupIdx < SPANISH_CLOTHES_GROUPS.length - 1) {
                    setGroupIdx(g => g + 1); 
                    setItemIdx(0); 
                    setPhase('listen');
                    setCorrectInGroup(0);
          ...
          {phase === 'all-done' && (
            <motion.div key="done" initial={{ opacity: 0 }} animate={{ opacity: 1 }}
                ...
                {SPANISH_CLOTHES.map((p, i) => (
                  <motion.div key={p.word}
                    ...
                    style={{
                      ...
                      boxShadow: `0 4px 15px ${p.color}88`,
                    }}
                  >
              ...
              <h1 className="text-3xl md:text-5xl font-black text-white">¡Armario completo!</h1>
              <p className="text-lg md:text-xl text-white/60 max-w-md mx-auto">تعلمت 15 قطعة ملابس + 15 جملة كاملة بالجرامر!</p>
              <p className="text-sm text-yellow-300/80 max-w-md mx-auto">
                دلوقتي تقدر تستخدم Llevo علشان توصف ملابسك بالإسباني! 🎉
              </p>
