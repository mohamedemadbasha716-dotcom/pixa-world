...
} from '@/data/spanish-2/consuegra-test';
...
const LESSON_ID = 'es-consuegra-test';
...
// 🌬️ Windmill Component - طاحونة هواء متحركة
function AnimatedWindmill({ size = 200 }: { size?: number }) {
  return (
    <motion.div
    ...
    >
      ...
      {/* مراوح دوّارة (زخرفة) */}
      <motion.div
        className="absolute top-[8%] left-1/2 -translate-x-1/2 text-4xl md:text-5xl opacity-60"
        animate={{ rotate: 360 }}
      ...
      <motion.div
        ...
        className="absolute bottom-2 left-1/2 -translate-x-1/2 text-2xl opacity-70"
      >
        🌾
      </motion.div>
    </motion.div>
  ...
  return (
    <div className="w-full max-w-md mx-auto">
      <div className="flex items-center justify-between mb-1.5 px-1">
        ...
        <span className="text-[10px] md:text-xs font-black text-green-300">
      ...
      <div className="w-full h-2.5 md:h-3 bg-black/40 rounded-full overflow-hidden border border-white/20">
        <motion.div 
          ...
          style={{ background: 'linear-gradient(to right, #4ADE80, #22C55E, #15803D)' }}
          initial={{ width: 0 }}
...
function SpanishConsuegraTestInner() {
    ...
    <div className="relative min-h-screen overflow-hidden" 
      ...
      <div className="fixed inset-0 pointer-events-none" style={{ zIndex: 0 }}>
        ...
        <div className="absolute inset-0" style={{
          background: isMobile 
            ? 'linear-gradient(180deg, rgba(10,25,15,0.85) 0%, rgba(5,20,10,0.92) 100%)'
            : 'radial-gradient(ellipse at 20% 20%, rgba(20,60,30,0.9) 0%, rgba(10,40,20,0.95) 50%, rgba(5,25,10,0.98) 100%)',
        }} />
        {/* تأثير أخضر متوهج في الخلفية */}
        <motion.div
          className="absolute inset-0 opacity-20"
          style={{ background: `radial-gradient(ellipse 100% 60% at 50% 30%, rgba(34,197,94,0.4), transparent 70%)` }}
        ...
        <AnimatePresence mode="wait">
          ...
          {phase === 'intro' && (
            <motion.div key="intro"
              ...
              <AnimatedWindmill size={isMobile ? 150 : 200} />
              ...
              <div className="bg-black/50 backdrop-blur-md rounded-2xl p-4 md:p-6 border-2 border-green-400/30 space-y-3 text-right">
                <h3 className="text-base md:text-lg font-black text-green-300 text-center">📋 معلومات الاختبار</h3>
                <div className="grid grid-cols-2 gap-3 text-sm md:text-base">
                  ...
                  <div className="bg-white/5 rounded-xl p-2 text-center">
                    <div className="text-2xl md:text-3xl font-black text-white">6</div>
              ...
              <p className="text-sm md:text-base text-white/70 max-w-md mx-auto">
                ⚔️ الاختبار بيغطي كل اللي اتعلمته في قلاع قشتالة: الجسم، الملابس، الطعام، المدرسة، المنزل، والمشاعر!
              </p>
          ...
          {phase === 'round-intro' && currentRound && (
            <motion.div key={`round-intro-${currentRoundIdx}`}
              ...
              <div className="space-y-2">
                <p className="text-green-300 font-black text-sm md:text-base">
              ...
              <div className="bg-black/50 backdrop-blur-md rounded-2xl p-4 border-2 border-green-400/30">
                <p className="text-sm md:text-base text-white">
                  📝 {currentRound.description}
                </p>
                <p className="text-xs md:text-sm text-green-300 font-bold mt-2">
          ...
          {phase === 'round-complete' && (
            <motion.div key="round-complete"
              ...
              <p className="text-lg md:text-xl text-green-300 font-bold">
                ...
                return (
                  <>
                    <motion.div
                      ...
                      transition={{ type: 'spring', duration: 1 }}
                    >
                      <AnimatedWindmill size={isMobile ? 130 : 180} />
                    </motion.div>
                    ...
                    <div className="bg-black/60 backdrop-blur-md rounded-2xl p-4 md:p-6 border-2 border-green-400/40 space-y-3">
                      <h3 className="text-base md:text-lg font-black text-green-300">📊 النتيجة النهائية</h3>
                    ...
                    {isExcellent && (
                      <motion.div
                        ...
                        transition={{ delay: 1 }}
                        className="bg-gradient-to-r from-green-500/30 to-yellow-500/30 backdrop-blur-md rounded-2xl p-4 border-2 border-green-400/50"
                      >
                        <p className="text-base md:text-lg font-black text-green-300 mb-1">
                          🏆 إنجاز أسطوري! 🏆
                        </p>
                        <p className="text-xs md:text-sm text-white">
                          أنهيت <span className="font-black text-green-300">قلاع قشتالة</span> بنجاح! جاهز لسواحل المتوسط 🌊⛵
                        </p>
                      </motion.div>
                    )}
      ...
      {phase === 'question' && (
        <ToroBull mood={toroMood} message={toroMessage} idleGlowColor={currentQuestion?.color || '#22C55E'} />
      )}
      
      <ConfettiBurst trigger={confettiTrigger} x={confettiPos.x} y={confettiPos.y} 
        colors={['#4ADE80', '#22C55E', '#FFD700', '#DC2626', '#ffffff']} />
    </div>
  );
}

export default function SpanishConsuegraTestPage() {
  return (
    <Suspense fallback={<div className="min-h-screen bg-[#07090D] flex items-center justify-center text-white">Loading...</div>}>
      <SpanishConsuegraTestInner />
    </Suspense>
  );
}