...
import { COUNTRIES, COUNTRY_GROUPS, type CountryItem } from '@/data/german/berlin-countries';
...
const TOTAL_ANSWERS_PER_LESSON = TOTAL_COUNTRIES * 3;
const LESSON_ID = 'berlin-countries';
...
const DARK_COLORS: Record<string, string> = {
  '#FFD700': '#7D5310', '#3B82F6': '#1E3A8A', '#EF4444': '#7F1D1D',
  '#22C55E': '#15803D', '#DC2626': '#7F1D1D', '#F59E0B': '#92400E',
  '#06B6D4': '#155E75', '#8B5CF6': '#4C1D95', '#A78BFA': '#5B21B6',
  '#F472B6': '#9D174D', '#10B981': '#064E3B', '#0EA5E9': '#075985',
};
...
function similarityScore(a: string, b: string): number {
  const norm = (s: string) => normalizeGerman(s);
...
function checkAnswer(input: string, item: CountryItem): boolean {
...
function generateChoices(correctId: string, all: CountryItem[], count = 3): CountryItem[] {
...
function getGroupBackground(groupIdx: number, isMobile: boolean): string {
  ...
  return `/card-image/countries/bg-group${Math.min(groupIdx + 1, 3)}-${suffix}.webp`;
}
...
// 🖼️ HeroDisplay
// ═══════════════════════════════════════
function HeroDisplay({ item, isMobile, showWord = false }: { item: CountryItem; isMobile?: boolean; showWord?: boolean }) {
...
function ChoiceCard({ item, allItems, isMobile, onCorrect, onWrong }: {
  item: CountryItem; allItems: CountryItem[]; isMobile: boolean;
  onCorrect: (cx: number, cy: number) => void; onWrong: () => void;
}) {
  const [choices, setChoices] = useState<CountryItem[]>([]);
  ...
  const handleChoice = (c: CountryItem, e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    <div className={`w-full ${isMobile ? 'max-w-md' : 'max-w-3xl'} mx-auto ${isMobile ? 'p-3' : 'p-6'} rounded-[1.5rem] md:rounded-[2rem] relative overflow-hidden`}
      ...
      <div className={`relative z-10 flex flex-col items-center ${isMobile ? 'gap-2.5' : 'gap-4'}`}>
        <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}
          ...
          <span className={`font-black ${isMobile ? 'text-xs' : 'text-base'} text-gray-800`}>استمع جيداً واختر العلم الصحيح</span>
        </motion.div>
        ...
        <span className={`font-black text-white ${isMobile ? 'text-xs' : 'text-base'}`}>اضغط على العلم الصحيح 👇</span>
...
function WordBuilderMobile({ item, onComplete, onWrong }: {
  item: CountryItem; onComplete: (cx: number, cy: number) => void; onWrong: () => void;
}) {
  // بناء على أول كلمة فقط (لو فيه مسافة)
...
function ListenPhase({ item, allItems, onDone, onKarlReact, onCombo, onCorrect, isMobile }: {
  item: CountryItem; allItems: CountryItem[]; onDone: () => void;
          ...
          <div className="grid lg:grid-cols-5 gap-8 items-center">
            <div className="lg:col-span-3 flex flex-col items-center gap-4">
              ...
              <div className="flex items-center gap-3">
                {item.artikel && <ArtikelBadge artikel={item.artikel} />}
...
function SpeakPhase({ item, isMobile, onSuccess, onSkip }: {
  item: CountryItem; isMobile: boolean;
...
function MatchGame({ group, onComplete, onCorrect, onKarlReact, onCombo }: {
  group: CountryItem[]; onComplete: () => void;
  onCorrect: (x: number, y: number) => void;
  onKarlReact: (m: KarlMood) => void; onCombo: () => void;
}) {
  const isMobile = useIsMobile();
  const [matched, setMatched] = useState<Set<string>>(new Set());
  const [shuffledWords, setShuffledWords] = useState<CountryItem[]>(() => shuffle(group));
  ...
  const tryMatch = (src: DragSource, tgt: DragSource, cx: number, cy: number) => {
    ...
    if (src.id === tgt.id) {
      const it = group.find(x => x.id === src.id)!;
      speakGerman(it.de); playCoinSound(); onCombo(); onKarlReact('happy'); onCorrect(cx, cy);
      setConfettiPos({ x: cx, y: cy }); setConfettiTrigger(t => t + 1);
      setSuccessPair(src.id); setTimeout(() => setSuccessPair(null), 600);
  ...
  const renderCard = (item: CountryItem, side: 'image' | 'word') => {
      ...
      <motion.div key={`${side}-${item.id}`} data-match-target={item.id} data-side={side}
        ...
        {side === 'image'
          ...
          : <div className="w-full h-full flex flex-col items-center justify-center px-1">
              {item.artikel && <div className="text-[8px] font-black uppercase opacity-80 text-white">{item.artikel}</div>}
  ...
  return (
    <>
      <ConfettiBurst trigger={confettiTrigger} x={confettiPos.x} y={confettiPos.y} colors={['#FFD700', '#A78BFA', '#F472B6', '#FFF']} />
      <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }}
        ...
        <div className="flex items-center gap-3 w-full max-w-md px-2">
          <div className="px-3 py-1 rounded-full flex items-center gap-1.5"
            ...
            <span className="text-[10px] font-black text-white">طابق الأعلام بالكلمات</span>
          </div>
        ...
        <div className="w-full flex flex-col items-center gap-1">
          <span className="text-[9px] text-cyan-300/80 font-black uppercase">الأعلام</span>
          <div className="flex items-center justify-center gap-1.5 flex-wrap" dir="ltr">{group.map(c => renderCard(c, 'image'))}</div>
        </div>
        ...
        <div className="w-full flex flex-col items-center gap-1">
          <span className="text-[9px] text-pink-300/80 font-black uppercase">بالألمانية — اسحب</span>
...
// 🏠 Main
// ═══════════════════════════════════════
function GermanCountriesLessonInner() {
  ...
  const currentGroup = COUNTRY_GROUPS[groupIdx];
  ...
  const handleCombo = () => { /* simplified */ };

  const calcRating = (s: number) => { const r = s / (COUNTRIES.length * 3); return r >= .67 ? 3 : r >= .34 ? 2 : 1; };
  ...
  const nextGroup = async () => {
    if (groupIdx < COUNTRY_GROUPS.length - 1) {
    ...
    <div className="text-white relative" style={{ fontFamily: "'Tajawal',sans-serif", minHeight: '100vh' }} dir="rtl">
      ...
      <div className="flex flex-col items-center justify-center relative px-3 md:px-6 mx-auto w-full"
        ...
        <AnimatePresence mode="wait">
          {phase === 'listen' && <ListenPhase key={`l-${groupIdx}-${itemIdx}`} item={currentItem} allItems={COUNTRIES} onDone={handleListenDone} onKarlReact={handleKarlReact} onCombo={handleCombo} onCorrect={handleCorrect} isMobile={isMobile} />}
          ...
          {phase === 'speak' && <SpeakPhase key={`s-${groupIdx}-${itemIdx}`} item={currentItem} isMobile={isMobile} onSuccess={(cx, cy) => { handleCorrect(cx, cy); handleKarlReact('celebrate'); setTimeout(handleSpeakDone, 800); }} onSkip={handleSpeakDone} />}
          ...
          {testSuccess && (
            <motion.div key="success" initial={{ opacity: 0, scale: .85 }} animate={{ opacity: 1, scale: 1 }}
              ...
              <motion.button whileHover={{ scale: 1.04 }} whileTap={{ scale: .97 }} onClick={nextGroup}
                ...
                style={{ background: 'linear-gradient(135deg,#06D6A0,#3B82F6)', boxShadow: '0 10px 40px rgba(6,214,160,0.5)' }}>
                {groupIdx < COUNTRY_GROUPS.length - 1 ? 'المجموعة التالية ←' : '🗺️ رجوع للخريطة'}
              </motion.button>
            </motion.div>
          )}
        </AnimatePresence>
...
export default function GermanCountriesLesson() {
  return (
