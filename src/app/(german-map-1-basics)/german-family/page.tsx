...
// الصفحة الرئيسية
// ═══════════════════════════════════════
type Phase = 'learn-word' | 'quiz' | 'grammar' | 'group-success' | 'group-fail' | 'all-done';
...
export default function GermanFamilyPage() {
  ...
  const LESSON_ID = 'german-family'; // ✅ اتصلحت من 'berlin'
  ...
  // ✅ بعد الـ quiz: لو فيه grammar أو dialogue → روح grammar
  const handleQuizPass = () => {
    const hasGrammarOrDialogue = 
      (group.grammarItems && group.grammarItems.length > 0) ||
      (group.dialogueItems && group.dialogueItems.length > 0);
    
    if (hasGrammarOrDialogue) {
      setPhase('grammar');
      savePosition(groupIdx, itemIdx, 'grammar');
    } else {
  ...
  const handleGrammarComplete = (correctCount: number) => {
    const totalGrammarItems = 
      (group.grammarItems?.length ?? 0) + 
      (group.dialogueItems?.length ?? 0);
    
    if (correctCount === totalGrammarItems) {
    ...
    <div className="min-h-screen text-white relative" style={{ fontFamily: "'Tajawal', sans-serif" }} dir="rtl">
        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div className="flex gap-1.5 justify-center flex-wrap">
            {group.items.map((item, i) => {
              const isDone = phase === 'quiz' || phase === 'grammar' || phase === 'group-success' || i < itemIdx;
      ...
      <div className="pt-36 pb-32 px-6 min-h-screen flex flex-col justify-center relative" style={{ zIndex: 10 }}>
        <AnimatePresence mode="wait">
          ...
          {/* ✅ Phase الجديدة: Grammar/Dialogue */}
          {phase === 'grammar' && (
            <GrammarMiniPhase
              key={`grammar-${groupIdx}`}
              section={group}
              isMobile={typeof window !== 'undefined' && window.innerWidth < 1024}
              onComplete={handleGrammarComplete}
              onCorrect={handleStarEarned}
              onKarlReact={handleKarlReact}
            />
          )}
          {phase === 'group-success' && (
